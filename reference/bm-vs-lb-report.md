...
## Rules for LB-Safe Optimization

1. **Never cache output buffers** — allocate fresh every call
2. **Never use CUDA graphs** — they replay fixed execution plans
3. **Only cache truly constant metadata** — things derived from shapes/configs, not data
4. **Don't optimize for specific data patterns** — LB data is random every iteration
5. **Test with realistic warmup assumptions** — your first few calls may be cold
6. **Expect +1-5% overhead on LB** — this is structural and unavoidable
7. **A BM improvement that relies on cache warmth may show as regression on LB** — especially for memory-bound shapes < 20µs
