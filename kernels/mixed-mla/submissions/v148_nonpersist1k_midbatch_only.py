...
"""
v148: Restrict non-persistent a16w8 to the medium-batch 1k shapes, while keeping the large-batch 8k ps=128 experiment.
- Use non-persistent a16w8 only for bs=32/64 with kv=1024.
- Keep bs<=4,kv=1024 on bf16 non-persistent and bs=256,kv=1024 on bf16 persistent.
- Keep bf16_persist page_size=128 only for bs>=64,kv>=4096, with page_size=64 for smaller 8k batches.
"""
...
def _get_path(batch_size, kv_seq_len):
    """Decide which path to use: 'bf16', 'bf16_persist', 'a16w8', or 'a16w8_nonpersist'."""
    if batch_size <= 4 and kv_seq_len <= 1024:
        return 'bf16'
    if kv_seq_len >= 4096:
        return 'bf16_persist'
    if kv_seq_len >= 1024 and batch_size < 256:
        return 'a16w8_nonpersist'
    if kv_seq_len >= 1024:
        return 'bf16_persist'
    return 'a16w8'
...
def _get_cached_a16w8_nonpersist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build cached buffers for a16w8 non-persistent path."""
    key = ('a16w8_nonpersist', batch_size, kv_seq_len)
    if key in _cache:
        return _cache[key]
