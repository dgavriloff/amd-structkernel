...
"""
v150: Persistent-only hybrid baseline with bf16_persist page_size=128 only for bs=256,kv>=4096.
- Keep the corrected persistent-only routing from v149.
- Limit the larger bf16 persistent page size to the single heaviest 8k batch.
- Avoid touching bs=4/32/64 8k shapes, where prior larger-page experiments have been less reliable.
"""
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 128 if kv_seq_len >= 4096 and batch_size >= 256 else 64
