...
"""
v136: Use config["kv_seq_len"] for NSPLIT — no GPU-CPU sync at all.

v123 cached NSPLIT after first call, but first call per shape still did
kv_indptr .item() sync. Now use config["kv_seq_len"] directly.
Same NSPLIT values: 8 for kv<=2048, 16 for kv>2048.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # NSPLIT from config — no GPU-CPU sync at all
