...
"""
v132: XCD-aware PID remapping for stage1.
MI300X/MI355X dispatches consecutive PIDs round-robin across XCDs (chiplets).
Default ordering scatters heads for the same (batch, split) across all XCDs,
causing each XCD to independently fetch the same KV cache from HBM.
Remap PIDs so each XCD handles a contiguous block of logical work, grouping
heads that share KV data onto the same XCD for L2 cache reuse.
"""
...
@triton.jit
def _remap_xcd(pid, NUM_XCDS: tl.constexpr):
    """Remap PID so each XCD handles a contiguous block of logical work."""
    total = tl.num_programs(0)
    pids_per_xcd = (total + NUM_XCDS - 1) // NUM_XCDS
    tall_xcds = total % NUM_XCDS
    # Hardware dispatches pid round-robin: XCD = pid % NUM_XCDS
    xcd = pid % NUM_XCDS
    local_pid = pid // NUM_XCDS
    # Remap: XCD i handles logical pids [i*pids_per_xcd, (i+1)*pids_per_xcd)
    # For tall XCDs (0..tall_xcds-1): pids_per_xcd items each
    # For short XCDs (tall_xcds..NUM_XCDS-1): pids_per_xcd-1 items each
    new_pid = tl.where(
        tall_xcds == 0,
...
def _stage1(
    ...
    pid = _remap_xcd(tl.program_id(0), NUM_XCDS)
