...
"""
v122: Eliminate kv_indptr .item() GPU-CPU sync by using fixed NSPLIT=16.
- Fixed NSPLIT=16 for all cases — kernel already handles empty splits with early return.
- Removes 2x .item() calls that triggered GPU-CPU sync per invocation.
- Buffer cache uses single key since NSPLIT is now constant.
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Cache intermediate buffers — single key since NSPLIT is constant
    buf_key = tq
