...
"""
v097: Fix CUDA graph correctness by including Q quantization inside graph.

Key changes over v096:
1. Pre-allocate fp8 Q and scale buffers with fixed pointers.
2. Use dynamic_per_tensor_quant (in-place) to write into pre-allocated buffers.
3. Include the quantization call INSIDE the CUDA graph so each replay
   re-quantizes Q from current bf16 data -- fixes stale cache bug in v096.
4. fp8 path for all shapes, non-persistent (splits=1) and persistent (splits>1).
...
def _build_meta_persistent(batch_size, q_seq_len, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, num_kv_splits):
    ...
    # Pre-allocate fp8 Q buffers for in-place quantization
...
# CUDA graph cache: key -> (graph, output_tensor)
_graph_cache = {}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if num_splits == 1:
        ...
        # Warmup: quantize Q in-place then run attention
        ...
        # Capture graph: includes Q quantization + attention
        ...
        # Warmup: quantize Q in-place then run stage1+reduce
        dynamic_per_tensor_quant(q_fp8, q_reshaped, q_scale)
        ...
        # Capture graph: includes Q quantization + stage1 + reduce
        graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(graph):
            dynamic_per_tensor_quant(q_fp8, q_reshaped, q_scale)
            mla_decode_stage1_asm_fwd(
