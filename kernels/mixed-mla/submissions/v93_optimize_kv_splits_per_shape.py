...
"""
v093: Optimize NUM_KV_SPLITS per shape for fp8, use fp8 for all shapes.

Key changes from v092:
1. Use per-shape num_kv_splits for fp8 path instead of fixed 32.
   - For bs=256, kv=8192: only 1 split needed (all CUs occupied by batches).
   - For bs=4, kv=1024: ~8 splits to fill CUs.
   - Fewer splits = less reduce overhead.
2. Use a8w8 (fp8) for ALL shapes including bs<=4, kv<=1024.
   The bf16 path was slower due to higher bandwidth requirements.
3. Account for fp8 min_block_n constraint from aiter's get_meta_param.
...
def _choose_num_kv_splits(batch_size, kv_seq_len):
    """Choose optimal num_kv_splits using aiter's heuristic + fp8 constraints."""
    total_kv = batch_size * kv_seq_len
    avg_kv = total_kv / batch_size
    overhead = 84.1

    # Find best splits using CU occupancy * efficiency heuristic
    ...
    # Apply fp8 min_block_n constraint (from aiter get_meta_param)
    # nhead * max_seqlen_q = 16 * 1 = 16
    min_block_n = 128  # for nhead*qseqlen=16 with fp8
...
def _get_cached(batch_size, q_seq_len, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, num_kv_splits):
    key = (batch_size, q_seq_len, kv_seq_len, num_kv_splits)
    if key in _cache:
        return _cache[key]
    cached = _build_meta(
