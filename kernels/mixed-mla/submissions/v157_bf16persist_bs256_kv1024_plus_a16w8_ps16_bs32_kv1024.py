...
"""
v157: Combine two selective 1k routing changes on the recovered hybrid baseline.

1. Use a16w8 persistent page_size=16 only for bs=32,kv=1024.
2. Route bs=256,kv=1024 to bf16_persist instead of a16w8.

The ps=16 attempt only failed when bs=64 was included, so keep it isolated to
bs=32. Separately, the heaviest 1k shape may benefit from bf16 persistent's
simpler path and fewer pages without disturbing the smaller 1k cases.
"""
