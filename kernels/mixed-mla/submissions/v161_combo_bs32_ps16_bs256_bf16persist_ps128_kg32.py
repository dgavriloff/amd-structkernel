...
"""
v161: Keep the 1k selective routes, but cap kv_granularity on the bs=256 bf16 path.
Based on the recovered hybrid baseline.

1. Use a16w8 persistent page_size=16 only for bs=32,kv=1024.
2. Route bs=256,kv=1024 to bf16_persist with page_size=128.
3. For that bs=256 bf16-persistent 1k shape only, use kv_granularity=32 instead
   of tying it to page_size=128.

This tests whether the large page count reduction helps without forcing overly
coarse metadata work chunks on the heaviest 1k shape.
"""
...
def _build_persistent_meta(
    batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, kv_granularity=None
):
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    kv_granularity = 32 if kv_seq_len == 1024 and batch_size >= 256 else None
