...
"""
v116: Micro-optimizations for closing gap to #1.
- NSPLIT=4 for kv<=1024, NSPLIT=8 for kv<=4096, NSPLIT=16 for kv>4096.
- Pre-allocate and cache mid buffers to avoid per-call allocation overhead.
- num_stages=3 for better K/V load pipelining.
- Minimize Python-side tensor operations.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Cache mid buffers to avoid allocation overhead
    cache_key = (tq, NSPLIT, "mid")
    ...
    mid_v, mid_lse = _cache[cache_key]

    # Ensure buffers are large enough
    needed_v = tq * NSPLIT * NUM_HEADS * V_HEAD_DIM
    needed_lse = tq * NSPLIT * NUM_HEADS
    if mid_v.numel() < needed_v or mid_lse.numel() < needed_lse:
        mid_v = torch.empty(needed_v, dtype=torch.bfloat16, device="cuda")
        mid_lse = torch.empty(needed_lse, dtype=torch.float32, device="cuda")
        _cache[cache_key] = (mid_v, mid_lse)
