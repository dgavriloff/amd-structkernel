...
"""
v135: Adaptive NSPLIT with NSPLIT=1 for small workloads.
- NSPLIT=1 for bs<=4,kv<=1024: direct output, skip stage2 entirely.
- NSPLIT=4 for bs<=32,kv<=1024: fewer splits, less reduction.
- NSPLIT=8 for kv<=2048: moderate parallelism.
- NSPLIT=16 for kv>2048: high parallelism for long sequences.
Based on v123 Triton MXFP4 K + FP8 V kernel.
"""
...
def _stage1(
    ...
    if DIRECT_OUTPUT:
        # NSPLIT=1: write directly to output
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf_key not in _buf_cache:
        if NSPLIT == 1:
            # Direct output mode: mid_v IS the output buffer
    ...
    if NSPLIT == 1:
        # Direct output: stage1 writes to o, no stage2
        _stage1[(bs * NUM_HEADS,)](
            q, kv_pk, kv_sc, kv_fp8,
            kv_indptr, qo_indptr,
            o.view(-1), mid_lse if mid_lse is not None else fp8_scale_tensor,  # mid_lse unused
            fp8_scale_tensor,
