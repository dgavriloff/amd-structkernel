...
"""
v178: Use 1-stage fmoe_g1u1 kernel for ksplit=0 shapes: single kernel does
quant+gate_up_GEMM+SwiGLU+down_GEMM+reduction, eliminating inter-stage bf16->fp4
requantization round-trip and second kernel launch overhead.
"""
import os
import functools
import torch
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if metadata.ksplit > 1:
        ...
        # 1-stage fmoe_g1u1 path: single kernel for quant+GEMM1+SwiGLU+GEMM2+reduce
        # Avoids inter-stage bf16->fp4 requantization round-trip
        # ASM kernel uses un-shuffled weights + un-shuffled scales
        w1_raw = gate_up_weight
        w2_raw = down_weight
        w1_scale_flat = gate_up_weight_scale.view(dtypes.fp8_e8m0).view(E, -1)
        w2_scale_flat = down_weight_scale.view(dtypes.fp8_e8m0).view(E, -1)

        # Quantize activations to fp4 (non-fused, separate from sorting)
        a1, a1_scale = per_1x32_f4_quant_hip(hidden_states)

        # Sort activation scales to match moe_sorting output
        a1_scale = fp4_utils.moe_mxfp4_sort(
            a1_scale, sorted_ids, num_valid_ids,
            token_num, block_size_M,
        )

        # Single-kernel fused MoE: gate_up GEMM + SwiGLU + down GEMM + weighted reduce
        aiter.fmoe_g1u1(
            moe_out,
            a1, w1_raw, w2_raw,
            ...
            topk,
            a1_scale, w1_scale_flat, w2_scale_flat,
