...
"""
v190: Keep the v183 bundle, but directly patch the returned cktile metadata to
force block_m=32 only for the tiniest sparse E=257 cktile shape (bs=16).
This isolates whether the bs16 sparse path alone benefits when block_m is
overridden through returned metadata instead of ignored cfg entries.
"""
import os
import functools
import torch
...
def _inject_configs():
    ...
    def _patched_get_2stage_cfgs(
        ...
        if expert == 257 and token == 16 and metadata.ksplit == 2 and metadata.block_m != 32:
            metadata = _fused_moe_module.MOEMetadata(
                metadata.stage1,
                metadata.stage2,
                32,
