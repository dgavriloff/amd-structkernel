...
"""
v189: Keep the v183 bundle, but directly patch the returned cktile metadata to
force block_m=32 for sparse E=257 cktile shapes (bs=16 and bs=128).
Earlier E257 block_m attempts only wrote cfg_2stages entries, which historical
notes say the cktile path ignores unless metadata is rewritten directly.
"""
import os
import functools
import torch
...
def _inject_configs():
    ...
    def _patched_get_2stage_cfgs(
        ...
        # cktile_moe ignores cfg_2stages block_m for sparse shapes; patch the
        # returned metadata directly to test larger sorting granularity.
        if expert == 257 and token in (16, 128) and metadata.ksplit == 2 and metadata.block_m != 32:
            metadata = _fused_moe_module.MOEMetadata(
                metadata.stage1,
                metadata.stage2,
                32,
