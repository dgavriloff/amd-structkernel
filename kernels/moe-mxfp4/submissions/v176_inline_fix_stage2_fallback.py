...
"""
v176: Inline cktile_moe_stage1 for ksplit path + inline ck_moe_stage1 and flydsl_stage2
for CK 2-stage path. Pre-allocate cktile tmp_out/out buffers. Fix stage2 dispatch to
fall back to metadata.stage2 partial for non-FlyDSL shapes (e.g. check_implementation).
"""
import os
import functools
import torch
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if metadata.ksplit > 1:
        ...
        # Stage 2: dispatch based on kernel type (FlyDSL vs CK)
        ...
        if _s2_parsed is not None:
            # FlyDSL stage2 path (inlined)
            ...
            # CK stage2 fallback: use metadata.stage2 partial directly
            metadata.stage2(
                a2_quant, w1, w2,
                sorted_ids, sorted_expert_ids, num_valid_ids,
                moe_out, topk,
