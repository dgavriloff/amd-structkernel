...
"""
v289: Real split-8 with an `8x32` gluon reduction retile.

Keep the current fused-hint baseline and the true 8-way split-K geometry, then
retile only the split-K reduction stage to increase parallelism on the
16x2112x7168 path.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 32  # Retile the real split-8 reduction path for more parallel blocks
