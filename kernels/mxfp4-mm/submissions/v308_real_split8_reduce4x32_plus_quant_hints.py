...
"""
v308: Re-test the real split-8 branch with restored quant-kernel hints.

Keep the restored v263-style fused-quant hints, but switch the large-K path to
the real split-8 geometry and pair it with the more parallel `4x32` reduction.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Re-test the true split-8 branch with the restored fused-quant hints.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 128,
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 32  # Match the denser reduction used on the real split-8 branch
