...
"""
v189: Update M<=32 K>1024 config from legacy BSM=32/BSN=64/BSK=512/NW=8/NS=1
to BSM=8/BSN=128/BSK=256/NW=4/NS=2/.cg — match proven K<=1024 config.

Hypothesis: 32x7168x2048 (and 32xNx4096) shapes use a legacy config from v21 era
with only 112 blocks (0.44 waves) and no software pipelining (NS=1). Updating to
BSM=8/BSN=128/BSK=256/NW=4/NS=2 doubles grid to 224 blocks (0.875 waves) and
enables double-buffered pipelining. This matches the config proven optimal for
M<=32 K<=1024 in v099.
"""
import torch
import triton
