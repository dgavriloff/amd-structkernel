...
"""
v314: Restore stride-alignment hints on the split-K=7 reduce4x32 waves=1 + assume branch.

Start from the current best benchmarked split-K=7 `4x32` reduction path, keep
the lower waves-per-EU hint, and add only true extent/inner-stride assumptions
to the fused quant kernel, then restore the compile-time stride alignment facts.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Re-test the repaired split-K=7 branch with a lower occupancy hint.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    # Reduce kernel params — sweep the denser reduction tile from the best BM branch
    REDUCE_BSM = 4
    REDUCE_BSN = 32
