...
"""
v285: Keep `REDUCE_BSM=4` and narrow the split-K reduction to `REDUCE_BSN=16`.

Stay on the known-correct padded reduction metadata path and increase only the
reduction kernel's N parallelism for the 16x2112x7168 split-K shape.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 16  # Narrower reduction tile on the padded split-K path
