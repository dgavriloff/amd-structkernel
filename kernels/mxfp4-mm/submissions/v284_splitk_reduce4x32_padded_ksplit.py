...
"""
v284: Retile the split-K reduction to `REDUCE_BSM=4` and `REDUCE_BSN=32`.

Stay on the known-correct padded reduction metadata path and probe a smaller
row tile for the 16x2112x7168 split-K reduction.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 32  # Smaller row tile for the M=16 split-K reduction
