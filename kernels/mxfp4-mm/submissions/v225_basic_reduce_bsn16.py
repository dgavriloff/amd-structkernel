...
"""
v225: Switch split-K reduce from gluon (BSN=64) to basic (BSN=16).
Library note: BSN=16 gives best perf with fp32 partials.
Grid goes from 1x33 to 1x132 reduce blocks.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    # Reduce kernel params — basic reduce with BSN=16 (library note: best for fp32 partials)
    REDUCE_BSM = 16
    REDUCE_BSN = 16  # Library note: BSN=16 best perf with fp32 partials
