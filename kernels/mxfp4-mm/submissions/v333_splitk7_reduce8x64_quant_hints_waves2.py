...
"""
v333: Keep the split-K=7 `8x64` branch and restore `waves_per_eu=2`.

Stay on the restored fused-quant hints with the wider `8x64` reduction tile,
but re-check the higher explicit large-K occupancy hint on this coarser
reduction geometry.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Keep the 8x64 split-K=7 branch and restore the higher waves hint.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 64  # Complete the 8x64 occupancy sweep on the valid split-K branch
