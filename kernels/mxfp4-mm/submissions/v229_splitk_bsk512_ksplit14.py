...
"""
v228: Split-K (16x2112x7168): AMD-tuned BSK=512 KSPLIT=14 NS=1 waves=1
(from BSK=256 KSPLIT=7 NS=2 waves=2). 1 K-iter per split eliminates
inner loop. 476 blocks (1.86 waves) vs 238 (0.93 waves).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # AMD-tuned config for 16x2112x7168 (gfx950-GEMM-A16WFP4_PRESHUFFLED-N=2112-K=7168.json)
        # BSK=512 KSPLIT=14: 1 K-iter per split, 476 blocks (1.86 waves)
        # num_stages=1: no pipelining needed with 1 iter
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
