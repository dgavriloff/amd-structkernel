...
"""
v315: Let the compiler choose waves on the split-K=7 reduce4x32 full-hints branch.

Start from the current best benchmarked split-K=7 `4x32` reduction path, keep
the full quant-kernel hint set, and remove the explicit large-K waves hint to
test compiler-selected residency on the stronger reduction geometry.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Keep the stronger 4x32 branch but let the compiler choose residency.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
