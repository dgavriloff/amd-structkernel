...
"""
v324: Restore the `v263` fused-path hint branch for leaderboard-oriented evaluation.

Keep the `v211` geometry and cache policy fixed, but add only the fused quant
row-stride `tl.multiple_of` facts and the `tl.max_contiguous` lane hints.
"""
import torch
import triton
