...
"""
v321: Keep the strongest `4x32` split-K geometry and remove the regressing tl.assume bundle.

Return to the strongest `4x32` reduction geometry, keep the alignment and
contiguous-lane hints, drop the tl.assume extent/unit-stride facts that
regressed v314, and let the compiler choose wave residency.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Return to the strongest 4x32 branch without the regressing tl.assume bundle.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    # Reduce kernel params — strongest benchmarked reduction geometry on the repaired branch
    REDUCE_BSM = 4
    REDUCE_BSN = 32
