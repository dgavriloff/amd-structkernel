...
"""
v221: waves_per_eu=1 for M<=32 K<=1024 fused path (from 2).

Keeps the v211 cache=None change on the K=512 path, but relaxes the occupancy
hint one step to reduce the chance that waves_per_eu=2 is over-constraining
register allocation on the two M=32 leaderboard shapes.
"""
import torch
import triton
