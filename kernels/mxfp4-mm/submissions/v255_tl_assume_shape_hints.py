...
"""
v255: Extend the compiler-hint baseline with shape-multiple facts.

Keep the `tl.assume` guidance from v253 and add more explicit multiple-of hints
for the hot fused quant path's tile-aligned shape components and packed fp4
store width.
"""
import torch
import triton
...
def _fused_mxfp4_quant_shuffle_kernel(
    ...
    for pid_n in tl.range(start_n, min(start_n + NUM_ITER, N), num_stages=NUM_STAGES):
        ...
        tl.multiple_of(out_offs_n, BLOCK_SIZE_N // 2)
        out_offs = (
