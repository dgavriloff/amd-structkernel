...
"""
v307: Keep quant hints and retile the repaired split-K=7 branch to `8x32`.

Hold the restored v263-style fused-quant hints fixed, but switch the repaired
split-K=7 branch back to the stronger recent `8x32` gluon reduction control.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 32  # Recheck the strongest benchmarked repair with quant hints restored
