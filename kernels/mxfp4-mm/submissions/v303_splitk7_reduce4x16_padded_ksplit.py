...
"""
v303: Benchmark the valid split-K=7 `4x16` reduction control.

Keep the repaired `BLOCK_SIZE_K=256` split-K path and the required padded
power-of-two `MAX_KSPLIT`, then retile only the reduction stage to the denser
`4x16` geometry.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 16  # Valid split-K=7 densest nearby reduction control
