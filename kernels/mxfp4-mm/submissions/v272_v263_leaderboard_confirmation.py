...
"""
v272: Queue the exact `v263` fused-path hint set for leaderboard confirmation.

Restore the strongest recent benchmark candidate unchanged so the next scarce
submission slot can be spent on confirming the best fixed-input result rather
than another weaker speculative variant.
"""
import torch
import triton
