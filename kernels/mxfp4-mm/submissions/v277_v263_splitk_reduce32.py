...
"""
v277: Sweep the split-K reduction midpoint with `REDUCE_BSN=32`.

Keep the validated `v263` fused-path hint set unchanged and probe only the
remaining safe reduction-width midpoint for the 16x2112x7168 split-K path.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 16
    REDUCE_BSN = 32  # Midpoint reduce tile for the split-K partial buffer
