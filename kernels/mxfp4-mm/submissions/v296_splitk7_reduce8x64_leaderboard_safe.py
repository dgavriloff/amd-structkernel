...
"""
v296: Promote the safer valid split-K=7 `8x64` branch after the `8x32` leaderboard revert.

Keep the repaired `BLOCK_SIZE_K=256` split-K path, avoid the leaderboard-unstable
`8x32` reduction retile, and move the live candidate to the lower-risk `8x64`
reduction geometry that already passed the server test gate.
"""
import torch
import triton
