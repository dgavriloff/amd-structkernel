...
"""
v316: Restore waves=2 on the split-K=7 reduce4x32 full-hints branch.

Start from the current best benchmarked split-K=7 `4x32` reduction path, keep
the full quant-kernel hint set, and close the controlled large-K occupancy
sweep by restoring the explicit `waves_per_eu=2` point on the same geometry.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Close the occupancy sweep on the stronger 4x32 branch.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
