...
"""
v302: Benchmark the valid split-K=7 `4x32` reduction control.

Keep the repaired `BLOCK_SIZE_K=256` split-K path and the required padded
power-of-two `MAX_KSPLIT`, then retile only the reduction stage to the more
parallel `4x32` geometry.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 32  # Valid split-K=7 high-parallel reduction control
