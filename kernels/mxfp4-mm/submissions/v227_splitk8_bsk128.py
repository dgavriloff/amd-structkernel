...
"""
v227: Split-K path uses BLOCK_SIZE_K=128 with NUM_KSPLIT=8.

This creates a real 8-way split-K configuration for the 16x2112x7168 shape;
with BSK=256, aiter's normalization collapses nominal split-8 back to split-7.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Real split-K=8 with BSK=128 for large-K shapes (e.g., 16x2112x7168)
        # BSM=8 gives 272 blocks instead of the split-7 path's 238.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 128,
