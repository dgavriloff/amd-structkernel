...
"""
v329: Keep the split-K=7 `4x64` branch but let the compiler choose waves.

Stay on the restored quant-hints path with the new `4x64` reduction geometry,
but remove the explicit large-K `waves_per_eu` hint so the compiler can pick
residency for the widened reduction tile.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Keep the 4x64 split-K=7 branch and remove the explicit waves hint.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 64  # Keep the four-row tile and re-check width with compiler-chosen waves
