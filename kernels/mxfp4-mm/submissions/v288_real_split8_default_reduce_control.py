...
"""
v287: Real split-8 control on the current baseline with default reduction tiling.

Use `BLOCK_SIZE_K=128` with `NUM_KSPLIT=8` for the K>4096 split-K shape, but
restore the split-K reduction kernel to the default `16x64` tile.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 16
    REDUCE_BSN = 64  # Control: default split-K reduction tile on real split-8
