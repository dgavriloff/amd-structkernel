...
"""
v322: Keep the `4x32` split-K winner and make `MAX_KSPLIT` exact.

Keep the `4x32` reduction winner and no-assume quant hints fixed, but stop
padding `MAX_KSPLIT` from 7 to 8 in the reduce kernel metadata so the
tail-masked eighth lane is not staged at all.
"""
import torch
import triton
