...
"""
v294: Valid split-K=7 control with a `16x32` gluon reduction retile.

Stay on the repaired `BLOCK_SIZE_K=256` split-K path and separate the effects
of row-tile and width-tile changes by keeping the default row tile while
narrowing only the reduction width.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 16
    REDUCE_BSN = 32  # Valid split-K=7 control isolating the narrower reduce width
