...
"""
v291: Real split-8 with a `4x32` gluon reduction retile.

Keep the current fused-hint baseline and the true 8-way split-K geometry, then
probe the most aggressive nearby reduction tile to see whether the extra split
benefits from still more reduction parallelism on the 16x2112x7168 path.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Real split-K=8 on the current v280 baseline for the large-K shape.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 128,
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 32  # Most aggressive nearby retile for the real split-8 reduction path
