...
"""
v215: BSK=128 NW=4 NS=2 for M=64 fused path (from BSK=256).

Halves per-iteration work, doubles K-iterations from 8 to 16.
Smaller A/B/scale loads per iteration may improve pipelining.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=16 BSN=128 BSK=128 NW=4 NS=2
        # 4*56=224 blocks, 16 K-iters with pipelining
        # BSK=128: halved per-iter work, doubled iterations for better pipeline fill
