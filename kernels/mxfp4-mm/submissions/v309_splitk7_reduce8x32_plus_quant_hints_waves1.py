...
"""
v309: Re-test split-K=7 `8x32` with restored hints and lower waves-per-EU.

Return to the repaired split-K=7 branch with the `8x32` reduction tile, keep
the restored fused-quant hints, and lower only the large-K occupancy hint.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 32  # Hold the strongest recent reduction tile fixed
