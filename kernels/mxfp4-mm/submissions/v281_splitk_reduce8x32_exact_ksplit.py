...
"""
v281: Combine the `8x32` split-K reduction retile with exact partial-count metadata.

Keep the current fused-path baseline intact, retain the `8x32` split-K reduce
tile, and trim `MAX_KSPLIT` down to the actual number of partials.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 32  # Retile split-K reduction for the M=16 large-K shape
