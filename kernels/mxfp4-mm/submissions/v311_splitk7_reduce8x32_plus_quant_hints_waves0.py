...
"""
v311: Keep split-K=7 `8x32` with restored quant hints, but drop waves-to-EU.

Hold the current BM-leading `8x32` repaired branch fixed and remove only the
large-K occupancy hint so the compiler can choose wave residency itself.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Return to the repaired split-K=7 benchmark leader for promotion.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
