...
"""
v273: Keep the `v263` fused path and shrink split-K reduction tiles.

Preserve the stronger fused quant stride hints from `v263` and narrow only the
16x2112x7168 split-K fp32 partial reduction tile to test whether the reduction
kernel is the remaining hot spot after the fused-path hint improvements.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 16
    REDUCE_BSN = 16  # Narrower reduce tile for the split-K partial buffer
