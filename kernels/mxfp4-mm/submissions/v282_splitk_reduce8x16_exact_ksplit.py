...
"""
v282: Narrow the `8x32` exact split-K reduction retile down to `8x16`.

Keep the current fused-path baseline intact, preserve exact partial-count
metadata, and increase only the split-K reduction's N parallelism.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 16  # Further narrow split-K reduction for the M=16 large-K shape
