...
"""
v310: Re-test split-K=7 `4x32` with restored hints and lower waves-per-EU.

Keep the repaired split-K=7 branch and restored fused-quant hints, but pair
the lower large-K occupancy hint with the denser `4x32` reduction tile.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 4
    REDUCE_BSN = 32  # Sweep the lower occupancy hint on the denser reduction tile
