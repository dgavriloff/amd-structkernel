...
"""
v290: Real split-8 with an `8x64` gluon reduction retile.

Keep the current fused-hint baseline and the true 8-way split-K geometry, then
change only the reduction row tile so the split-K branch can be compared
against both the default `16x64` and the denser `8x32` follow-up.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 64  # Change only the reduction row tile on the real split-8 path
