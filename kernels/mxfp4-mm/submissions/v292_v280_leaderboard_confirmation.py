...
"""
v292: Restore the current best fixed-input split-K candidate for leaderboard confirmation.

Return to the valid split-K=7 / `BLOCK_SIZE_K=256` path and keep the strongest
recent reduction retile, `8x32`, as the confirmation candidate.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 32  # Restore the strongest recent split-K reduction retile
