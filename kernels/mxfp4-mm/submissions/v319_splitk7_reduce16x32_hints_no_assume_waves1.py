...
"""
v319: Keep the `16x32` split-K branch, but remove the regressing tl.assume bundle.

Start from the current best benchmarked split-K=7 `4x32` reduction path, keep
the `16x32` reduction geometry and the alignment/contiguous hints, but drop the
tl.assume extent and unit-stride facts that regressed v314 on benchmark.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Keep the 16x32 reduction geometry with the lower occupancy hint.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
