...
"""
v323: Restore the exact `v310` split-K=7 `4x32` winner for leaderboard confirmation.

Return to the best measured branch: keep the repaired split-K=7 path, keep the
restored quant alignment/contiguous hints, and restore the lower explicit
waves-per-EU hint that produced the strongest benchmark result.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Restore the exact v310 large-K branch for leaderboard confirmation.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
