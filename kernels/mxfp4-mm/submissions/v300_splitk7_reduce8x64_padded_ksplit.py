...
"""
v300: Restore the valid split-K=7 `8x64` branch with padded power-of-two metadata.

Drop the invalid exact-partial-count follow-ups, keep the repaired
`BLOCK_SIZE_K=256` split-K path, and return to the already test-passed `8x64`
reduction geometry with the required padded `MAX_KSPLIT`.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 8
    REDUCE_BSN = 64  # Valid split-K=7 8x64 reduction geometry with padded metadata
