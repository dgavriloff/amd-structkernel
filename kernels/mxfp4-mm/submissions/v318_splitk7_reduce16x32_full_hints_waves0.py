...
"""
v318: Let the compiler choose waves on the `16x32` full-hints split-K branch.

Start from the current best benchmarked split-K=7 `4x32` reduction path, keep
the full quant-kernel hint set, and keep the `16x32` reduction geometry while
removing the explicit large-K occupancy hint.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Keep the 16x32 reduction geometry, but let the compiler choose waves.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
