...
"""
v305: Stage split-K=4 on the repaired BSK=256 path with default reduction.

Keep the power-of-two split-K=4 main-kernel change from `v304`, but restore
the default `16x64` reduction geometry so the split-count effect can be
measured without the noisier `8x32` reduction retile.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Stage a power-of-two split count on the repaired BSK=256 path.
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 16
    REDUCE_BSN = 64  # Default reduction geometry as the split-count control
