...
"""
v115: tl.dot for V accum + BN tuning.
- Use tl.dot(p[1,BN], V[BN,512]) for V accumulation instead of scalar mul+reduce.
- Leverages matrix multiply hardware for V accumulation.
- Keep MXFP4 dot_scaled for Q*K^T, FP8 for V source data.
...
def _stage1(
    ...
    m_i = float("-inf")
    l_i = 0.0
    acc_v = tl.zeros([1, V_DIM], dtype=tl.float32)
    ...
    for blk_s in range(ss, se, BN):
        ...
        # V from FP8: load [BN, V_DIM] fp8, convert to bf16
        ov = tl.arange(0, V_DIM)
        ...
        # V accum via tl.dot: p[1,BN] @ V[BN,V_DIM] -> [1,V_DIM]
        p_2d = p[None, :].to(tl.bfloat16)
        acc_v += tl.dot(p_2d, v_bf16).to(tl.float32)
        m_i = m_new

    inv_l = 1.0 / l_i
    ov = tl.arange(0, V_DIM)
    result = tl.reshape(acc_v, [V_DIM])
    tl.store(Mid_lse + lse_off, m_i + tl.log(l_i))
    tl.store(Mid_v + v_off + ov, (result * inv_l).to(tl.bfloat16))
