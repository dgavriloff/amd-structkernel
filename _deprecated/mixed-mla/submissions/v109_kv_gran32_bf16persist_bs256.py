...
"""
v109: kv_granularity=32 for bf16_persist bs>=256 (sub-page scheduling).
Based on v089.

With ps=64, kv_granularity=64 by default (1 work block per page).
kv_granularity=32 creates 2 work blocks per page for finer scheduling.
Only for bs>=256 where the large token count benefits from better load balancing.
"""
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 64
    kv_gran = 32 if batch_size >= 256 else None
