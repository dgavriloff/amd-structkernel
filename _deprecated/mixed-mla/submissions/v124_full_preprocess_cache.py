...
"""
v124: Cache all tensor preprocessing and shape constants.
- Cache PKD, N_SC, FP8_STRIDE on first call (constant across all shapes).
- Minimize Python ops in hot path: fewer dict lookups, no conditional branches after first call.
- Keep NSPLIT cache, buffer cache, fp8_scale tensor pointer.
"""
...
_buf_cache = {}
_nsplit_cache = {}
_shape_cache = {}  # stores (PKD, N_SC, FP8_STRIDE) after first call
...
def _preprocess_once(kv_data):
    """Extract and cache shape constants from first call."""
    ...
    kv_fp8_raw, _ = kv_data["fp8"]
    kv_fp8 = kv_fp8_raw.view(torch.float8_e4m3fnuz).reshape(kv_fp8_raw.shape[0], -1)
    PKD = kv_pk.shape[-1]
    N_SC = kv_sc.shape[-1]
    FP8_STRIDE = kv_fp8.shape[-1]
    _shape_cache["PKD"] = PKD
    _shape_cache["N_SC"] = N_SC
    _shape_cache["FP8_STRIDE"] = FP8_STRIDE
    _shape_cache["pk_needs_view"] = kv_pk_raw.dtype != torch.uint8
    _shape_cache["sc_needs_view"] = kv_sc_raw.dtype != torch.uint8
    _shape_cache["pk_needs_reshape"] = kv_pk_raw.dim() > 2 if kv_pk_raw.dtype == torch.uint8 else kv_pk.dim() > 2
    _shape_cache["sc_needs_reshape"] = kv_sc_raw.dim() > 2 if kv_sc_raw.dtype == torch.uint8 else kv_sc.dim() > 2
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # First call: extract and cache shape constants
    if "PKD" not in _shape_cache:
        _preprocess_once(kv_data)

    PKD = _shape_cache["PKD"]
    N_SC = _shape_cache["N_SC"]
    FP8_STRIDE = _shape_cache["FP8_STRIDE"]

    # Prepare MXFP4 tensors (minimal ops, shapes known)
    kv_pk_raw, kv_sc_raw = kv_data["mxfp4"]
    kv_pk = kv_pk_raw.view(torch.uint8) if _shape_cache["pk_needs_view"] else kv_pk_raw
    kv_sc = kv_sc_raw.view(torch.uint8) if _shape_cache["sc_needs_view"] else kv_sc_raw
    if _shape_cache["pk_needs_reshape"]:
        kv_pk = kv_pk.reshape(kv_pk.shape[0], -1)
    if _shape_cache["sc_needs_reshape"]:
        kv_sc = kv_sc.reshape(kv_sc.shape[0], -1)
