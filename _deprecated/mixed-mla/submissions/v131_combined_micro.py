...
"""
v131: Combined micro-optimizations:
1. Remove .to(tl.int64) from FP8 V offsets (max offset fits int32)
2. Pre-scale Q by sm_scale before loop (saves 1 f32 multiply per block iter)
3. Skip V store for empty splits (stage2 ignores via exp(-inf)=0)
"""
...
def _stage1(
    ...
    # Pre-scale Q by sm_scale to avoid per-block multiply
    q_vec = (q_vec * 0.041666666666666664).to(tl.bfloat16)
    ...
    for blk_s in range(ss, se, BN):
        ...
        kpt = tl.trans(kp)
        scores_2d = tl.dot_scaled(q_vec[None, :], None, "bf16",
                                   kpt, ks, "e2m1")
