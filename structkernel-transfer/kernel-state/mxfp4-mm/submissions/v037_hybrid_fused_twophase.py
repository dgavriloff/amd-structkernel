...
"""
v037: Hybrid fused/two-phase approach — best of both worlds.
- M <= 32: Use gemm_a16wfp4_preshuffle (fused quant+GEMM in single Triton kernel).
  Eliminates ~6.4µs quant kernel launch. 15-22% faster for these shapes.
- M >= 64: Use v021 two-phase (Triton quant + ASM 32x128 GEMM).
  ASM kernel has better tile utilization for larger M.
- v036 showed fused is 15-22% faster for M<=32 but 70-142% slower for M>=64.
- Target: win on all shapes by dispatching per-M.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            # Fused path: reshape B for preshuffle kernel
            # Will be populated on first call with actual tensors
            _buffers[key] = {
                'mode': 'fused',
                'out': torch.empty((M, N), dtype=torch.bfloat16, device=device),
                'B_w': None,  # populated on first call
                'B_sc': None,  # populated on first call
            }
        else:
            # Two-phase path (v021)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused':
        # Fused quant+GEMM via Triton preshuffle kernel
        if buf['B_w'] is None:
        ...
        # Two-phase: Triton quant + ASM GEMM (v021)
