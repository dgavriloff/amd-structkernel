...
"""
v189: Gluon reduce BSN=128 + .wt on M=256 quant stores.

Based on v188 (9.01µs LB). Change: REDUCE_BSN=128 for gluon reduce kernel
(v188 used BSN=64). BSN=128 means 17 reduce blocks (vs 33 for BSN=64).
With 16 M-rows, BSM=16 gives 1 M-block, so reduce_grid=(1,17).
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 16
    REDUCE_BSN = 128  # Try larger blocks for fewer reduce blocks
