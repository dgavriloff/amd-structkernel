...
"""
v093: Tune split-K reduce kernel block size for 16x2112x7168.

Hypothesis: The reduce kernel for split-K=7 uses REDUCE_BLOCK_SIZE_N=64, creating
only 33 blocks (1 M-tile x 33 N-tiles) for 256 CUs. The library notes
"REDUCE_BLOCK_SIZE_N=16 gives best perf with fp32 partials". With BSN=16, the
reduce grid becomes 132 blocks — better CU utilization.

Also try REDUCE_BLOCK_SIZE_M=1 for M=16 (no wasted rows in reduce).

Target: 16x2112x7168 shape (14.0us LB, biggest single shape contributor).
"""
import torch
import triton
