...
"""
v144: BSM=8 for M=256 quant kernel to improve CU utilization.

Hypothesis: M=256 two-phase quant uses BSM=32, giving grid (8, 24)=192 blocks
for K=1536. With BSM=8, grid becomes (32, 24)=768 blocks — 3 full waves on
256 CUs vs 0.75 waves. Better CU saturation should reduce quant kernel time.
Also try NW=4 with BSM=8 for more warp-level parallelism per block.

Target: 256x3072x1536 (M=256 two-phase path).
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            NUM_ITER = 1
            # BSM=8 for M>=128: more blocks (768 vs 192 for M=256 K=1536) to saturate CUs
            # BSM=32 for smaller M (but M>64 only reaches here for M=256 in practice)
            BLOCK_SIZE_M = 8
            BLOCK_SIZE_N = 64
            NUM_WARPS = 4  # 4 warps to hide memory latency with smaller BSM
            NUM_STAGES = 1
