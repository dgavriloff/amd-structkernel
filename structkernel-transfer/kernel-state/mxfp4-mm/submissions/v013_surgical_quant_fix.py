...
"""
v013: Fix M<=32 large-K quant config (surgical).
- v012 regressed: BSN=256/512 caused register spill across all shapes.
- v013: Keep all v006 configs, ONLY fix the M<=32 K>1024 path.
  Use BSN=128, 4 warps (same as M>32 path) instead of BSN=32, 1 warp.
  For M=16, K=7168: grid goes from (1,224)x1warp to (1,14)x4warps.
...
def _get_or_create_buffers(M, K, device):
    ...
    if key not in _buffers:
        ...
        if K <= 1024:
            ...
            # Use same config as M>32 path: BSN=128, 4 warps, iterated
            NUM_ITER = 4
            BLOCK_SIZE_M = 32
            BLOCK_SIZE_N = 128
            NUM_WARPS = 4
