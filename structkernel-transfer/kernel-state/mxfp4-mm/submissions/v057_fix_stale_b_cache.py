...
"""
v057: Fix stale B cache bug — recompute B_w/B_sc when B_shuffle changes.

v055/v056 both produced IDENTICAL leaderboard failures (11487 mismatched on
4x2880x512 seed 4565) despite completely different scale preparation. Root cause:
B_w and B_sc are cached per (M,K,N) shape key and only computed when B_w is None.
If leaderboard tests same shape with different seeds (different B data), cached
B_w/B_sc from first seed are stale → completely garbled output.
