...
"""
v207: .wt on M=256 quant scale stores (11th attempt).

Hypothesis: .wt cache_modifier on M=256 scale stores pushes scale data
to L2 faster for subsequent ASM GEMM read. Confirmed real -0.8% to -3.1%
on M=256 across 10 prior tests. M<=64 paths completely unchanged.
Based on v188 (gluon reduce + .wt fp4 stores).
"""
import torch
import triton
