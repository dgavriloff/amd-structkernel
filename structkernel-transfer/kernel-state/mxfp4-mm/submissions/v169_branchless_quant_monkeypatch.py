...
"""
v169: Monkey-patch _mxfp4_quant_op with branchless algorithm from MOE kernel.

Hypothesis: The library _mxfp4_quant_op uses a 3-way tl.where branch
(saturate/normal/denormal) with separate computation paths. The MOE kernel
in aiter has an optimized branchless variant that uses unified bitfield
extraction + tl.maximum for exponent clamping + single formula for e2m1
conversion. This reduces instruction count in the GEMM inner loop for all
fused shapes (M<=64) where quant is inline.

Monkey-patching the library module's _mxfp4_quant_op before first kernel
compilation makes the preshuffle kernel use the optimized version.

Target: All fused shapes (M<=64), especially 64x7168x2048 and 16x2112x7168.
"""
import torch
import triton
...
# === Optimized branchless _mxfp4_quant_op (from MOE kernel in aiter) ===
@triton.jit
def _fast_mxfp4_quant_op(
    ...
    """
    Branchless MXFP4 quantization. Same interface as _mxfp4_quant_op but uses
    unified bitfield extraction instead of 3-way tl.where branching.
    """
    NUM_QUANT_BLOCKS: tl.constexpr = BLOCK_SIZE_N // MXFP4_QUANT_BLOCK_SIZE
    x = x.reshape(BLOCK_SIZE_M, NUM_QUANT_BLOCKS, MXFP4_QUANT_BLOCK_SIZE)

    # Calculate scale (identical to original)
    ...
    # === Branchless FP32 -> MXFP4 conversion (from MOE kernel) ===
    ...
    # Handle denormals: if exponent < E8_BIAS, shift mantissa right
    adjusted_exponents = tl.core.sub(E8_BIAS, e + 1, sanitize_overflow=False)
    m = tl.where(e < E8_BIAS, (0x400000 | (m >> 1)) >> adjusted_exponents, m)

    # Clamp exponent: for normals subtract bias difference, for subnormals keep 0
    e = tl.maximum(e, E8_BIAS - E2_BIAS) - (E8_BIAS - E2_BIAS)

    # Combine sign, exponent, mantissa with rounding and saturation
    # Round nearest with tie breaking up: add 1 to bit right of LSB, shift right
...
# === Monkey-patch the library module so preshuffle kernel uses our fast quant ===
import aiter.ops.triton._triton_kernels.gemm.basic.gemm_a16wfp4 as _gemm_module
_gemm_module._mxfp4_quant_op = _fast_mxfp4_quant_op

# Now import the kernel (it will resolve _mxfp4_quant_op to our patched version)
from aiter.ops.triton._triton_kernels.gemm.basic.gemm_a16wfp4 import (
    _gemm_a16wfp4_preshuffle_kernel,
)

# Also patch the standalone quant module for the two-phase path
import aiter.ops.triton._triton_kernels.quant.quant as _quant_module
_quant_module._mxfp4_quant_op = _fast_mxfp4_quant_op

# Import original _mxfp4_quant_op reference for standalone quant kernel
# (our fused quant kernel also calls _mxfp4_quant_op)
_mxfp4_quant_op = _fast_mxfp4_quant_op
