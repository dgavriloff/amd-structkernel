...
"""
v213: HIP environment variable tuning for reduced kernel dispatch latency +
      .wt on M=256 scale stores (confirmed 15x).

Hypothesis: Two independent changes:
1. Set HIP_FORCE_DEV_KERNARG=1 at import time — forces kernel arguments into
   device memory instead of host pinned memory. On MI355X, device-side kernarg
   avoids PCIe round-trip for argument setup, potentially reducing per-launch
   overhead by 0.5-1us. Never tested before (85 failed leaves, but no env var
   tuning attempted).
2. .wt on M=256 scale stores — confirmed real -0.8 to -3.1% on M=256 across
   15 prior tests. Combined with env var change to potentially push past LB
   noise floor.
"""
import os
