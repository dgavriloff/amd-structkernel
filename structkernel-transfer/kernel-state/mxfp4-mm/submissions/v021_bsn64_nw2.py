...
"""
v021: Try BSN=64 NW=2 for quant kernel — double work per block with 2 warps.
- v019 uses BSN=32 NW=1: NUM_QUANT_BLOCKS=1, grid very large.
- BSN=64 NW=2: NUM_QUANT_BLOCKS=2, grid halved, 2 warps for latency hiding.
- 64x7168x2048: grid=(2,32)=64 blocks (vs 128). Still 25% CU util.
- 256x3072x1536: grid=(8,24)=192 blocks (vs 384). Still 75% CU util.
- Target: quant phase — amortize kernel launch overhead with larger blocks.
"""
import torch
import triton
