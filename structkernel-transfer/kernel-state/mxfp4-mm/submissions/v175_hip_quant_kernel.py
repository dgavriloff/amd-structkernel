...
"""
v175: REDUCE_BSM=8 for split-K reduce + always-recreate B views.

Hypothesis: Combine two independently marginal changes on v165 base:
1. REDUCE_BSM=8 for split-K reduce: 264 reduce blocks vs 132 with BSM=16.
   v159 showed -1.0% LB geomean on v099 base (9.08 vs 9.16), borderline noise.
   On v165 base (which already has waves_per_eu=2 for split-K GEMM), the
   improved reduce CU utilization may compound.
2. Remove data_ptr check for B_w/B_sc views: always recreate views inline.
   Eliminates dict.get() + data_ptr() + comparison (~100ns). v172 showed this
   was neutral, but combined with REDUCE_BSM=8 the marginal Python savings
   may help.

Target: 16x2112x7168 split-K shape (reduce phase).
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    # Reduce kernel params — REDUCE_BSM=8 for more CU utilization (264 blocks vs 132)
    REDUCE_BSM = 8
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        # Split-K path with REDUCE_BSM=8 (264 reduce blocks)
