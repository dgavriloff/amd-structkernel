...
"""
v185: Custom preshuffle kernel with fast_math=True on tl.dot_scaled.

Hypothesis: The aiter MoE a4w4 kernel uses fast_math=True on tl.dot_scaled
but the preshuffle GEMM kernel does not. fast_math=True enables the compiler
to use faster instruction sequences (e.g. fused operations, relaxed
intermediate precision). Since we're already in MXFP4, precision impact
should be minimal. This has never been tried before (0 occurrences in our
submission history). Apply to both fused_direct and fused_splitk paths.
"""
import torch
import triton
...
# Custom preshuffle kernel with fast_math=True on tl.dot_scaled
# Copied from aiter _gemm_a16wfp4_preshuffle_kernel with fast_math=True added
@triton.heuristics(
    {
...
def _custom_preshuffle_kernel(
    ...
    if (pid_k * SPLITK_BLOCK_SIZE // 2) < K:
        ...
        for k in range(pid_k * num_k_iter, (pid_k + 1) * num_k_iter):
            ...
            # KEY CHANGE: fast_math=True (from MoE a4w4 kernel pattern)
