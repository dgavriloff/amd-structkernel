...
"""
v094: Direct dispatch for ALL fused shapes (not just split-K).

Hypothesis: The gemm_a16wfp4_preshuffle wrapper adds Python overhead:
serialize_dict -> deserialize_str roundtrip, assertions, config mangling.
Direct dispatch of _gemm_a16wfp4_preshuffle_kernel with pre-computed params
eliminates this per-call overhead (~0.5-1µs).

Target: All fused shapes (M<=64), especially 4x2880x512, 32x4096x512, 32x2880x512.
"""
import torch
import triton
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            if config["NUM_KSPLIT"] > 1:
                ...
                # Non-split-K: direct dispatch (no wrapper overhead)
                K_kernel = K // 2
                BSK = config["BLOCK_SIZE_K"]
                BSN = max(config["BLOCK_SIZE_N"], 32)
                BSM = config["BLOCK_SIZE_M"]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # Non-split-K fused path: direct kernel dispatch (no wrapper overhead)
        b_ptr = B_shuffle.data_ptr()
        if buf['B_w'] is None or buf.get('_b_ptr') != b_ptr:
            buf['B_w'] = B_shuffle.view(torch.uint8).reshape(N // 16, (K // 2) * 16)
            bs_shape = B_scale_sh.shape
