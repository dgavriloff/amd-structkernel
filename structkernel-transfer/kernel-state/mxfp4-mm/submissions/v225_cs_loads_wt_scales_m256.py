...
"""
v225: evict_first A loads + .wt scale stores in M=256 quant kernel.

Based on v188 (gluon reduce + .wt fp4 stores). Two targeted changes to M=256 quant:
1. eviction_policy="evict_first" on A loads: A data is read once for quantization,
   then never accessed again. Evict-first reduces L2 pollution, leaving more
   cache for B_shuffle/B_scale_sh that ASM GEMM reads next.
2. .wt on scale stores (from .cg): confirmed -2.4% BM on M=256 across 21 tests.
   Combined with evict_first loads for potential compound effect on LB.
"""
import torch
import triton
...
def _fused_mxfp4_quant_shuffle_kernel(
    ...
    for pid_n in tl.range(start_n, min(start_n + NUM_ITER, N), num_stages=NUM_STAGES):
        ...
        if EVEN_M_N:
            x = tl.load(x_ptr + x_offs, eviction_policy="evict_first").to(tl.float32)
        else:
            x_mask = (x_offs_m < M)[:, None] & (x_offs_n < N)[None, :]
            x = tl.load(x_ptr + x_offs, mask=x_mask, eviction_policy="evict_first").to(
                tl.float32
            )
