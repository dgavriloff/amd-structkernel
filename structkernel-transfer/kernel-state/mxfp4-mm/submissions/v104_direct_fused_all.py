...
"""
v104: Direct dispatch for ALL fused shapes (including non-split-K M=64).

Hypothesis: The non-split-K fused path (M=64, M<=8, M<=32 K<=1024) currently uses
the gemm_a16wfp4_preshuffle wrapper, which does config serialize/deserialize,
arch check, shape recompute, and lambda grid on every call. Direct dispatch
pre-computes all parameters, eliminating Python-side overhead.

v094 tested this and found -0.9% (below 1% threshold). But that was from 9.62us.
Now at 9.16us, the same absolute savings should be a larger percentage.
Also, v094 may not have pre-computed strides and grid as aggressively.

Target: All fused shapes, especially M=64 (13.6us LB).
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # Non-split-K fused path: direct dispatch (bypass wrapper)
        b_ptr = B_shuffle.data_ptr()
        if buf['B_w'] is None or buf.get('_b_ptr') != b_ptr:
            buf['B_w'] = B_shuffle.view(torch.uint8).reshape(N // 16, (K // 2) * 16)
            bs_shape = B_scale_sh.shape
