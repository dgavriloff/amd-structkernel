...
"""
v200: num_stages=1 for M=64 fused path (eliminate double-buffering register pressure).

Hypothesis: M=64 (64x7168x2048) uses BSM=16 BSN=128 BSK=256 NW=4 NS=2 waves_per_eu=1.
NS=2 double-buffers the K-loop (8 iterations). With 224 blocks for 256 CUs (0.88 waves),
register pressure from double-buffering may limit occupancy. NS=1 reduces VGPR usage,
potentially allowing the compiler to achieve higher occupancy (2 waves/SIMD).
NS=1 was tested for M=32 K=512 (v134: +25-29% worse) and for M=64 with BSK=512 NW=8
(v161: +110% worse), but never for M=64 with BSK=256 NW=4. M=64 has 8 K-iterations
(vs 2 for M=32 K=512), making the pipeline less beneficial per iteration.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=16 BSN=128 BSK=256 NW=4 NS=1
        # 4*56=224 blocks, 8 K-iters without pipelining
        # NS=1 reduces register pressure vs NS=2, potentially improving occupancy
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
