...
"""
v011: Bypass gemm_a4w4 wrapper — direct ASM call with pre-allocated output.
- Pre-allocate GEMM output tensor per (M, N) shape.
- Call gemm_a4w4_asm directly, skipping wrapper overhead:
  - Saves torch.empty() allocation (~0.5-1µs per call)
  - Saves get_GEMM_config() LRU lookup
  - Saves get_gfx() check
- Use kernelName="" (auto-select, same as default behavior).
...
# Pre-allocated buffers keyed by (M, K) for quant, (M, N) for GEMM output
_buffers = {}
...
def _get_or_create_buffers(M, K, N, device):
    """Get pre-allocated quant + GEMM output buffers for given shape."""
    key = (M, K, N)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Direct ASM call with pre-allocated output — skip wrapper overhead
    gemm_a4w4_asm(
        ...
        B_scale_sh,
        buf['gemm_out'],
        "",  # kernelName="" triggers auto-selection (same as default)
        None,  # bias
        1.0,  # alpha
        0.0,  # beta
        True,  # bpreshuffle
