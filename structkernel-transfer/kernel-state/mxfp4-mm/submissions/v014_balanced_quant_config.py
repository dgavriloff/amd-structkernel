...
"""
v014: Fix M<=32 large-K quant config — balance occupancy vs block size.
- v012: BSN=256 → register spill. v013: BSN=128,NI=4 → only 14 blocks, CU starvation.
- v014: BSN=128, NI=1, NW=4. For M=16,K=7168: grid=(1,56), 224 waves (same as v006).
  Bigger blocks (4096 vs 1024 elements), more warps (4 vs 1), same wave count.
- Target: quant phase for 16x2112x7168.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, device):
    ...
    if key not in _buffers:
        ...
        # Block size config — same as v006 except M<=32 large-K fix
        if M <= 32:
            NUM_ITER = 1
        ...
        if K <= 1024:
            ...
            # FIX: M<=32 with large K was using BSN=32, 1 warp (224 blocks for K=7168)
            # BSN=128, NI=1, NW=4: grid=(1,56) for K=7168 → 224 waves (same as v006)
            # but each block does 4x more work with 4x more threads
            NUM_ITER = 1
            BLOCK_SIZE_M = 32
            BLOCK_SIZE_N = 128
            NUM_WARPS = 4
