...
"""
v220: .wt on M=256 scale stores + server version probe.

Based on v188. Single change: .wt cache modifier on M=256 quant scale stores
(from .cg). The .wt on fp4 stores is already in v188. Adding .wt on scale
stores was tested in v196 (M=256 BM -1.6%, LB masked by noise). Resubmitting
to check for server software updates that could change the baseline.
"""
import torch
import triton
...
import sys
print(f"Server: torch={torch.__version__}", file=sys.stderr)
