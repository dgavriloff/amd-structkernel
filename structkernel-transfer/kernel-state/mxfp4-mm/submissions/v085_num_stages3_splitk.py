...
"""
v085: num_stages=3 for split-K=7 path (16x2112x7168).

For 16x2112x7168 split-K=7 BSK=256: each block does 4 K-iters.
num_stages=3 triple-buffers for deeper compute/memory overlap.
v065 tried NW=8 NS=3 which had register pressure; NW=4 NS=3 is untried.
Also: no cache_modifier (.cg) for split-K path to reduce L1 pressure.
"""
import torch
import triton
