...
"""
v142: Library quant + e8m0_shuffle for M=256 two-phase path.

Hypothesis: The library's dynamic_mxfp4_quant kernel may have better Triton codegen
than our custom fused quant+shuffle kernel. Use library quant (which stores scales in
natural order) followed by e8m0_shuffle (a fast permute+contiguous). This trades one
fused kernel for two operations but may benefit from the library's optimized codegen.
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # Library quant + shuffle for M > _FUSED_M_THRESHOLD
        x_fp4, bs_e8m0 = dynamic_mxfp4_quant(A)
        bs_shuffled = e8m0_shuffle(bs_e8m0)

        gemm_a4w4_asm(
            x_fp4.view(dtypes.fp4x2),
            B_shuffle,
            bs_shuffled.view(dtypes.fp8_e8m0),
            B_scale_sh,
            buf['gemm_out'],
            _ASM_KERNEL_32x128,
            None,
