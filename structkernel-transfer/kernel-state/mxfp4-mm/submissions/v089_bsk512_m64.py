...
"""
v089: BSK=512 for M=64 (64x7168x2048).

v077 uses BSK=256 → 8 K-iters with NS=2 pipelining.
BSK=512 → 4 K-iters with NS=2. Larger per-iteration tiles improve
compute density. 4 K-iters is still plenty for NS=2 pipelining.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=16 BSN=128 BSK=512 NW=4 NS=2
        # 4*56=224 blocks, 4 K-iters with pipelining
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
