...
"""
v105: BSN=64 NW=2 for M<=4 shapes (doubled grid: 45 blocks vs 23 for 4x2880x512).

Hypothesis: M=4 shapes currently use BSN=128 NW=4, creating only 23 blocks for 4x2880x512.
With BSN=64 NW=2, we get 45 blocks (nearly 2x). Each block does half the N-work with
half the warps, so per-block time is similar, but CU utilization doubles from 9% to 18%.
