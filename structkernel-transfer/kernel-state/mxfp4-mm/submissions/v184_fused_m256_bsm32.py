...
"""
v184: Fused preshuffle path for M=256 with BSM=32 BSN=128 BSK=256.

Hypothesis: Replace M=256 two-phase path (Triton quant + ASM GEMM) with
single fused preshuffle kernel (PREQUANT=True). Previous fused attempt (v158)
used BSM=8 which is 4x below MFMA nonkdim=16, causing massive ALU waste.
BSM=32 gives proper MFMA utilization with 192 blocks (0.75 waves).

Key difference from v158: BSM=32 (not 8) means each block does a 32x128
output tile — 4x more work per block with full MFMA utilization. This
eliminates the separate quant kernel launch (~6.4us) and ASM GEMM launch
(~5.8us) in favor of a single Triton kernel that does both.

Target: M=256 shape (256x3072x1536). All other shapes unchanged from v165.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=256 (256x3072x1536): BSM=32 BSN=128 BSK=256 NW=4 NS=2
        # 8*24=192 blocks (0.75 waves), 3 K-iters with pipelining
        # Fused PREQUANT path eliminates separate quant+GEMM launches
        return {
            "BLOCK_SIZE_M": 32,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
