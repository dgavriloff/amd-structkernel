...
"""
v043: Shape-specific ASM kernel tile sizes for two-phase path.
- M<=32: fused quant+GEMM via gemm_a16wfp4_preshuffle.
- M>=64: two-phase (Triton quant + ASM GEMM with shape-specific tiles).
  - M=64: 64x128 tile (matches M exactly, 56 tiles, 5.4 CUs/tile)
  - M=256: 32x128 tile (192 tiles, good occupancy)
- Fused configs: M<=8 gets BSM=8 BSN=128 NW=4.
- Hypothesis: 64x128 tile for M=64 improves data locality vs 2x 32-row tiles.
"""
import torch
import triton
...
# ASM kernel names — shape-specific tile sizes
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            # Select ASM kernel tile size based on M
            if M == 64:
                asm_kernel = _ASM_KERNEL_64x128
                asm_tile_m = 64
            else:
                asm_kernel = _ASM_KERNEL_32x128
                asm_tile_m = 32

            padded_M = ((M + asm_tile_m - 1) // asm_tile_m) * asm_tile_m
