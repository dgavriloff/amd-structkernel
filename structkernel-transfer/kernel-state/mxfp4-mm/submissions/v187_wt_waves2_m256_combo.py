...
"""
v187: Combination sweep — .wt fp4 stores + waves_per_eu=2 for M=256 quant.

Combines two independently neutral changes that target different aspects of
the M=256 two-phase path:
1. .wt (write-through) cache modifier on fp4 stores in quant kernel — pushes
   fp4 data to L2 simultaneously with L1 write, making it available to ASM
   GEMM faster. v176/v177/v178/v182 all showed consistent -1.5 to -3% on
   M=256 but LB noise on M=32 shapes prevented geomean improvement.
2. waves_per_eu=2 for M=256 quant kernel — forces higher occupancy. The quant
   kernel uses BSM=32 BSN=64 NW=2 with only 8*24=192 blocks (0.75 waves).
   waves_per_eu=2 encourages the compiler to use fewer registers, potentially
   enabling more concurrent wavefronts and hiding memory latency.

Target: M=256 shape only. All other shapes unchanged from v165.
"""
import torch
import triton
