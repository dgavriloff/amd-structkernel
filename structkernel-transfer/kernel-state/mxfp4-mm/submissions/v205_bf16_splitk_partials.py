...
"""
v205: BF16 split-K partials for 16x2112x7168.

Hypothesis: Use bf16 instead of fp32 for split-K partial sums. This halves
memory bandwidth for writing (7 splits * 16 * 2112 * 2B vs 4B = 59KB vs 118KB)
and reading in the reduce kernel. Uses REDUCE_BSN=128 (from 64) since bf16
partials allow wider reduce blocks. Correctness risk: bf16 truncation of each
partial sum before reduction, but rtol=1e-2 tolerance may accommodate this.
Based on aiter's _USE_GEMM_SPLITK_BF16 flag pattern (never tested on this kernel).
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    # Pre-allocate y_pp — use bf16 partials to halve memory bandwidth
    y_pp = torch.empty((NUM_KSPLIT, M, N), dtype=torch.bfloat16, device=device)

    # Reduce kernel params — bf16 partials allow wider BSN (128 vs 64)
    REDUCE_BSM = 16
    REDUCE_BSN = 128  # Wider blocks for bf16 partials (vs 64 for fp32)
