...
"""
v139: Direct dispatch for M=64 fused path — pre-compute grid, strides, and all
parameters at init time to eliminate wrapper overhead for the largest fused kernel.
Also pre-compute B_w/B_sc strides to avoid .stride() calls on reshaped views.

Based on v099 (9.16us LB). Target: 64x7168x2048 (13.6us LB).
"""
import torch
import triton
...
def _prepare_direct_dispatch(M, N, K, config, device):
    """Pre-compute all params for non-split-K direct dispatch."""
    K_kernel = K // 2
    BSM = config["BLOCK_SIZE_M"]
    BSN = max(config["BLOCK_SIZE_N"], 32)
    BSK = config["BLOCK_SIZE_K"]

    # For non-split-K, SPLITK_BLOCK_SIZE = 2*K_kernel (full K range)
    ...
    # Pre-compute B_w strides for reshaped view: (N//16, (K//2)*16)
    B_w_stride0 = (K_kernel) * 16  # stride for dim 0
    B_w_stride1 = 1                # stride for dim 1 (contiguous)
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            if config["NUM_KSPLIT"] > 1:
                ...
                # M=64 direct dispatch: pre-compute all params for minimum overhead
                ...
                # Non-split-K small M: use wrapper
                _buffers[key] = {
                    'mode': 'fused',
                    'out': torch.empty((M, N), dtype=torch.bfloat16, device=device),
                    'B_w': None,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # Direct dispatch for M>32 non-split-K (e.g., M=64)
        b_ptr = B_shuffle.data_ptr()
        if buf['B_w'] is None or buf.get('_b_ptr') != b_ptr:
            buf['B_w'] = B_shuffle.view(torch.uint8).reshape(N // 16, (K // 2) * 16)
            bs_shape = B_scale_sh.shape
        ...
        _gemm_a16wfp4_preshuffle_kernel[(dp['grid_size'],)](
            A,
            buf['B_w'],
            out,
            buf['B_sc'],
