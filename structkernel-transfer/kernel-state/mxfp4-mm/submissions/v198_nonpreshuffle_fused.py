...
"""
v198: Non-preshuffle kernel for M<=64 non-split-K shapes.

Hypothesis: Replace _gemm_a16wfp4_preshuffle_kernel with _gemm_a16wfp4_kernel
(non-preshuffle) for non-split-K M<=64 shapes. The non-preshuffle kernel:
1. Eliminates the inner-loop B reshape+permute (6 ops per K iteration)
2. Eliminates the inner-loop B_scales reshape+permute (7 ops per K iteration)
3. Uses simpler stride-based B access instead of tile-coalesced format
4. Is a LIBRARY kernel (not custom), so may have a cached binary on the server

v127 tested this WITH XCD remap (which is known +3% worse from v192). This
test isolates the non-preshuffle format WITHOUT XCD remap.

B_q is (N, K/2), needs unshuffle of B_scale_sh to get (N, K/32) raw scales.
"""
import torch
import triton
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            if config["NUM_KSPLIT"] > 1:
                ...
                # Non-split-K: use non-preshuffle kernel (simpler B access, no inner-loop reshape)
                K_packed = K // 2  # packed K for kernel
                BSK = config["BLOCK_SIZE_K"]
                BSN = max(config["BLOCK_SIZE_N"], 32)
                BSM = config["BLOCK_SIZE_M"]
                SPLITK_BLOCK_SIZE = 2 * K_packed  # No split-K

                # Adjust BSK if needed (match wrapper logic)
                if BSK >= 2 * K_packed:
                    BSK = triton.next_power_of_2(2 * K_packed)
                ...
                _buffers[key] = {
                    'mode': 'fused_nonpreshuffle',
                    'out': torch.empty((M, N), dtype=torch.bfloat16, device=device),
                    'B_q_raw': None,  # B_q transposed view
                    'B_scale_raw': None,  # Un-shuffled B scales
                    'grid_size': grid_size,
                    'K_packed': K_packed,
                    'BLOCK_SIZE_M': BSM,
                    'BLOCK_SIZE_N': BSN,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    elif buf['mode'] == 'fused_nonpreshuffle':
        # Non-preshuffle path: use _gemm_a16wfp4_kernel with raw B_q format
        b_ptr = B_q.data_ptr()
        if buf['B_q_raw'] is None or buf.get('_b_ptr') != b_ptr:
            # B_q is (N, K/2) packed FP4 — transpose to (K/2, N) for kernel
            buf['B_q_raw'] = B_q.view(torch.uint8).T  # (K/2, N), non-contiguous
            # Un-shuffle B scales from B_scale_sh to get raw (N, K/32)
            bs_uint8 = B_scale_sh.view(torch.uint8)
            raw_scales = _unshuffle_e8m0(bs_uint8)
            # Trim to actual N x K/32 (remove padding)
            k_scale = K // 32
            buf['B_scale_raw'] = raw_scales[:N, :k_scale].contiguous()
            buf['_b_ptr'] = b_ptr

        out = buf['out']

        _gemm_a16wfp4_kernel[(buf['grid_size'],)](
            A,
            buf['B_q_raw'],
            out,
            buf['B_scale_raw'],
            ...
            A.stride(0),
            A.stride(1),
            buf['B_q_raw'].stride(0),
            buf['B_q_raw'].stride(1),
            0,  # stride_ck (no split-K)
            out.stride(0),
            out.stride(1),
            buf['B_scale_raw'].stride(0),
            buf['B_scale_raw'].stride(1),
