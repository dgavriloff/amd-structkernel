...
"""
v106: BSK=512 NS=1 for M<=4 shapes (eliminate K-loop overhead).

Hypothesis: For 4x2880x512, K_kernel=256. With BSK=256, we get 2 K-iterations
and 2-stage pipelining. But pipelining has overhead (extra loads, barriers) that
may exceed the benefit for such small tiles (4x128 = 512 elements per block).
BSK=512 covers the entire K in 1 iteration, eliminating loop overhead entirely.

NW=4 NS=1 since with 1 K-iter, num_stages>1 is pointless.
