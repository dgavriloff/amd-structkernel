...
"""
v107: waves_per_eu=2 for M<=4 shapes to increase occupancy via wave switching.

Hypothesis: M=4 shapes (4x2880x512) have only 23 blocks for 256 CUs (9% util).
With waves_per_eu=0 (auto), the compiler may choose low occupancy for maximum
register usage. Setting waves_per_eu=2 forces 2 waves per EU, which increases
memory latency hiding through wave switching at the cost of register pressure.

v073 tested waves_per_eu=2 for M<=8 but those shapes actually hit the K>4096
split-K config, not the M<=8 config. This is the first test of waves_per_eu=2
specifically for the config that 4x2880x512 actually uses.

Target: 4x2880x512 (6.34us LB).
"""
import torch
import triton
