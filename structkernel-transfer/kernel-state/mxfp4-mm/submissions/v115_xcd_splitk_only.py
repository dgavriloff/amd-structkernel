...
"""
v115: XCD remapping only for split-K path (16x2112x7168).

Hypothesis: v114 showed XCD remap helps 16x2112x7168 (-12.7% BM) but hurts
small-grid shapes (M=32 K=512, M=64). The split-K path has 238 blocks where
adjacent PIDs share B-data locality across XCDs. For non-split-K shapes with
fewer blocks, the remap computation overhead and disrupted GROUP_SIZE_M
locality pattern hurt more than they help.

Apply XCD remap only to fused_splitk path, keep library wrapper for
non-split-K fused shapes.
"""
import torch
import triton
...
def _preshuffle_xcd_kernel(
    ...
    # XCD remapping for MI355X L2 locality
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            if config["NUM_KSPLIT"] > 1:
                # Split-K path: use XCD-remapped direct dispatch with tuned reduce kernel
                ...
                # Non-split-K: use library wrapper (no XCD remap benefit for small grids)
                _buffers[key] = {
                    'mode': 'fused',
                    'out': torch.empty((M, N), dtype=torch.bfloat16, device=device),
                    'B_w': None,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # Non-split-K fused path: use library wrapper (no XCD remap benefit)
        b_ptr = B_shuffle.data_ptr()
        if buf['B_w'] is None or buf.get('_b_ptr') != b_ptr:
            buf['B_w'] = B_shuffle.view(torch.uint8).reshape(N // 16, (K // 2) * 16)
            bs_shape = B_scale_sh.shape
