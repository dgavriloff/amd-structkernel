...
"""
v022: Try BSN=128 NW=4 for quant kernel — quadruple work per block with 4 warps.
- v021 uses BSN=64 NW=2: geomean 12.8µs.
- BSN=128 NW=4: NUM_QUANT_BLOCKS=4, grid quartered vs BSN=32, 4 warps for latency hiding.
- 64x7168x2048: grid=(2,16)=32 blocks (vs 64 with BSN=64).
- 4x2880x512: grid=(1,4)=4 blocks — low but may be fine with 4 warps per block.
- Target: quant phase — test if 4 warps outweighs reduced parallelism.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # BSN=128 NW=4: quadruple work per block with 4 warps for latency hiding.
        NUM_ITER = 1
