...
"""
v202: Gluon reduce REDUCE_BSN=32 + .wt on M=256 scale stores.

Based on v188. Two changes targeting different paths:
1. REDUCE_BSN=32 for gluon reduce kernel (from 64): 66 reduce blocks vs 33,
   doubling CU utilization for the 16x2112x7168 split-K reduce. BSN=128 was
   tested (v189, +4.5% worse), BSN=32 never tested with gluon reduce.
2. .wt cache modifier on M=256 quant scale stores (from .cg) — v196 showed
   M=256 BM -1.6% (12.3 vs 12.5), consistent with .wt fp4 stores pattern.
   Combined with existing .wt fp4 stores for compound M=256 improvement.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    # Reduce kernel params — gluon version, BSN=32 for more reduce blocks
    REDUCE_BSM = 16
    REDUCE_BSN = 32  # 66 blocks vs 33 with BSN=64, better CU utilization
