...
"""
v031: Shape-specific GEMM kernel selection.
- M<=32: 32x128 (proven optimal, avoids large-tile waste)
- M=64: 64x128 (exact M coverage in 1 tile, better register reuse;
  56 tiles vs 112 with 32x128, each tile covers full M=64)
- M=256: 32x128 (keeps 192 tiles for good CU utilization)
Target: GEMM phase for M=64 (currently 9.6µs = 54% of total)
"""
import torch
import triton
...
# ASM kernel names for shape-specific selection
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # Shape-specific GEMM kernel selection
        if M == 64:
            asm_kernel = _ASM_KERNEL_64x128  # exact M coverage, 1 M-tile
            gemm_tile_m = 64
        else:
            asm_kernel = _ASM_KERNEL_32x128  # proven optimal for M<=32, M=256
            gemm_tile_m = 32

        # GEMM output must be padded to multiple of tile_M
        padded_M = (M + gemm_tile_m - 1) // gemm_tile_m * gemm_tile_m
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Shape-specific ASM kernel: 64x128 for M=64, 32x128 for all others
    gemm_a4w4_asm(
