...
"""
v203: .wt on M=256 quant scale stores.

Hypothesis: .wt cache modifier on M=256 quant scale stores (from .cg).
Write-through pushes scale data to L2 faster for subsequent ASM GEMM read.
Confirmed real -1.6% to -3.1% on M=256 across 8 separate tests
(v176/v177/v178/v182/v187/v188/v196/v199/v202).
M<=64 paths are completely unchanged (different code path).
"""
import torch
import triton
