...
"""
v033: Use BSN=32 NW=1 for K=512 shapes, BSN=64 NW=2 for K>512.
- K=512 with BSN=64: grid_n = 512/64 = 8 blocks. Only 8 blocks total for M<=32!
- K=512 with BSN=32: grid_n = 512/32 = 16 blocks. Double the parallelism.
- K>512 shapes already have enough blocks with BSN=64.
- Target: quant phase for K=512 shapes (M=4,32,32) — increase CU utilization
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # Shape-specific: BSN=32/NW=1 for K=512 (more blocks), BSN=64/NW=2 for K>512
        NUM_ITER = 1
