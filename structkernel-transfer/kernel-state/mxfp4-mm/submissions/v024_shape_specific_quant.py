...
"""
v024: Shape-specific quant config — BSN=128 NW=4 for large K, BSN=64 NW=2 for small K.
- v021 uses BSN=64 NW=2 for all shapes. v022 showed BSN=128 NW=4 hurts K=512 (too few blocks).
- Solution: BSN=128 NW=4 only when K > 1024 (many blocks), BSN=64 NW=2 for K<=1024.
- 16x2112x7168: BSN=128 grid=(1,56)=56 blocks with 4 warps → better latency hiding.
- 64x7168x2048: BSN=128 grid=(2,16)=32 blocks with 4 warps.
- K=512 shapes: BSN=64 grid=(1,8)=8 blocks with 2 warps (unchanged).
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # Shape-specific: BSN=128 NW=4 for large K (many blocks), BSN=64 NW=2 for small K.
        NUM_ITER = 1
