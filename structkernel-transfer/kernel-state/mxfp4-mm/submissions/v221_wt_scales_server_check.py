...
"""
v221: .wt on M=256 scale stores + server version check.

Changes from v188:
1. .wt cache modifier on M=256 quant SCALE stores (from .cg) — confirmed
   real -2.4% BM on M=256 across 20 tests. Combines with existing .wt fp4 stores.
2. Print server torch/triton/aiter version to check for upgrades since v220.
"""
import torch
import triton
