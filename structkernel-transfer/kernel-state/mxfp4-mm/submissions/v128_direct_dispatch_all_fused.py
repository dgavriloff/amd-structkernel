...
"""
v128: Direct dispatch for ALL fused shapes (not just split-K).

Hypothesis: The non-split-K fused path currently uses gemm_a16wfp4_preshuffle()
wrapper which adds overhead: serialize_dict() + torch.ops.aiter dispatch +
deserialize_str() + grid lambda creation + redundant shape calculations.
By using direct _gemm_a16wfp4_preshuffle_kernel dispatch (same as split-K path),
we eliminate all Python wrapper overhead.

v094/v104 tested this individually and found <1% improvement. But those tests
used a different code structure. This version unifies ALL fused shapes under
the same direct dispatch pattern, which may interact better with CPU prediction
and reduce branch overhead.

Target: All fused shapes. M=64 (13.6us) and M=32 K=512 shapes most likely to benefit.
"""
import torch
import triton
...
# gemm_a16wfp4_preshuffle wrapper no longer needed — using direct kernel dispatch
