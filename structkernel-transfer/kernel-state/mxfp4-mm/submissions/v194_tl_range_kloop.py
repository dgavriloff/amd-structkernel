...
"""
v194: Custom preshuffle kernel with tl.range K-loop pipelining.

Hypothesis: The library preshuffle kernel uses Python `range()` for the K-loop,
and relies on the compiler's global `num_stages=2` for pipelining. Replacing with
`tl.range(..., num_stages=2)` provides explicit loop-level software pipelining
hints to the Triton compiler. This has never been tried — previous custom kernels
(v185/v186) changed fast_math and acc patterns but kept Python `range()`.
The tl.range approach is used in other aiter kernels (moe_op_e2e, normalization,
rope) for better pipeline scheduling.
"""
import torch
import triton
...
# Custom preshuffle kernel with tl.range for K-loop pipelining
@triton.heuristics(
    {
...
def _custom_preshuffle_kernel(
    ...
    """Custom preshuffle kernel with tl.range K-loop for explicit pipelining."""
    ...
    if (pid_k * SPLITK_BLOCK_SIZE // 2) < K:
        ...
        # KEY CHANGE: Use tl.range with num_stages=2 for explicit loop pipelining
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        _custom_preshuffle_kernel[(kp['grid_size'],)](
            A,
            buf['B_w'],
            y_pp,
            buf['B_sc'],
        ...
        _custom_preshuffle_kernel[(buf['grid_size'],)](
            A,
            buf['B_w'],
            out,
            buf['B_sc'],
