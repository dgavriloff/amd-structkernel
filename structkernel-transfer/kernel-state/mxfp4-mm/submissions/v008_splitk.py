...
"""
v006: Pre-allocate quant tensor buffers to reduce allocation overhead.
- Pre-allocate x_fp4 and blockscale buffers per shape, reuse across calls.
- Hoist block config computation to module level (only depends on shape).
- Target: quant phase overhead (~1-2µs from 3 tensor allocations per call).
"""
import torch
import triton
