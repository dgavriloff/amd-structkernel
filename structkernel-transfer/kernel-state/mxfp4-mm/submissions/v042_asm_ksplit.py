...
"""
v042: Add log2_k_split to ASM GEMM in two-phase path.
- M<=32: fused quant+GEMM via gemm_a16wfp4_preshuffle.
- M>=64: two-phase (Triton quant + ASM 32x128 GEMM with k-split).
- Shape-specific fused configs: M<=8 gets BSM=8 BSN=128 NW=4.
- New: ASM GEMM gets log2_k_split computed per shape to use idle CUs.
  - 32x128 tile: M=64 -> 112 tiles -> cusPerTile=2.7 -> splitK=1
  - 32x128 tile: M=256 -> 448 tiles -> cusPerTile=0.68 -> splitK=0
- Hypothesis: k-split on M=64 (64x7168x2048) uses idle CUs for ~10% improvement.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 8:
        # Tuned config for very small M: smaller BSM, larger BSN
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
        ...
        # Use None to let gemm_a16wfp4_preshuffle pick its own config
        return None
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            # Compute log2_k_split: use idle CUs for k-dimension parallelism
            # ASM tile is 32x128, MI355X has 304 CUs
            tile_m, tile_n, tile_k = 32, 128, 128
            tile_num = ((M + tile_m - 1) // tile_m) * ((N + tile_n - 1) // tile_n)
            cus_per_tile = 304 / tile_num
            log2_ksplit = 0
            while cus_per_tile >= pow(2, log2_ksplit + 1) and (pow(2, log2_ksplit + 1) * tile_k) < 2 * K:
                log2_ksplit += 1
            log2_ksplit = min(log2_ksplit, 3)  # cap at 3 like aiter does
