...
"""
v023: BSN=64 NW=2 NI=2 NUM_STAGES=2 — software pipelining within each block.
- v021 uses BSN=64 NW=2 NI=1 NS=1: no pipelining.
- NI=2 NS=2: each block processes 2 tiles of 64 elements, overlapping loads with compute.
- Grid halves vs v021 but pipelining should compensate.
- 16x2112x7168: grid=(1,56)=56 blocks (vs 112). Still 22% CU util.
- 64x7168x2048: grid=(2,16)=32 blocks (vs 64). 12.5% CU util.
- Target: quant phase — test if pipelining overhead wins over CU reduction.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # BSN=64 NW=2 NI=2 NS=2: software pipelining within each block.
        NUM_ITER = 2
