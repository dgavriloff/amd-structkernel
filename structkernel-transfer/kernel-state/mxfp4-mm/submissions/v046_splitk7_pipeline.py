...
"""
v046: Custom split-K=7 + num_stages=2 for 16x2112x7168 shape.
Based on v044 with one change: for M=16 N=2112 K=7168 specifically,
use NUM_KSPLIT=7 (vs library's 14) with num_stages=2 for pipelining.
Split-K=7 means 2 K-loop iterations per block (vs 1 with split-K=14),
enabling Triton software pipelining. Grid drops from 238 to 119 blocks
(still 46% CU occupancy) but each block does 2x the work with pipelining.
Reduce kernel has half the partials (7 vs 14).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        # Perfect tile for M=4: BSM=4 eliminates wasted rows (vs BSM=8 which wastes 50%)
        return {
            "BLOCK_SIZE_M": 4,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
        ...
        # Tuned config for small M: BSM=8 BSN=128
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
        ...
        # Custom config for 16x2112x7168: split-K=7 with num_stages=2 for pipelining.
        # Library uses split-K=14 (1 K-iter/block), we use 7 (2 K-iters, pipelined).
        # Grid: 7*1*17=119 blocks (vs 14*1*17=238). Reduce: 7 partials (vs 14).
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
        ...
        # Fallback for other M>8 K>1024 shapes: let library pick
        return None
