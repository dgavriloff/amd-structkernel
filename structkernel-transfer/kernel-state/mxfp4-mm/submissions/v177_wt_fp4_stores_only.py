...
"""
v177: .wt (write-through) cache modifier on fp4 stores in M=256 quant kernel only.

Hypothesis: v176 showed consistent -1.5% on M=256 LB (both runs) with .wt on fp4
AND scale stores. M=32 shapes regressed +2.1-2.6% but those use a completely
different code path (fused preshuffle), so the regression was noise.
This version applies .wt ONLY to fp4 stores, keeping .cg on scale stores
(which is already optimal). Write-through pushes fp4 data to L2 simultaneously
with L1 write, making it available to the ASM GEMM faster.

Target: M=256 two-phase path (quant kernel fp4 stores).
"""
import torch
import triton
