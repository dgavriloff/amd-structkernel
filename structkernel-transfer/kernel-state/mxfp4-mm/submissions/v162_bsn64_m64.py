...
"""
v162: BSN=64 NW=2 for M=64 fused path — double CU utilization via smaller N-tile.

Hypothesis: M=64 (64x7168x2048) currently uses BSN=128 NW=4 with 224 blocks (0.88 waves).
With BSN=64, we get 112 N-tiles * 4 M-tiles = 448 blocks (1.75 waves).
NW=2 matches the BSN=64 config used by M<=32 K<=1024 shapes which proved optimal.
BSN=64 was the winning config for M<=8 shapes. More blocks better saturate 256 CUs.

Target: 64x7168x2048 (~8.8us BM, ~13.0us LB).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=16 BSN=64 BSK=256 NW=2 NS=2
        # 4*112=448 blocks (1.75 waves) vs 224 blocks (0.88 waves)
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 64,
            "BLOCK_SIZE_K": 256,
