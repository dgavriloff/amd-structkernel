...
"""
v131: matrix_instr_nonkdim=32 for M=64 fused path.

Hypothesis: AMD's tuned AFP4WFP4 configs use matrix_instr_nonkdim=32 for M=64.
The 32x32 MFMA instruction may improve compute throughput vs 16x16.
BSM=32 to ensure M tile >= MFMA nonkdim. 2*56=112 blocks.
Each block handles double the M work, fewer blocks but better MFMA utilization.
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=32 BSN=128 BSK=256 NW=4 NS=2
        # 2*56=112 blocks, 4 K-iters with pipelining, 32x32 MFMA
        return {
            "BLOCK_SIZE_M": 32,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
