...
"""
v030: Minimize Python dispatch overhead with pre-cached strides and direct calls.
- Cache fp4 tensor strides alongside buffers to avoid .stride() calls
- Use cached fp4x2 and fp8_e8m0 views to avoid .view() overhead per call
- Pre-compute all constant arguments to minimize Python work per call
- Target: Python dispatch overhead — reduce per-call Python ops.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # Pre-compute views to avoid .view() overhead per call
        x_fp4_view = x_fp4.view(dtypes.fp4x2)
        blockscale_view = blockscale.view(dtypes.fp8_e8m0)

        # Pre-cache strides
        fp4_stride_m, fp4_stride_n = x_fp4.stride()
        ...
        _buffers[key] = {
            ...
            'NUM_WARPS': NUM_WARPS,
            'grid': grid,
            'fp4_stride_m': fp4_stride_m,
            'fp4_stride_n': fp4_stride_n,
            'M': M,
        }
    return _buffers[key]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    _fused_mxfp4_quant_shuffle_kernel[buf['grid']](
        ...
        buf['fp4_stride_m'], buf['fp4_stride_n'],
        M=M,
        N=K,
