...
"""
v087: BSM=8 for M=64 (64x7168x2048).

v077 uses BSM=16 for M=64 → grid=(4,56)=224 blocks, 0.875 waves.
BSM=8 → grid=(8,56)=448 blocks, 1.75 waves. Better CU saturation.
More blocks = better memory latency hiding via warp scheduling.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=8 BSN=128 BSK=256 NW=4 NS=2
        # 8*56=448 blocks, 8 K-iters with pipelining, 1.75 waves
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
