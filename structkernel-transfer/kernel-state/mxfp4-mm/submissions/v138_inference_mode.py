...
"""
v099: BSM=8 for M<=32 K<=1024 shapes to double CU utilization.

Hypothesis: M=32 K=512 shapes use BSM=16, giving 2 M-tiles.
With BSM=8, we get 4 M-tiles — doubling the grid from 46-64 to 92-128 blocks.
More blocks better saturate 256 CUs for these small workloads.
Similar to how BSM=8 helped the split-K path in v096.

Target: 32x4096x512 (7.04us LB), 32x2880x512 (7.23us LB).
v138: torch.inference_mode() on custom_kernel to eliminate autograd dispatch overhead.

Hypothesis: Every PyTorch tensor operation (view, reshape, stride, shape access)
goes through the autograd dispatch table even when no gradients are needed.
torch.inference_mode() disables this dispatch, saving ~100-200ns per tensor op.
With 5-10 tensor ops per call at 6-13us kernel times, this could save 1-2%.
"""
import torch
import triton
