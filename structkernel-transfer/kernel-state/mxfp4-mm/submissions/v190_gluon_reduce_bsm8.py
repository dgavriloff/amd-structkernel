...
"""
v190: Gluon reduce BSM=8 + .wt on M=256 quant stores.

Based on v188 (9.01µs LB). Change: REDUCE_BSM=8 (from 16) for gluon reduce.
With M=16 and BSM=8, reduce_grid=(2,33) = 66 blocks vs (1,33) = 33 blocks.
More blocks = better CU utilization for the reduce phase.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    # Reduce kernel params — gluon version with BSM=8 for more blocks
    REDUCE_BSM = 8
