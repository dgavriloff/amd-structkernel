...
"""
v114: Add XCD remapping to preshuffle GEMM kernel for MI355X L2 locality.

Hypothesis: The _gemm_a16wfp4_preshuffle_kernel is missing remap_xcd() which
all other aiter GEMM kernels use. On MI355X with 8 XCDs (each with own L2),
without remapping adjacent PIDs go to different XCDs, causing poor B-data
reuse in L2. Adding remap_xcd should improve L2 hit rate for all fused shapes.

Target: All fused shapes (M<=64), especially 16x2112x7168 (238 blocks)
and 64x7168x2048 (224 blocks) which have enough blocks to benefit from
better XCD distribution.
"""
import torch
import triton
...
def _preshuffle_xcd_kernel(
    ...
    """Preshuffle GEMM kernel with XCD remapping for MI355X L2 locality."""
    ...
    # XCD remapping — the key difference from library preshuffle kernel
...
def _prepare_direct_dispatch(M, N, K, config, device):
    """Pre-compute all params for direct dispatch of non-split-K fused shapes."""
    K_kernel = K // 2
    BSK = config["BLOCK_SIZE_K"]
    ...
    # NUM_KSPLIT=1, SPLITK_BLOCK_SIZE = full K
    SPLITK_BLOCK_SIZE = triton.cdiv(2 * K_kernel, BSK) * BSK
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            if config["NUM_KSPLIT"] > 1:
                ...
                # Non-split-K: use direct dispatch with XCD remapping
                direct_params = _prepare_direct_dispatch(M, N, K, config, device)
                _buffers[key] = {
                    'mode': 'fused_direct',
                    'out': torch.empty((M, N), dtype=torch.bfloat16, device=device),
                    'B_w': None,
...
def _reshape_b(B_shuffle, B_scale_sh, N, K):
    """Reshape B and scales for preshuffle kernel format."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        # Split-K path with XCD-remapped kernel and tuned reduce (REDUCE_BSN=16)
        b_ptr = B_shuffle.data_ptr()
        if buf['B_w'] is None or buf.get('_b_ptr') != b_ptr:
            buf['B_w'], buf['B_sc'] = _reshape_b(B_shuffle, B_scale_sh, N, K)
            buf['_b_ptr'] = b_ptr
        ...
        _preshuffle_xcd_kernel[(kp['grid_size'],)](
            A,
            buf['B_w'],
            y_pp,
            buf['B_sc'],
        ...
        # Non-split-K fused path with XCD-remapped kernel
        b_ptr = B_shuffle.data_ptr()
        if buf['B_w'] is None or buf.get('_b_ptr') != b_ptr:
            buf['B_w'], buf['B_sc'] = _reshape_b(B_shuffle, B_scale_sh, N, K)
            buf['_b_ptr'] = b_ptr

        dp = buf['direct_params']
        out = buf['out']

        _preshuffle_xcd_kernel[(dp['grid_size'],)](
            A,
            buf['B_w'],
            out,
            buf['B_sc'],
            ...
            buf['B_w'].stride(0),
            buf['B_w'].stride(1),
            0,  # stride_ck = 0 for non-split-K (single slice)
            out.stride(0),
            out.stride(1),
            buf['B_sc'].stride(0),
            buf['B_sc'].stride(1),
            BLOCK_SIZE_M=dp['BLOCK_SIZE_M'],
            BLOCK_SIZE_N=dp['BLOCK_SIZE_N'],
            BLOCK_SIZE_K=dp['BLOCK_SIZE_K'],
            GROUP_SIZE_M=dp['GROUP_SIZE_M'],
            NUM_KSPLIT=dp['NUM_KSPLIT'],
            SPLITK_BLOCK_SIZE=dp['SPLITK_BLOCK_SIZE'],
            num_warps=dp['num_warps'],
            num_stages=dp['num_stages'],
            waves_per_eu=dp['waves_per_eu'],
            matrix_instr_nonkdim=dp['matrix_instr_nonkdim'],
            PREQUANT=True,
            cache_modifier=dp['cache_modifier'],
        )
