...
"""
v214: HSA_ENABLE_SDMA=0 to disable System DMA engine + .wt M=256 scale stores.

Hypothesis: HSA_ENABLE_SDMA=0 disables the SDMA (System DMA) engine, forcing all
memory copy operations to use shader-based DMA instead. This can reduce latency
for small memory operations by avoiding SDMA queue scheduling overhead. Our kernel
performs small-to-medium memory operations (M<=256, K<=7168). The clear_l2_cache_large
between LB iterations allocates/frees 64GB — with SDMA disabled, this may complete
faster, leaving more time budget for the actual kernel.
Combined with .wt on M=256 scale stores (confirmed 15x real -0.8 to -3.1% on M=256).
Never tested: no HSA/HIP environment variable tuning has been attempted in 85+ leaves.
"""
import os
