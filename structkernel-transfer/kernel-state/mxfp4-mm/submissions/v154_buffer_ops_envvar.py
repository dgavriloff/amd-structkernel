...
"""
v154: Enable AMD buffer load/store operations via Triton env var.

Hypothesis: AMDGCN_ENABLE_BUFFER_LOAD_STORE=1 enables buffer descriptor-based
memory operations in the Triton AMDGPU backend. Buffer operations use hardware
address generation units, potentially reducing VALU pressure and improving
memory throughput for both the fused preshuffle kernel and quant kernel.
This env var must be set before Triton compiles any kernel.
"""
import os
os.environ['AMDGCN_ENABLE_BUFFER_LOAD_STORE'] = '1'
import torch
import triton
