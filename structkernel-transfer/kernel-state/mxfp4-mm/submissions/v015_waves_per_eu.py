...
"""
v015: Tune waves_per_eu for quant kernel occupancy.
- v012-v014 reverted: quant config tuning was neutral or worse.
- v015: Set waves_per_eu=2 (from 0/auto) to force lower register usage
  → more concurrent waves per CU → better latency hiding for all shapes.
- Target: quant phase across all shapes.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, device):
    ...
    if key not in _buffers:
        ...
        # Block size config (same as v006)
        if M <= 32:
            NUM_ITER = 1
