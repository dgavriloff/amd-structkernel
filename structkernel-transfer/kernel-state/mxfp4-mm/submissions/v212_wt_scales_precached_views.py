...
"""
v212: .wt on M=256 scale stores + pre-cached dtype views.

Changes from v188:
1. .wt cache modifier on M=256 quant scale stores (from .cg) — confirmed real
   -0.8 to -3.1% improvement on M=256 across 14 prior tests.
2. Pre-cache x_fp4.view(fp4x2), blockscale.view(fp8_e8m0), and gemm_out[:M]
   at buffer allocation time to eliminate per-call view/slice overhead (~50ns).
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            x_fp4_raw = torch.empty((M, K // 2), dtype=torch.uint8, device=device)
            blockscale_raw = torch.empty((SCALE_M, SCALE_N), dtype=torch.uint8, device=device)
            gemm_out_raw = torch.empty((padded_M, N), dtype=torch.bfloat16, device=device)

            _buffers[key] = {
                'mode': 'two_phase',
                'x_fp4': x_fp4_raw,
                'blockscale': blockscale_raw,
                'gemm_out': gemm_out_raw,
                # Pre-cached views to avoid per-call view/slice overhead
                'x_fp4_view': x_fp4_raw.view(dtypes.fp4x2),
                'blockscale_view': blockscale_raw.view(dtypes.fp8_e8m0),
                'gemm_out_slice': gemm_out_raw[:M],
                'SCALE_N': SCALE_N,
