...
"""
v222: Library wrapper for split-K dispatch + .wt on M=256 scale stores.

Hypothesis: Two changes targeting different phases:
1. Use gemm_a16wfp4_preshuffle (library wrapper) with skip_reduce=True for
   16x2112x7168 split-K shape. Library wrapper goes through serialize_dict +
   torch_compile_guard dispatch path which may hit a different/better cached
   binary than direct kernel dispatch. Our custom config passed through.
   Then gluon reduce kernel for the reduction.
2. .wt cache modifier on M=256 quant scale stores (from .cg) — confirmed real
   -0.8% to -3.8% on M=256 across 21 tests.
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    """Pre-compute all params for split-K dispatch (16x2112x7168).
    Uses library wrapper with skip_reduce=True, then gluon reduce."""
    K_kernel = K // 2
    ...
    # Build config dict for library wrapper
    wrapper_config = {
        'BLOCK_SIZE_M': BSM,
        'BLOCK_SIZE_N': BSN,
        'BLOCK_SIZE_K': BSK,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        # Split-K path: library wrapper with skip_reduce + gluon reduce
        ...
        # Use library wrapper with skip_reduce to get fp32 partials
        y_pp = gemm_a16wfp4_preshuffle(
            A, buf['B_w'], buf['B_sc'],
            config=kp['wrapper_config'],
            skip_reduce=True,
        )
