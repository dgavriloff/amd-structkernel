...
"""
v055: Debug fused preshuffle — use shuffle_scales format instead of e8m0_shuffle reshape.

Hypothesis: e8m0_shuffle format (designed for ASM GEMM) may differ from shuffle_scales
format (designed for Triton preshuffle GEMM). Previous versions just reshaped e8m0_shuffle
output, but the kernel expects shuffle_scales format.

Approach: Reverse e8m0_shuffle to get raw (N, K//32) scales, then apply shuffle_scales
to get (N//32, K) format. Use fused path for M<=32, two-phase for M>=64.
Force NUM_KSPLIT=1 for all fused configs to avoid split-K correctness issues.

Target: quant+GEMM — fused path eliminates ~6.4us quant launch overhead for M<=32.
"""
import torch
import triton
...
def _e8m0_unshuffle(scale_sh):
    """Reverse e8m0_shuffle to get raw (padN, padKs) scale tensor.
    e8m0_shuffle: view(sm//32, 2, 16, sn//8, 2, 4).permute(0,3,5,2,4,1).view(sm,sn)
    Inverse: view as permuted shape, apply inverse permute, view back.
    """
    sm, sn = scale_sh.shape
    # After e8m0_shuffle, data is in shape (sm//32, sn//8, 4, 16, 2, 2) logically
...
def _shuffle_scales_for_preshuffle(scales, N, K_scale):
    """Apply shuffle_scales format for Triton preshuffle kernel.
    Input: (N, K_scale) raw scales where N%32==0 and K_scale%8==0.
    Output: (N//32, K_scale*32) shuffled scales.
    """
    sm, sn = N, K_scale
    s = scales[:sm, :sn].contiguous()
...
def _convert_scales_for_preshuffle(B_scale_sh, N, K):
    """Convert B_scale_sh (e8m0_shuffle format) to shuffle_scales format."""
    bs = B_scale_sh.view(torch.uint8)
    padN, padKs = bs.shape
    K_scale = K // 32

    # Step 1: Reverse e8m0_shuffle to get raw padded scales
    raw_scales = _e8m0_unshuffle(bs)

    # Step 2: Apply shuffle_scales on the valid (N, K_scale) region
    return _shuffle_scales_for_preshuffle(raw_scales, N, K_scale)
...
def _get_fused_config(M, N, K):
    ...
    if M <= 8:
        ...
        # For M<=32, K>1024 — use default-like config with forced NUM_KSPLIT=1
        return {
            "BLOCK_SIZE_M": 32,
            "BLOCK_SIZE_N": 64,
            "BLOCK_SIZE_K": 512,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused':
        if buf['B_w'] is None:
            # Reshape B_shuffle for preshuffle kernel: (N, K//2) -> (N//16, K//2*16)
            buf['B_w'] = B_shuffle.view(torch.uint8).reshape(N // 16, (K // 2) * 16)
            # Convert scale format: e8m0_shuffle -> shuffle_scales
            buf['B_sc'] = _convert_scales_for_preshuffle(B_scale_sh, N, K)
