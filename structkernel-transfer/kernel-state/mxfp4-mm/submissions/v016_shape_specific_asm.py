...
"""
v016: Shape-specific ASM kernel selection for GEMM.
- Bypass gemm_a4w4 wrapper to call gemm_a4w4_asm directly with optimal kernel names.
- The default heuristic picks large tiles (e.g. 256x256) for small M, wasting compute.
- Based on tuned CSV analysis, 32x128 is optimal for small M shapes.
- Pre-allocate GEMM output buffer per (M, N) shape.
- Target: GEMM phase — especially 16x2112x7168 (20.8µs, 77% of total).
"""
import torch
import triton
