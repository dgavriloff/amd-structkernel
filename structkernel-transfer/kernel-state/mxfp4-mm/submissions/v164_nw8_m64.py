...
"""
v164: NW=8 for M=64 fused path — more warps for better memory latency hiding.

Hypothesis: M=64 (64x7168x2048) currently uses NW=4 with BSM=16 BSN=128 BSK=256.
With NW=8, each CU has 8 warps (512 threads) to hide memory latency. The preshuffle
kernel loads A (bf16), B (shuffled fp4), and B_scales per K-iteration. More warps
allow overlapping loads with compute. waves_per_eu=0 lets compiler choose optimal occupancy.
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=16 BSN=128 BSK=256 NW=8 NS=2
        # 4*56=224 blocks, 8 warps for better memory latency hiding
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
