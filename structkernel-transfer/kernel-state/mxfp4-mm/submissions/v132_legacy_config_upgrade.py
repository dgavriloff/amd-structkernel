...
"""
v132: Upgrade M<=32 K>1024 legacy config from BSM=32/BSN=64/NW=8/NS=1
to BSM=8/BSN=128/BSK=256/NW=4/NS=2 (matching the winning K<=1024 config).

Hypothesis: The M<=32 K>1024 K<=4096 path uses legacy config from before
the BSM=8 optimization. v100 tested BSM=8 here and saw no change on BM
shapes (none hit this path). But LB secret shapes might include shapes
with M<=32 and 1024<K<=4096 (e.g., 32xNx2048, 16xNx4096).
BSM=8 with BSK=256 NS=2 enables software pipelining and doubles the grid
vs the legacy BSM=32.

Target: LB secret shapes with M<=32 K>1024 K<=4096.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M<=32 K>1024 K<=4096: same winning config as K<=1024
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
