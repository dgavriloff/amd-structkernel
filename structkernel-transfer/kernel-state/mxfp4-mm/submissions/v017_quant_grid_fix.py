...
"""
v017: Remove K<=1024 quant config override for better CU utilization.
- K<=1024 path uses BLOCK_SIZE_N=256, creating only 2 blocks for K=512.
- With 256 CUs, 2 blocks = 0.8% utilization = waste.
- Remove the override so M<=32 path is used: BSN=32, grid=(1,16) = 16 blocks.
- Target: quant phase for K=512 shapes (4x2880x512, 32x4096x512, 32x2880x512).
"""
import torch
import triton
