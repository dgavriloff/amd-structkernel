...
"""
v100: BSM=8 BSN=128 BSK=256 NW=4 NS=2 for M<=32 K>1024 path.

Hypothesis: The M<=32 K>1024 path uses BSM=32 BSN=64 BSK=512 NW=8 NS=1
(legacy auto-tuned config). With BSM=32 and M=16, half the rows are wasted.
BSM=8 BSN=128 BSK=256 NW=4 NS=2 (the pattern that works for K<=1024 and
split-K) should work better: no wasted rows, more blocks, software pipelining.

Target: Secret leaderboard shapes with M<=32 K>1024 (e.g., 16x3072x1536).
"""
import torch
import triton
