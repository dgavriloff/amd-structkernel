...
"""
v218: GPU_MAX_HW_QUEUES=1 + .wt on M=256 scale stores.

Hypothesis: GPU_MAX_HW_QUEUES=1 limits hardware command queues to 1,
reducing GPU command processor scheduling overhead for our single-queue
workload. Previous env vars (HIP_FORCE_DEV_KERNARG, HSA_ENABLE_SDMA,
AMD_DIRECT_DISPATCH) targeted different mechanisms. HW queues control
the GFX command processor's queue management which may add overhead when
multiple queues exist but only one is active.
Also: .wt on M=256 scale stores (confirmed 17x beneficial on M=256).
"""
import os
