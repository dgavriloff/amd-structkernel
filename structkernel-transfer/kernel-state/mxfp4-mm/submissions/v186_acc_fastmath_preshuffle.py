...
"""
v186: Custom preshuffle kernel with acc=acc pattern + fast_math=True.

Hypothesis: The aiter MoE a4w4 kernel uses both:
1. acc=acc parameter to tl.dot_scaled (in-place accumulation, may enable
   fused MFMA accumulate instead of separate add)
2. fast_math=True
The preshuffle GEMM kernel uses accumulator += tl.dot_scaled() (external add).
The MFMA instruction natively supports fused accumulate, so acc=acc may
generate fewer instructions. v185 tested fast_math alone (neutral); this
tests the acc=acc codegen path which is a different change.
"""
import torch
import triton
...
# Custom preshuffle kernel with acc=acc pattern from MoE kernel
@triton.heuristics(
    {
...
def _custom_preshuffle_kernel(
    ...
    if (pid_k * SPLITK_BLOCK_SIZE // 2) < K:
        ...
        for k in range(pid_k * num_k_iter, (pid_k + 1) * num_k_iter):
            ...
            # KEY CHANGE: acc=acc pattern (in-place accumulate, from MoE kernel)
            # + fast_math=True
            acc = tl.dot_scaled(a, a_scales, "e2m1", b, b_scales, "e2m1", acc=acc, fast_math=True)
