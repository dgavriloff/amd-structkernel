...
"""
v217: AMD_DIRECT_DISPATCH=1 + .wt on M=256 scale stores.

Hypothesis: Two changes targeting different phases:
1. AMD_DIRECT_DISPATCH=1 enables ROCm direct dispatch mode, bypassing host-side
   command buffer management. This reduces kernel launch overhead for all shapes.
   Never tested before (v213 tested HIP_FORCE_DEV_KERNARG, v214 tested HSA_ENABLE_SDMA).
2. .wt cache modifier on M=256 scale stores (from .cg) — confirmed real -0.8 to -3.1%
   on M=256 across 17 tests. Combined with dispatch optimization for potential
   compound effect.
"""
import os
