...
"""
v193: Combination sweep for M=64 — three previously-neutral changes combined.

Hypothesis: v080 (waves_per_eu=2, neutral), v102 (GROUP_SIZE_M=2, -0.6% BM),
and v103c (cache_modifier=None, -0.8% BM) each showed within-noise improvements
on M=64. Combining all three may yield a compound improvement detectable above
noise. Changes: waves_per_eu=1→2, GROUP_SIZE_M=1→2, cache_modifier=".cg"→None.
Based on v188 (global best).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # Combo sweep: waves_per_eu=2 (v080 neutral), GROUP_SIZE_M=2 (v102 -0.6% BM),
        # cache_modifier=None (v103c -0.8% BM) — combined for compound effect
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
