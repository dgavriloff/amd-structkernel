...
"""
v097: NW=2 for split-K path (fewer warps, less register pressure).

Hypothesis: BSM=8 NW=4 uses 238 blocks with 4 warps each. With NW=2,
each block uses fewer registers, potentially allowing higher occupancy.
For BSM=8 BSN=128 with only 8 rows per block, 2 warps may provide
sufficient memory parallelism while reducing register pressure.

Target: 16x2112x7168 shape (12.0us LB).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if K > 4096:
        ...
        # BSM=8: 238 blocks (0.93 waves), NW=2 for less register pressure
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
