...
"""
v130: cache_modifier=None for split-K path (16x2112x7168).

Hypothesis: The split-K=7 path has 7 blocks processing overlapping B data
for different K ranges. With .cg (L2-only cache), B data loaded by one
block's K range cannot be reused in L1 by another block processing
the same B tiles but different K range. With cache_modifier=None (L1+L2),
B data stays in L1 cache and can be reused by blocks on the same CU.

Library tuned config for M_LEQ_16 uses cache_modifier=null (not .cg).
The .cg modifier was chosen based on the M_LEQ_8 config which was tuned
for BSM=8 shapes, but the split-K path has different B access patterns.

Target: 16x2112x7168 (11.9us LB).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if K > 4096:
        ...
        # cache_modifier=None: L1+L2 caching for B data reuse across split-K blocks
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
