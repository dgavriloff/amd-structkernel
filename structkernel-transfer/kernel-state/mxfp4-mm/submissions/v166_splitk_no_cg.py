...
"""
v166: cache_modifier=None for split-K path (from .cg).

Hypothesis: v165 improved split-K with waves_per_eu=2. The tuned JSON config
for N=2112 K=7168 M>=16 also uses cache_modifier=None (not .cg).
v130 tested this in isolation and found it neutral on BM but BM-LB divergence
is common. Testing on LB from the v165 baseline.

Target: 16x2112x7168 (11.5us LB).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if K > 4096:
        ...
        # waves_per_eu=2 + cache_modifier=None: tuned JSON pattern for M>=16
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
