...
"""
v219: .wt on M=256 scale stores + server version probe.

Hypothesis: Two changes:
1. .wt cache modifier on M=256 quant SCALE stores (from .cg) — pushes scale data
   to L2 faster for subsequent ASM GEMM read. Confirmed real -0.8% to -3% on
   M=256 BM across 18 tests, but LB noise has masked it at geomean level.
2. Server environment may have been updated since v204 (6 branches/~10 sessions ago).
   A newer Triton/aiter version could produce different cached binaries.
"""
import torch
import triton
