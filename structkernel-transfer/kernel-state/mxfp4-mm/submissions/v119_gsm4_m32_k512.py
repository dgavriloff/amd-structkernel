...
"""
v119: GROUP_SIZE_M=4 for M<=32 K<=1024 path.

Hypothesis: With BSM=8 (from v099), we now have 4 M-tiles for M=32 shapes.
GROUP_SIZE_M=4 groups all 4 M-tiles together, ensuring each group of blocks
shares the same B-tile in L2 cache. This is different from v072's test because
BSM was 16 then (only 2 M-tiles, GROUP_SIZE_M=2). With 4 M-tiles, the B-tile
reuse opportunity is larger.
