...
"""
v208: HIP quant kernel for M=256 two-phase path.

Hypothesis: Replace Triton _fused_mxfp4_quant_shuffle_kernel with aiter's native
HIP quant kernel (dynamic_per_group_scaled_quant_fp4) for the M=256 two-phase path.
The HIP kernel uses hardware v_cvt_scalef32_pk_fp4_bf16 and has lower launch overhead
than Triton (~2us vs ~6.4us). This is the most frequently requested untried direction
across 59 branches. M<=64 paths are completely unchanged.
"""
import torch
import triton
...
from aiter.ops.quant import dynamic_per_group_scaled_quant_fp4
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            # Pre-allocate buffers for HIP quant output
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # HIP quant kernel — native HIP, lower launch overhead than Triton
