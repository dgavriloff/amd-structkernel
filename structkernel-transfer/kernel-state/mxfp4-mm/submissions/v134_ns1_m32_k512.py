...
"""
v134: num_stages=1 for M<=32 K<=1024 fused shapes.

Hypothesis: With only 2 K-iterations (K_kernel=256, BSK=256), the double-
buffering from num_stages=2 provides minimal pipelining benefit.
num_stages=1 reduces register pressure, potentially improving occupancy.
The preshuffle kernel's K-loop uses regular range() so Triton treats
num_stages as a general register/cache policy hint.

Target: 32x4096x512 (6.54us LB), 32x2880x512 (6.64us LB).
"""
import torch
import triton
