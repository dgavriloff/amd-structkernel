...
"""
v223: .cs (cache streaming) on M=256 scale stores.

Hypothesis: Replace .cg with .cs on M=256 quant scale stores. Cache streaming
marks data as most-recently-used in cache, keeping scales hot for the
immediately subsequent ASM GEMM kernel. Different from .wt (write-through,
bypasses L1) and .cg (cache global, L2 only). .cs was attempted on fp4 stores
in v020 but rejected — may have been a Triton version issue (now Triton 3.6.0).
Never tested on scale stores specifically.
"""
import torch
import triton
