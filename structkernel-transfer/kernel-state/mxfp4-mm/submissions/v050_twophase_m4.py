...
"""
v050: Route M<=4 to two-phase path (leaderboard correctness fix).
Fused preshuffle GEMM produces incorrect results for M=4 with both BSM=4 and BSM=8.
Route M<=4 to the two-phase Triton quant + ASM GEMM path which is proven correct.
Keep all other v047 changes: BSM=16 BSN=128 for M=32, split-K=7 BSK=256 for 16x2112x7168.
"""
import torch
import triton
...
_FUSED_M_MIN = 4  # M<=4 uses two-phase (fused preshuffle has correctness issues for M=4)
...
def _get_fused_config(M, N, K):
    ...
    if M <= 8:
        ...
        # Custom config for 16x2112x7168: BSK=256 split-K=7 num_stages=2.
        # BSK=256 gives 4 K-iterations per block (vs 2 with BSK=512),
        # enabling deeper software pipelining.
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
