...
"""
v038: Hybrid with custom configs for fused path.
- Same hybrid as v037: fused for M<=32, two-phase for M>=64.
- Pass custom config to fused path:
  - For all fused shapes: BLOCK_SIZE_N=128, BLOCK_SIZE_K=512, num_warps=4,
    waves_per_eu=2. (Matches tuned config for 16x2112x7168.)
  - K=512 shapes: NUM_KSPLIT=1 (can't split K=512 further).
  - 16x2112x7168: NUM_KSPLIT=14 (per specialized config).
- Target: improve fused path by using tuned tile configs.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    """Get optimized config for fused quant+GEMM path."""
    # Base config matching tuned preshuffle configs
    config = {
        "BLOCK_SIZE_M": 32 if M >= 32 else (16 if M >= 16 else 8),
        "BLOCK_SIZE_N": 128,
        "BLOCK_SIZE_K": 512,
        "GROUP_SIZE_M": 1,
        "num_warps": 4,
    ...
    # Use split-K for large K
    if K >= 2048:
        # Per tuned config, NUM_KSPLIT=14 for K=7168
        config["NUM_KSPLIT"] = max(1, K // 512)
    return config
