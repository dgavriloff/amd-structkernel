...
"""
v019: Extend small-block quant config to M=64/M=256 for better CU utilization.
- M>32 path uses BSM=32 BSN=128 NI=4: grid=(2,4)=8 blocks for M=64 K=2048 (3% CU util).
- M<=32 path uses BSM=M BSN=32 NI=1: grid=(1,K/32) blocks (much better CU util).
- Extend M<=32 style to all M<=256: BSM=next_pow2(M) BSN=32 NI=1 NW=1.
- 64x7168x2048: grid=(2,64)=128 blocks (50% CU util vs 3.1%).
- 256x3072x1536: grid=(8,48)=384 blocks (150% CU util vs 9.4%).
- Target: quant phase for M=64 and M=256 shapes.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # Use small blocks (BSM=32, BSN=32) for ALL shapes to maximize CU utilization.
        # M>32 used BSM=32 BSN=128 NI=4: grid=(2,4)=8 blocks for M=64 K=2048 (3% CU util).
        # BSM=32 BSN=32 NI=1: grid=(2,64)=128 blocks for M=64 K=2048 (50% CU util).
        NUM_ITER = 1
