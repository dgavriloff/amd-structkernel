...
"""
v141: Library-optimized quant config for M=256 two-phase path.

Hypothesis: The M=256 quant kernel uses BSM=32 BSN=64 NW=2 NUM_ITER=1 NUM_STAGES=1
(192 blocks, no pipelining). The aiter library uses BSM=32 BSN=128 NW=4 NUM_ITER=4
NUM_STAGES=2 (24 blocks with 4 warps and 2-stage pipelining over 4 iterations).
The pipelining enables compute/memory overlap within each block, potentially reducing
the quant kernel time. Previous tests (v110, v111, v113) tested parameters individually
but never the exact library combination.
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            # Library-optimized config: BSM=32 BSN=128 NW=4 NUM_ITER=4 NUM_STAGES=2
            # Enables 2-stage pipelining across 4 iterations per block
            NUM_ITER = 4
            BLOCK_SIZE_M = 32
            BLOCK_SIZE_N = 128
            NUM_WARPS = 4
