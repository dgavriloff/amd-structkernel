...
"""
v153: ASM GEMM log2_k_split=1 for M=256 two-phase path.

Hypothesis: M=256 N=3072 K=1536 with 32x128 ASM tile gives 192 blocks (0.75 waves).
With log2_k_split=1, each block handles K/2=768 instead of K=1536, doubling to
384 blocks (1.5 waves). Better CU utilization may offset the split-K reduction cost.
The ASM GEMM handles split-K internally (unlike v103a which used 256x256 tile).
