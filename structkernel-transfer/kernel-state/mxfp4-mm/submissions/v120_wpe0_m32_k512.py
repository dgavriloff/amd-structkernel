...
"""
v120: waves_per_eu=0 for M<=32 K<=1024 path.

Hypothesis: With BSM=8 (from v099), we have 92-128 blocks for M=32 K=512 shapes.
waves_per_eu=2 was set to force higher occupancy when BSM=16 gave fewer blocks.
Now with BSM=8 and more blocks, auto-occupancy (waves_per_eu=0) might produce
better register allocation. waves_per_eu=0 was tested for split-K (v098, neutral)
but never for the M<=32 K<=1024 config with BSM=8.

Target: 32x4096x512 (6.54µs LB), 32x2880x512 (6.64µs LB).
"""
import torch
import triton
