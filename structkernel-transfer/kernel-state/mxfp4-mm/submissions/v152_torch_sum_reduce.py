...
"""
v152: Replace Triton reduce kernel with torch.sum for split-K path.

Hypothesis: The Triton reduce kernel (_gemm_afp4wfp4_reduce_kernel) launches 132
blocks to sum 7 fp32 partial results of shape (16, 2112). torch.sum(y_pp, dim=0)
uses highly optimized PyTorch/hipBLAS reduction that may be faster than a custom
Triton kernel launch for this small reduction.

Target: 16x2112x7168 split-K path.
"""
import torch
import triton
        ...
        if M <= _FUSED_M_THRESHOLD:
            ...
            if config["NUM_KSPLIT"] > 1:
                ...
                _buffers[key] = {
                    ...
                    '_sum_buf': torch.empty((M, N), dtype=torch.float32, device=device),
                    'B_w': None,
                    'B_sc': None,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # Use torch.sum instead of Triton reduce kernel for split-K reduction
        # y_pp is (7, 16, 2112) fp32 -> sum over dim=0 -> (16, 2112) fp32 -> bf16
        torch.sum(y_pp[:kp['ACTUAL_KSPLIT']], dim=0, out=buf['_sum_buf'])
        out.copy_(buf['_sum_buf'])
