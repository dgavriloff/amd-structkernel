...
"""
v090: num_stages=3 for 16x2112x7168 split-K=7 path.

With split-K=7 BSK=256, each block does 4 K-iters. num_stages=3 enables
triple-buffering for deeper compute/memory overlap. v065 tried NW=8 NS=3
which had register pressure; this uses NW=4 NS=3.
Also removes .cg cache modifier to reduce L1 contention in split-K.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if K > 4096:
        ...
        # num_stages=3 for triple-buffered pipeline (4 K-iters per block)
        # No cache modifier to reduce L1 contention in split-K
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
