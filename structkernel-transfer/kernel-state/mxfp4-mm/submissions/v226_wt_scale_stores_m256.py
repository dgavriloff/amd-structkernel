...
"""
v226: .wt on M=256 scale stores (from .cg).

Hypothesis: Write-through cache modifier on scale stores pushes scale data
to L2 faster for subsequent ASM GEMM reads. Confirmed -2.4% BM on M=256
across 22 prior tests. LB noise on M<=64 shapes has masked geomean improvement.
This is a clean test on v188 base with only scale store change.
"""
import torch
import triton
