...
"""
v129: waves_per_eu=4 for M=64 fused path.

Hypothesis: The non-preshuffle A16WFP4 tuned config for N=7168, K=2048 uses
waves_per_eu=4 for M_LEQ_64. Our preshuffle kernel uses waves_per_eu=1.
Higher waves_per_eu forces lower register usage per wave, enabling more
concurrent waves per CU. With 224 blocks and 256 CUs, higher occupancy via
wave switching could better hide memory latency.

waves_per_eu=0 (auto) and 2 were tested as neutral (v073, v103d).
waves_per_eu=4 has NOT been tested for this path.

Target: 64x7168x2048 (13.6us LB).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # waves_per_eu=4 forces higher occupancy via wave switching (library tuned value)
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
