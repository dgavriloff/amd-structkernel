...
"""
v160: Split-K=2 for M=64 fused path to double CU utilization.

Hypothesis: M=64 (64x7168x2048) uses NUM_KSPLIT=1 with 224 blocks (0.88 waves).
With split-K=2, each block processes K_kernel/2=512 elements (2 K-iters instead of 4),
and we get 448 blocks (1.75 waves). The reduce kernel for 2 partials should be cheap
(sum of 2 fp32 values per element). Net gain from better CU utilization should exceed
reduce kernel overhead.

Target: 64x7168x2048 (current ~13.0us LB).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # split-K=2: 448 blocks (1.75 waves) vs 224 blocks (0.88 waves)
        # Each block does 2 K-iters instead of 4, then reduce sums 2 fp32 partials
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
