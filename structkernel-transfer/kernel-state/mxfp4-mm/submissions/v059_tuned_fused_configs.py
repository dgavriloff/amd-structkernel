...
"""
v059: Tuned fused configs + data_ptr cache fix.

v058 proved fused path works on leaderboard (11.4us). This version re-applies
the tuned configs from v044 (BSM=4 for M<=4, BSM=16 BSN=128 for M=32 K<=1024)
which previously achieved 9.8us benchmark but failed leaderboard due to stale
cache bug (now fixed).
"""
import torch
import triton
...
# Threshold: use fused for M <= this value, but only for moderate K
_FUSED_M_THRESHOLD = 32
_FUSED_K_MAX = 4096  # M=16 K=7168 is 84.5us fused vs ~14us two-phase
...
def _get_fused_config(M, N, K):
    ...
    Tuned configs from v044 session.
    """
    if M <= 4:
        ...
        # For M=16/32 with short K: BSM=16 doubles M-tile count, BSN=128 NW=4
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
        ...
        # Fallback for M=16/32 with K>1024 (but K<=4096 due to _FUSED_K_MAX)
        return {
            "BLOCK_SIZE_M": 32,
            "BLOCK_SIZE_N": 64,
            "BLOCK_SIZE_K": 512,
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD and K <= _FUSED_K_MAX:
            _buffers[key] = {
                'mode': 'fused',
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused':
        # Recompute B_w/B_sc if B_shuffle changed (different seed/data for same shape)
