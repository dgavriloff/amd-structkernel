...
"""
v195: Remove .cg from quant kernel A loads (M=256 two-phase path).

Hypothesis: cache_modifier=".cg" bypasses L1 cache, going straight to L2.
For M=256 with BLOCK_SIZE_M=32, each block loads 32x64 bf16 = 4KB which fits
in L1 (32KB per CU on MI355X). Removing .cg allows L1 caching of A input data,
potentially reducing load latency. The .cg modifier was inherited from the
original kernel without testing alternatives. eviction_policy parameter also
tested — if supported on ROCm, evict_first would mark A data for early eviction
since it's read-once.
"""
import torch
import triton
