...
"""
v143: GROUP_SIZE_M=4 for M=64 fused path.

Hypothesis: M=64 with BSM=16 creates 4 M-tiles x 56 N-tiles = 224 blocks.
With GROUP_SIZE_M=1, blocks sweep across N first (block 0: M0-N0, block 1: M0-N1, ...).
With GROUP_SIZE_M=4, all 4 M-tiles share the same N-tile before advancing
(block 0-3: M0-3-N0, block 4-7: M0-3-N1, ...). This maximizes B-tile reuse in L2
since all 4 M-tiles read the same B data. v102 tested GSM=2 (neutral) but GSM=4
(matching the full M-tile count) hasn't been tested.
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # GROUP_SIZE_M=4: all 4 M-tiles share same B data in L2
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
