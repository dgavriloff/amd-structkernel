...
"""
v039: Hybrid with shape-specific configs for fused path.
- M<=32: fused quant+GEMM via gemm_a16wfp4_preshuffle.
- M>=64: two-phase (Triton quant + ASM 32x128 GEMM).
- Shape-specific fused configs:
  - M<=8: BSM=8 BSN=128 BSK=512 NW=4 (tuned config style, -14% on M=4)
  - M=16/32: Use default config (BSM=32 BSN=64 BSK=512 from "any")
- v038 showed BSN=128 hurts M=32 (+9%) but helps M=4 (-14%).
"""
import torch
import triton
...
# Also use fused for M=64 with tuned split-K config
_FUSED_M_THRESHOLD = 64


def _get_fused_config(M, N, K):
    """Get shape-specific config for fused quant+GEMM path.
    Only override when we know the default is suboptimal AND no specialized config exists.
    Return None to let gemm_a16wfp4_preshuffle use its own config lookup.
    """
    if M <= 8:
        # Tuned config for very small M: BSM=8 avoids wasting rows
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
    ...
    elif M <= 64 and K >= 2048:
        # M=64 with large K: split-K for parallelism
        return {
            "BLOCK_SIZE_M": 32,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
        ...
        # Let gemm_a16wfp4_preshuffle pick its own config
        return None
