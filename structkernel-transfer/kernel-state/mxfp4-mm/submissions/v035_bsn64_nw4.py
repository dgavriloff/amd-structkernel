...
"""
v035: BSN=64 NW=4 for quant kernel — 4 warps with 64-element blocks.
- v021 uses BSN=64 NW=2: 2 warps per block.
- BSN=64 NW=4: same grid as v021 but 4 warps for more memory-level parallelism.
- Extra warps can hide memory latency in the quant op (amax reduction, scale compute).
- Same block count as v021, so CU utilization unchanged.
- Target: quant phase — reduce per-block latency via warp-level parallelism.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # BSN=64 NW=4: 4 warps with 64-element blocks for more memory-level parallelism.
        NUM_ITER = 1
