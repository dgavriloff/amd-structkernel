...
"""
v018: Add software pipelining for quant kernel on large-K shapes (M<=32).
- When M<=32 and K>1024, use NUM_ITER=2 with NUM_STAGES=2 for pipeline overlap.
- This overlaps loads of the next tile with computation of the current tile.
- Grid blocks remain sufficient: e.g., K=7168 → 112 blocks (vs 224 before).
- Target: quant phase for 16x2112x7168 (quant ~6.4µs = 30% of 21.2µs total).
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        if M <= 32:
            ...
            NUM_WARPS = 1
            if K > 1024:
                # Large K: use pipelining (NUM_ITER=2, NUM_STAGES=2)
                NUM_ITER = 2
                NUM_STAGES = 2
            else:
                NUM_ITER = 1
        ...
        # Note: removed K<=1024 override (v006 had BLOCK_SIZE_N=256, creating only
        # 2 blocks for K=512 — terrible CU utilization on MI355X with 256 CUs)
