...
"""
v148: BSM=8 for M=64 fused path to double CU utilization.

Hypothesis: M=64 uses BSM=16 BSN=128 giving 4*56=224 blocks (0.88 waves).
BSM=8 gives 8*56=448 blocks (1.75 waves). BSM=8 was a win for M<=32 K<=1024
in v099. Similar improvement expected for M=64.

Target: 64x7168x2048 (13.4us LB).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # 8*56=448 blocks (1.75 waves), 8 K-iters with pipelining
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
