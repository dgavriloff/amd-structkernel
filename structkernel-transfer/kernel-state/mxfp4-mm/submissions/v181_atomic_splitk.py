...
"""
v181: Atomic-add split-K: eliminate reduce kernel for 16x2112x7168.

Hypothesis: The split-K path currently launches 2 kernels (preshuffle + reduce).
The reduce kernel has ~4us Triton launch overhead. By using tl.atomic_add in the
preshuffle kernel to accumulate directly into a pre-zeroed fp32 buffer, we eliminate
the reduce kernel entirely. The output is then converted to bf16 with a fast
PyTorch .to() call.

Trade-off: atomic_add has contention (7-way for split-K=7) but blocks finish at
different times, limiting actual contention. zero_() + to() add ~1us each but save
the ~4us reduce kernel launch overhead.

Target: 16x2112x7168 (11.5us -> ~8-9us estimated).
"""
import torch
import triton
...
# Custom preshuffle kernel with ATOMIC_ADD support for split-K
@triton.heuristics(
    {
...
def _preshuffle_atomic_kernel(
    ...
    """Preshuffle kernel with atomic_add for split-K accumulation.
    c_ptr must point to a pre-zeroed fp32 buffer of shape (M, N).
    """
    ...
    # Simple linear pid mapping for split-K (no pid_grid to avoid dependency)
    pid_m = pid // num_pid_n
    pid_n = pid % num_pid_n
    ...
    if (pid_k * SPLITK_BLOCK_SIZE // 2) < K:
        ...
        # Atomic add to shared fp32 output (no per-split stride needed)
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    Uses atomic_add to eliminate the reduce kernel.
    """
    K_kernel = K // 2
    ...
    # Pre-allocate fp32 accumulation buffer (replaces y_pp)
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            if config["NUM_KSPLIT"] > 1:
                # Split-K path: use atomic_add (no reduce kernel)
                splitk_params = _prepare_splitk_dispatch(M, N, K, config, device)
                _buffers[key] = {
                    'mode': 'fused_splitk_atomic',
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk_atomic':
        # Atomic split-K path: single kernel + bf16 convert (no reduce kernel)
        ...
        # Zero the accumulation buffer before atomic_add
        y_fp32.zero_()
        ...
        # Convert fp32 accumulation to bf16 output
        out.copy_(y_fp32)
