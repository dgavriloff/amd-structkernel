...
"""
v044: Shape-specific fused configs with BSM=4 for M<=4 and BSM=16 BSN=128 for M=32 K<=1024.
- M<=4: BSM=4 BSN=128 NW=4 .cg (eliminate 50% wasted rows vs BSM=8)
- 4<M<=8: BSM=8 BSN=128 NW=4 .cg (unchanged from v039)
- 8<M<=32, K<=1024: BSM=16 BSN=128 NW=4 .cg (2 M-tiles vs 1, better CU util)
- 8<M<=32, K>1024: config=None (preserves specialized split-K=14 for 16x2112x7168)
- M>=64: two-phase (Triton quant + ASM 32x128 GEMM)
Target: M=4 BSM=4 saves 50% wasted M-dim compute. M=32 BSM=16 doubles M-tiles.
"""
import torch
import triton
