...
"""
v054: Shape-specific log2_k_split for ASM GEMM.
16x2112x7168 has only 17 tiles (32x128) for 256 CUs = 6.6% occupancy.
log2_k_split=2 quadruples tiles to 68, improving CU utilization from 6.6% to 26.6%.
"""
import torch
import triton
...
# Fused preshuffle path disabled — fails leaderboard correctness for all tested M values.
# All shapes use two-phase: Triton quant+shuffle kernel → ASM GEMM.
_FUSED_M_MIN = 999999  # Effectively disabled
_FUSED_M_THRESHOLD = 0
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M > _FUSED_M_MIN and M <= _FUSED_M_THRESHOLD:
            ...
            # Shape-specific split-K for ASM GEMM to improve CU utilization
            n_tiles = triton.cdiv(N, 128)  # 128 = ASM N-tile size
            m_tiles = triton.cdiv(M, 32)   # 32 = ASM M-tile size
            total_tiles = n_tiles * m_tiles
            # Target ~128+ tiles for good CU utilization (256 CUs on MI355X)
            if total_tiles < 32:
                log2_k_split = 3  # 8x split
            elif total_tiles < 64:
                log2_k_split = 2  # 4x split
            elif total_tiles < 128:
                log2_k_split = 1  # 2x split
            else:
