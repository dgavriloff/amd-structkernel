...
"""
v165: Direct dispatch for ALL fused shapes + waves_per_eu=2 for split-K.

Hypothesis: Combine two independently marginal optimizations:
1. Direct kernel dispatch for non-split-K fused shapes (v094 showed -0.9% LB
   as standalone change). Bypasses gemm_a16wfp4_preshuffle wrapper overhead
   (serialize_dict, config lookup, grid lambda).
2. waves_per_eu=2 for split-K path. v098 tested waves_per_eu=0 (auto) but
   not waves_per_eu=2. The tuned JSON config for N=2112 K=7168 uses
   waves_per_eu=2 with cache_modifier=None for M>=16.

Target: All shapes, especially M<=32 non-split-K and 16x2112x7168 split-K.
"""
import torch
import triton
