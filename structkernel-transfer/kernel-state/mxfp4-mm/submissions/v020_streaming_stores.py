...
"""
v020: Add streaming store (.cs) for fp4 output writes.
- fp4 data is write-once (quant) then read-once (GEMM).
- Streaming stores bypass caches, freeing L2 space for GEMM reads of B matrix.
- Should reduce cache thrashing between quant and GEMM phases.
- Target: all shapes, especially GEMM-bound ones like 16x2112x7168.
"""
import torch
import triton
