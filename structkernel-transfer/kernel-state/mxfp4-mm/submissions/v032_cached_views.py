...
"""
v032: Cache .view() tensors and pre-compute all GEMM arguments.
- Pre-compute fp4x2 and e8m0 views of quant buffers at init time
- Cache the exact kernel name string
- Minimize per-call Python overhead by pre-computing everything possible
- Target: Python dispatch overhead (reduce per-call overhead by ~0.5µs)
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        ...
        # Pre-compute .view() tensors (avoids creating new tensor objects per call)
        ...
        _buffers[key] = {
            ...
            'gemm_out': gemm_out,
            'x_fp4_view': x_fp4_view,
            'blockscale_view': blockscale_view,
            'SCALE_N': SCALE_N,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Direct ASM call with 32x128 kernel — use pre-computed views
    gemm_a4w4_asm(
        buf['x_fp4_view'],
        B_shuffle,
        buf['blockscale_view'],
