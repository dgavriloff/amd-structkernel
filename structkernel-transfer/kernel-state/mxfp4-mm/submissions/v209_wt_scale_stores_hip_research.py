...
"""
v209: .wt on M=256 scale stores (12th attempt) + HIP quant kernel research.

Based on v188. Changes:
1. .wt cache modifier on M=256 scale stores (from .cg) — confirmed real -0.8% to
   -3.1% on M=256 across 11 tests (v176-v207). Only affects two_phase path.
2. M<=64 paths completely unchanged from v188.

Research: HIP quant kernel (dynamic_per_group_scaled_quant_fp4) tested as v208 —
FAILS correctness (509432/737280 mismatched for 256x2880x512). The HIP kernel uses
a different scale convention (fp4_scale * 0.25 stores reciprocal exponent) vs Triton
(floor(log2(amax))-2+127 stores direct exponent). Scale shuffle permutations are
bit-identical but the scale VALUES differ.
"""
import torch
import triton
