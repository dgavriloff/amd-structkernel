...
"""
v118: REDUCE_BSN=32 for split-K reduce kernel.

Hypothesis: Current REDUCE_BSN=16 gives 132 reduce blocks. BSN=32 gives 66 blocks.
Each block processes more N elements (32 vs 16), potentially better utilizing
memory bandwidth per block. BSN=8 (264 blocks) was too many tiny blocks (v095),
BSN=64 (33 blocks) was too few (library default). BSN=32 is untested.

Target: 16x2112x7168 (11.9µs LB).
"""
import torch
import triton
...
def _prepare_splitk_dispatch(M, N, K, config, device):
    ...
    REDUCE_BSM = 16
    REDUCE_BSN = 32  # Test BSN=32 (66 blocks) vs BSN=16 (132 blocks)
