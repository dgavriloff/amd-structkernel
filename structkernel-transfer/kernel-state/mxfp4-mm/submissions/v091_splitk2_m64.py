...
"""
v091: Split-K=2 for M=64 (64x7168x2048).

v077 has 224 blocks (0.875 waves) for M=64. Split-K=2 doubles to 448 blocks
(1.75 waves) for better CU saturation. Each block still does 4 K-iters
(BSK=256, K=2048, 2048/256=8, 8/2=4), sufficient for NS=2 pipelining.
v078 tried split-K=4 (+58%) which was too aggressive (only 2 K-iters).
Split-K=2 is more conservative with 4 K-iters and minimal reduce overhead.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=16 BSN=128 BSK=256 NW=4 NS=2, split-K=2
        # 4*56*2=448 blocks, 4 K-iters with pipelining, 1.75 waves
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
