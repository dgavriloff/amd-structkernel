...
"""
v025: Triton num_stages=2 for compiler pipeline hint.
- v021 uses num_stages=1 (Triton launch parameter).
- num_stages=2 tells the Triton compiler to use 2-stage software pipelining for loads.
- This can overlap memory loads with compute within the kernel body.
- Different from NUM_STAGES (tl.range loop param) — this controls Triton's own pipelining.
- Target: quant phase — test if compiler pipelining improves throughput.
"""
import torch
import triton
