...
"""
v147: BSM=4 for split-K path (16x2112x7168) to double CU utilization.

Hypothesis: Current BSM=8 gives 238 blocks (0.93 waves). BSM=4 gives 476 blocks
(1.86 waves) — double CU saturation. Each block processes 4 M-rows instead of 8,
with same BSK=256 and split-K=7. The BSM=4 config already works for M=4 shapes.
...
def _get_fused_config(M, N, K):
    ...
    if K > 4096:
        ...
        # BSM=4: 476 blocks (1.86 waves) vs BSM=8: 238 blocks (0.93 waves)
        return {
            "BLOCK_SIZE_M": 4,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
