...
"""
v196: .wt on quant scale stores (M=256 two-phase path).

Hypothesis: Scale stores currently use .cg (cache global, L2 only). Changing to
.wt (write-through) pushes scale data to L2 immediately for the ASM GEMM kernel
to consume, matching the .wt strategy that improved fp4 stores by -1.5 to -3%
on M=256 (confirmed 6 times). Scales are small (256x48 uint8 = 12KB for 256x1536)
so .wt overhead is minimal, but faster L2 availability could reduce GEMM stalls.
"""
import torch
import triton
