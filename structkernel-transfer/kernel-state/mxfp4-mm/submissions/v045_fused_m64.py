...
"""
v045: Extend fused path to M<=64 with BSM=16 BSN=128 NW=4 config.
- M<=4: BSM=4 BSN=128 NW=4 .cg
- 4<M<=8: BSM=8 BSN=128 NW=4 .cg
- 8<M<=32, K<=1024: BSM=16 BSN=128 NW=4 .cg
- 8<M<=32, K>1024: config=None (preserves split-K=14)
- 32<M<=64: BSM=16 BSN=128 NW=4 .cg, waves_per_eu=2 (fused, 4 M-tiles)
- M>64: two-phase (Triton quant + ASM 32x128 GEMM)
Hypothesis: BSM=16 config was -15% for M=32. For M=64, grid=4*56=224 blocks.
Fused eliminates 6.4µs quant overhead. Even if Triton GEMM is slower than ASM,
the quant savings + good tile config may net out positive.
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # For M=64: BSM=16 creates 4 M-tiles, BSN=128. Fused saves quant launch.
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
        ...
        # For M=16 with large K (e.g., 16x2112x7168): let library pick
        # specialized config with split-K=14
        return None
