...
"""
v171: Pre-compute B_w/B_sc strides as constants + skip data_ptr check.

Hypothesis: On LB with recheck=True, every iteration creates new B data.
The current code checks data_ptr() to detect changes and recreates views,
then calls .stride() on those views. Since strides are deterministic from
shape alone, pre-compute them at buffer creation time. Also, since B always
changes on LB, remove the data_ptr check and always recreate views.

This eliminates per-call overhead:
- 1 data_ptr() call (~50ns)
- 1 dict.get() call (~50ns)
- 4 .stride() calls (~200ns)
- 1 conditional branch

Total: ~300ns per call, small but compounds over 100 LB iterations.
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            # Pre-compute B_w and B_sc strides (deterministic from shape)
            bw_stride0 = (K // 2) * 16
            bw_stride1 = 1
            # B_scale_sh shape is typically (N_pad // 32, K_scale_pad * 32)
            # We'll compute bs strides after first B_scale_sh is seen
            if config["NUM_KSPLIT"] > 1:
                ...
                _buffers[key] = {
                    ...
                    'bs_stride0': None,  # computed on first call
                    'bs_stride1': 1,
                }
            else:
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        # Split-K path — always recreate views (LB changes data every iter)
        ...
        # Compute bs_stride0 on first call if needed
        if buf['bs_stride0'] is None:
            buf['bs_stride0'] = bs_shape[1] * 32
        ...
        # Non-split-K fused path — always recreate views (LB changes data every iter)
        B_w = B_shuffle.view(torch.uint8).reshape(N // 16, (K // 2) * 16)
        bs_shape = B_scale_sh.shape
        B_sc = B_scale_sh.view(torch.uint8).reshape(
            bs_shape[0] // 32, bs_shape[1] * 32
