...
"""
v161: BSK=512 NW=8 NS=1 for M=64 fused path — wider K-tile, fewer K-iterations.

Hypothesis: M=64 (64x7168x2048, K_kernel=1024) currently uses BSK=256 NW=4 NS=2
with 4 K-iterations per block. With BSK=512 NW=8, each block processes 512 K-elements
per iteration (2 iterations instead of 4). Fewer loop iterations reduce loop overhead.
NW=8 provides enough memory bandwidth to feed the wider K-tile.
NS=1 since we only have 2 K-iterations, pipelining adds register pressure.

Target: 64x7168x2048 (~8.8us BM).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=64 (64x7168x2048): BSM=16 BSN=128 BSK=512 NW=8 NS=1
        # 4*56=224 blocks, 2 K-iters (wider tile, fewer iterations)
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
