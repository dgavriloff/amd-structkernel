...
"""
v135: Use fp4*fp4 Triton kernel with XCD remap for M=256 two-phase path.

Hypothesis: The _gemm_afp4wfp4_kernel_preshuffle_scales kernel has XCD remap,
3-arg tl.dot_scaled, and .wt output stores. It's a library kernel (no
compilation artifact issue). For M=256 N=3072 K=1536, the tuned config uses
BSM=128 BSN=32 BSK=512 NW=4, giving 192 blocks with XCD distribution.
This could be faster than the ASM 32x128 GEMM which has no XCD awareness.

Target: 256x3072x1536 (13.3us LB).
"""
import torch
import triton
...
def _get_or_create_buffers(M, K, N, device):
    ...
    if key not in _buffers:
        if M <= _FUSED_M_THRESHOLD:
            ...
            NUM_ITER = 1
            BLOCK_SIZE_M_Q = min(32, triton.next_power_of_2(M))
            BLOCK_SIZE_N_Q = 64
            NUM_WARPS_Q = 2
            NUM_STAGES_Q = 1

            BLOCK_SIZE_M_Q = triton.cdiv(BLOCK_SIZE_M_Q, 32) * 32
            BLOCK_SIZE_N_Q = triton.cdiv(BLOCK_SIZE_N_Q, 32) * 32

            quant_grid = (
                triton.cdiv(M, BLOCK_SIZE_M_Q),
                triton.cdiv(K, BLOCK_SIZE_N_Q * NUM_ITER),
            )

            # fp4*fp4 Triton GEMM config (tuned for N=3072 K=1536)
            K_fp4 = K // 2
            gemm_config = {
                "BLOCK_SIZE_M": 128,
                "BLOCK_SIZE_N": 32,
                ...
                "cache_modifier": None,
                "NUM_KSPLIT": 1,
                "SPLITK_BLOCK_SIZE": 2 * K_fp4,
            }

            gemm_grid_size = triton.cdiv(M, gemm_config["BLOCK_SIZE_M"]) * triton.cdiv(N, gemm_config["BLOCK_SIZE_N"])

            _buffers[key] = {
                'mode': 'two_phase_triton',
                ...
                'SCALE_N': SCALE_N,
                'BLOCK_SIZE_M_Q': BLOCK_SIZE_M_Q,
                'BLOCK_SIZE_N_Q': BLOCK_SIZE_N_Q,
                'NUM_ITER': NUM_ITER,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if buf['mode'] == 'fused_splitk':
        ...
        # Two-phase: quant A, then fp4*fp4 Triton GEMM with XCD remap
        _fused_mxfp4_quant_shuffle_kernel[buf['quant_grid']](
            A,
            buf['x_fp4'],
            buf['blockscale'],
            *A.stride(),
            *buf['x_fp4'].stride(),
            M=M,
            N=K,
            BLOCK_SIZE_M=buf['BLOCK_SIZE_M_Q'],
            BLOCK_SIZE_N=buf['BLOCK_SIZE_N_Q'],
            NUM_ITER=buf['NUM_ITER'],
            NUM_STAGES=buf['NUM_STAGES_Q'],
            MXFP4_QUANT_BLOCK_SIZE=32,
            SCALING_MODE=0,
            SCALE_N_PAD=buf['SCALE_N'],
            num_warps=buf['NUM_WARPS_Q'],
            waves_per_eu=0,
            num_stages=1,
        )

        # Prepare A scales in preshuffled format: (SCALE_M//32, SCALE_N*32)
        bs_a = buf['blockscale']
        a_scales = bs_a.view(bs_a.shape[0] // 32, bs_a.shape[1] * 32)
        # B_q: (N, K//2), B transpose for kernel: (K//2, N)
        B_q_u8 = B_q.view(torch.uint8)
        B_q_T = B_q_u8.T  # non-contiguous view, stride-based access
        # B scales in preshuffled format: reshape from (SCALE_M_pad, SCALE_N) to (SCALE_M_pad//32, SCALE_N*32)
        bs_u8 = B_scale_sh.view(torch.uint8)
        b_scales = bs_u8.reshape(bs_u8.shape[0] // 32, bs_u8.shape[1] * 32)

        gc = buf['gemm_config']
        out = buf['gemm_out']
        K_fp4 = buf['K_fp4']

        _gemm_afp4wfp4_kernel_preshuffle_scales[(buf['gemm_grid_size'],)](
            buf['x_fp4'],
            B_q_T,
            out,
            a_scales,
            ...
            b_scales.stride(0),
            b_scales.stride(1),
            BLOCK_SIZE_M=gc['BLOCK_SIZE_M'],
            BLOCK_SIZE_N=gc['BLOCK_SIZE_N'],
            BLOCK_SIZE_K=gc['BLOCK_SIZE_K'],
            GROUP_SIZE_M=gc['GROUP_SIZE_M'],
            NUM_KSPLIT=gc['NUM_KSPLIT'],
            SPLITK_BLOCK_SIZE=gc['SPLITK_BLOCK_SIZE'],
            num_warps=gc['num_warps'],
            num_stages=gc['num_stages'],
            waves_per_eu=gc['waves_per_eu'],
            matrix_instr_nonkdim=gc['matrix_instr_nonkdim'],
            cache_modifier=gc['cache_modifier'],
        )
