...
"""
v176: .wt (write-through) cache modifier for fp4 output stores in M=256 quant kernel.

Hypothesis: The M=256 two-phase path writes fp4 data from the quant kernel
that is immediately consumed by the ASM GEMM. Using .wt (write-through) instead
of default .wb (write-back) pushes data to L2 simultaneously with the L1 write,
potentially making it available to GEMM CUs faster. The GEMM reads the fp4 data
from L2/VRAM, so write-through reduces the latency between quant output and
GEMM input.

Also try .wt on scale stores (currently .cg). The scales are also consumed
by the GEMM immediately.

Target: M=256 two-phase path (quant kernel stores).
"""
import torch
import triton
