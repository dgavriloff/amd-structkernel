...
"""
v163: BSK=512 for the split-K path (16x2112x7168) to reduce K-iterations per split.

Hypothesis: split-K=7 currently uses BSK=256, meaning each split does ~4 K-iterations.
With BSK=512, each split does ~2 K-iterations. Fewer loop iterations reduce loop overhead
and may allow better instruction scheduling. BSK=512 requires BSK//32//8=2 for scale reshape
(valid). NW=4 should provide enough bandwidth for the wider tile.

Target: 16x2112x7168 (~11.5us BM).
"""
import torch
import triton
...
def _get_fused_config(M, N, K):
    ...
    """
    if K > 4096:
        # Custom split-K=7 BSK=512 for large-K shapes (e.g., 16x2112x7168)
        # BSM=8: 238 blocks (0.93 waves), wider K-tile for fewer loop iterations
        return {
            "BLOCK_SIZE_M": 8,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
