...
"""
v136: Fused split-K=3 for M=256 N=3072 K=1536 to eliminate quant kernel.

Hypothesis: M=256 two-phase path costs ~13.3us (7us quant + 6.2us GEMM).
Fused preshuffle with split-K=3 BSM=16 BSK=256 creates 1152 blocks with
2 K-iterations per block (enables pipelining). Eliminates the 7us quant
kernel at the cost of slower Triton GEMM + ~2us reduce kernel.
Break-even estimate: ~11.3us GEMM + ~2us reduce = ~13.3us.
...
def _get_fused_config(M, N, K):
    ...
    if M <= 4:
        ...
        # M=256 (256x3072x1536): split-K=3 BSM=16 BSN=128 BSK=256 NW=4 NS=2
        # Grid: 3*16*24 = 1152 blocks, 2 K-iters per block with pipelining
        # Eliminates ~7us quant kernel launch overhead
        return {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 256,
