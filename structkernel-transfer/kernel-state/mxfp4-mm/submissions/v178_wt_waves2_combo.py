...
"""
v178: Combination sweep — two independently-neutral changes targeting different phases.

1. .wt (write-through) on M=256 quant fp4 stores: v176 showed consistent -1.5% on M=256
   across 2 LB runs. Write-through pushes fp4 data to L2 faster for ASM GEMM consumption.
2. waves_per_eu=2 for M<=4 fused path: v167 tested waves_per_eu=2 for ALL fused shapes
   and was neutral/slightly worse. This version only applies it to M<=4 where wave
   occupancy is most constrained (23 blocks for 256 CUs).

Target: Combination sweep — small independent effects may compound.
"""
import torch
import triton
