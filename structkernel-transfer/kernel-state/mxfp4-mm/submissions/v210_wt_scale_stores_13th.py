...
"""
v210: Based on v188 + .wt on M=256 scale stores.

Hypothesis: .wt cache modifier on M=256 quant scale stores (from .cg) —
confirmed real improvement across 12 prior tests (-0.8% to -3.1% on M=256 BM/LB).
Write-through pushes scale data to L2 faster for the subsequent ASM GEMM read.
M<=64 paths are completely unchanged (quant kernel only runs for M=256 two_phase).
"""
import torch
import triton
