...
"""
v058: Optimized fused path — simple reshape + data_ptr cache fix.

v057 proved the stale B cache was the root cause of leaderboard failures.
This version:
1. Uses simple reshape of B_shuffle/B_scale_sh (like v039), no re-quantization
2. Tracks B_shuffle.data_ptr() to invalidate cache on B data change
3. Limits fused path to M<=32 with K<=4096 (M=16 K=7168 was 84.5us fused vs ~14us two-phase)
"""
import torch
import triton
