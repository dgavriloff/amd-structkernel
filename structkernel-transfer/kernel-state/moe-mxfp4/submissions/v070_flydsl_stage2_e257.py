...
"""
v070: FlyDSL stage2 t32x128x256_atomic for bs512/E257/d=256.
v055 tested this and got -0.8% per-shape (249 vs 251us). Try again with explicit
stage1 kernel name to ensure it goes through the CK stage1 path (not heuristic).
Down-GEMM: N=7168, K=256, tile_n=128 -> 56 N-blocks, tile_k=256 -> 1 K-iteration.
"""
import torch
...
# Tuned CSV uses 64x32x32x128_1x1 for both stages (a4w4).
# Try keeping CK stage1 but using FlyDSL stage2 atomic.
# v055 showed -0.8% per-shape (249 vs 251us). Retry for confirmation.
