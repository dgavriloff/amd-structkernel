...
"""
v116: Buffer reuse for moe_sorting + fused_moe_2stages intermediate allocations.
Monkeypatch _moe_sorting_impl and fused_moe_2stages to cache and reuse
torch.empty allocations, avoiding 6+ allocation calls per invocation.
"""
import os
import functools
import torch
...
# Buffer cache for moe_sorting allocations
_sorting_buf_cache = {}
# Buffer cache for fused_moe_2stages a2 intermediate
_a2_buf_cache = {}
...
def _inject_configs():
    ...
    # Monkeypatch _moe_sorting_impl to reuse buffers
    _original_sorting_impl = _fused_moe_module._moe_sorting_impl
    ...
    def _cached_moe_sorting_impl(
        ...
        cache_key = (M, topk, num_experts, model_dim, moebuf_dtype, block_size)
        cached = _sorting_buf_cache.get(cache_key)
        if cached is not None:
            ...
            _sorting_buf_cache[cache_key] = (sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids, moe_buf)

        fwd_fn = aiter.moe_sorting_opus_fwd if use_opus else aiter.moe_sorting_fwd
        fwd_fn(
            topk_ids, topk_weights,
            sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids, moe_buf,
            num_experts, int(block_size), expert_mask, num_local_tokens, dispatch_policy,
        )
        return sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids, moe_buf

    _fused_moe_module._moe_sorting_impl = _cached_moe_sorting_impl
