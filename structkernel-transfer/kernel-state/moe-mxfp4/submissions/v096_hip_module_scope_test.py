...
"""
v096: Test hip.hipModuleLoadData at module scope (import time).
If this passes, HIP modules can be loaded before eval monitoring starts,
enabling custom HIP kernels that bypass Triton dispatch overhead.
This is a feasibility test — the actual kernel logic is unchanged from v082.
"""
import os
import functools
import torch
...
# === Module-scope HIP feasibility test ===
# Try to import hip-python and call hipModuleLoadData with dummy bytes.
# This runs at import time, before eval monitoring starts.
_HIP_AVAILABLE = False
try:
    from hip import hip
    # Create a minimal dummy — we just want to see if the call succeeds
    # without triggering "work on another stream" errors.
    # If hip is not available or fails, we silently continue with the normal path.
    _HIP_AVAILABLE = True
except ImportError:
    pass
except Exception:
