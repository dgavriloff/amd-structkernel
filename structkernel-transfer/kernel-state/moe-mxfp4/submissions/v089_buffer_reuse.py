...
"""
v089: Monkeypatch moe_sorting to reuse pre-allocated buffers.
Avoids torch.empty() overhead on each call by caching and reusing output tensors
...
def _inject_configs():
    ...
    # Monkeypatch _moe_sorting_impl to reuse pre-allocated buffers
    _original_moe_sorting_impl = _fused_moe_module._moe_sorting_impl
    _sorting_buf_cache = {}
    ...
    def _cached_moe_sorting_impl(
        ...
        cache_key = (M, topk, num_experts, model_dim, str(moebuf_dtype), block_size)

        if cache_key not in _sorting_buf_cache:
            _sorting_buf_cache[cache_key] = {
        ...
        bufs = _sorting_buf_cache[cache_key]
