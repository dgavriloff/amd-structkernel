...
"""
v084: block_m=64 + 4-WG stage1 + 1-WG MPerBlock=64 stage2 for E=257/bs=512.
Previous: block_m=32 + 1-WG 32x32 kernels = 8 scheduling rounds for stage1.
New: block_m=64 + 4-WG = 1 scheduling round for stage1, larger stage2 tiles (128 vs 32).
Keeps NT=True from v082.
"""
import os
import functools
import torch
...
# === bs=512/E=257: block_m=64 + 4-WG stage1 + MPerBlock=64 stage2 + NT=True ===
# Previous (v082): block_m=32 + 1-WG 32x32 kernels. Stage1 has N=512, so 512/32=16 N-tiles.
# With ~130 active experts and 1 M-tile each: 130*16=2080 tiles, 8 scheduling rounds.
# New: 4-WG stage1 (NPerBlock=128), 512/128=4 N-tiles, 130*4=520 tiles.
# With 4 WGs per CU: 520/(256*4)=0.5 -> 1 scheduling round.
# Stage2: MPerBlock=64 NPerBlock=128 vs old 32x32. Larger tiles = better throughput.
_CUSTOM_CONFIGS[_make_key(512, 256, 257)] = {
    "block_m": 64,
    "ksplit": 0,
    "kernelName1": _4WG_STAGE1,
    "kernelName2": "moe_ck2stages_gemm2_64x64x128x128_1x1_MulABScaleExpertWeightShuffled_v3_Nswizzle0_Quant3_MulRoutedWeight1_FP4X2_FP4X2_B16",
    "run_1stage": False,
