...
"""
v047: block_m=128 + MPerBlock=128 4-WG (256x128x128x128_1x4) for bs=512/E=33/d=2048.
v042 used block_m=64 + 4-WG MPerBlock=64 (339µs). block_m=128 creates fewer tile groups
(976 vs 1680). With 4-WG: 976/(256*4)=0.95 rounds vs 1680/(256*4)=1.64 rounds.
The heuristic already picks block_m=128 but uses its own kernel selection. Here we
explicitly inject the 4-WG kernel to ensure 4 workgroups per CU.
NOTE: v035 (GPU fault) used MPerBlock=64 with block_m=128 (mismatch).
Here MPerBlock=128 matches block_m=128 - should be safe.
"""
import torch
...
# === bs=512/E=33/d=2048: block_m=128 + MPerBlock=128 4-WG stage1 ===
# The heuristic already selects block_m=128 (fewest rounds: 4 with 1-WG).
# With 4-WG (256x128x128x128_1x4): 976/(256*4)=0.95 rounds = effectively 1 round.
# v042 used block_m=64 + 4-WG MPerBlock=64 (1.64 rounds). This should be better.
...
# NOTE: bs=512/E=33/d=512 NOT injected (4-WG neutral for this shape)
