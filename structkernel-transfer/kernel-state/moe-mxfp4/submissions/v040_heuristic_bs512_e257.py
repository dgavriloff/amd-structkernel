...
"""
v040: Force heuristic path for bs=512/E=257/d=256 by removing tuned CSV config.
Hypothesis: The tuned CSV sets block_m=32 and NT=False for this shape. But the
heuristic would select block_m=64 (better CU utilization) and NT=True
(tokens_per_expert=17 < 64). This combination may improve bs512/E257.
"""
import torch
...
# NOTE: bs=512/E=33 shapes are NOT injected (cktile_moe much worse for those)
...
def _inject_configs():
    ...
    # Remove tuned CSV config for bs=512/E=257/d=256 to force heuristic path
    # Heuristic selects block_m=64 + NT=True vs CSV's block_m=32 + NT=False
    _key_512_257 = _make_key(512, 256, 257)
    _fused_moe_module.cfg_2stages.pop(_key_512_257, None)
