...
"""
v129: FlyDSL t64x128x128_atomic stage2 for d=2048.
Try tile_m=64 with tile_k=128. tile_m=64 processes 2 rows per warp (vs 1 with tile_m=32),
potentially better for the larger M dimension with block_m=128 (2 sub-tiles vs 4).
Keep d=512 as t32x128x128 (v125 winner), d=2048 test only.
"""
import os
import functools
import torch
...
_flydsl_moe_kernels._KERNEL_PARAMS["flydsl_moe2_afp4_wfp4_bf16_t64x128x128_atomic"] = {
    "stage": 2, "a_dtype": "fp4", "b_dtype": "fp4", "out_dtype": "bf16",
    "tile_m": 64, "tile_n": 128, "tile_k": 128, "mode": "atomic", "MPerBlock": 64,
}
...
_FLYDSL_STAGE2_M64_K128 = "flydsl_moe2_afp4_wfp4_bf16_t64x128x128_atomic"

_CUSTOM_CONFIGS[_make_key(512, 2048, 33)] = {
    "block_m": 128,
    "ksplit": 0,
    "kernelName1": _4WG_STAGE1_M128,
    "kernelName2": _FLYDSL_STAGE2_M64_K128,  # v129: tile_m=64, tile_k=128 for d=2048
    "run_1stage": False,
}

# === bs=512/E=33/d=512: block_m=128 + 4-WG M128 stage1 + FlyDSL stage2 ===
# Keep d=512 as t32x128x128 (v125/v126 winner)
_CUSTOM_CONFIGS[_make_key(512, 512, 33)] = {
    "block_m": 128,
    "ksplit": 0,
    "kernelName1": _4WG_STAGE1_M128,
    "kernelName2": _FLYDSL_STAGE2_K128,
    "run_1stage": False,
}
