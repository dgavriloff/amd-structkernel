...
"""
v113: Override cktile_moe block_m to 32 for bs128/E33/d=512.
cktile_moe hardcodes block_m=16 for token<2048 (line 950 in fused_moe.py).
With 34.9 tokens/expert, block_m=32 reduces tile groups from 105 to 69 (-34%).
"""
import os
import functools
import torch
...
# With 139 tokens/expert, block_m=128 gives 2 tiles/expert (vs 3 with block_m=64).
# Fewer scheduling rounds may compensate for higher last-tile padding waste.
...
# Block_m overrides for cktile_moe shapes (keyed by (token, inter_dim, expert))
# These override the hardcoded block_m=16 in the cktile_moe return path
_CKTILE_BLOCK_M_OVERRIDES = {
    (128, 512, 33): 32,  # bs128/E33/d=512: 34.9 tok/expert, 69 vs 105 tile groups
}
...
def _inject_configs():
    ...
    # Monkeypatch get_2stage_cfgs to support use_non_temporal_load and block_m overrides
    ...
    def _patched_get_2stage_cfgs(
        ...
        # Override block_m for cktile_moe shapes
        override_key = (token, inter_dim, expert)
        if override_key in _CKTILE_BLOCK_M_OVERRIDES and metadata.ksplit > 1:
            new_block_m = _CKTILE_BLOCK_M_OVERRIDES[override_key]
            if metadata.block_m != new_block_m:
