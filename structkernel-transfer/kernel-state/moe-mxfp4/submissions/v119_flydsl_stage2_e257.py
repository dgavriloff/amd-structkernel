...
"""
v119: FlyDSL stage2 for bs512/E257 instead of CK stage2.
CK stage2 with NT=True gives 242-248µs. FlyDSL t32x128x256_atomic gave 249µs in v055.
With NT=True stage1 (kept from v082/v106), FlyDSL stage2 might perform differently
since FlyDSL uses atomic accumulation with different memory access patterns vs CK.
"""
import os
import functools
import torch
...
# Keep NT=True for stage1 (saves -1% from v082). Use FlyDSL stage2 instead of CK.
# FlyDSL uses atomic accumulation with different memory access patterns.
_CUSTOM_CONFIGS[_make_key(512, 256, 257)] = {
    ...
    "kernelName2": _FLYDSL_STAGE2,  # flydsl_moe2_afp4_wfp4_bf16_t32x128x256_atomic
    "run_1stage": False,
    "use_non_temporal_load": True,  # custom flag, read by monkeypatch (stage1 only)
}
