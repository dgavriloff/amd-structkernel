...
"""
v046: block_m=32 + MPerBlock=32 4-WG stage1 kernel for bs=512/E=33/d=2048.
v044 showed block_m=32 gives massive d=2048 improvement (304µs vs 349µs) via reduced
padding (5655 vs 6711 max_tokens), but used MPerBlock=64 kernel causing block_m!=MPerBlock
correctness failure. Fix: use 256x32x128x128_1x4 kernel (MPerBlock=32, 4-WG) to match
block_m=32 sorting. Also inject matching MPerBlock=32 stage2 kernel for consistency.
"""
import torch
...
# === bs=512/E=33/d=2048: block_m=32 + MPerBlock=32 4-WG stage1 ===
# v044 showed block_m=32 reduces max_tokens from 6711 to 5655 (-15.7% padding),
# giving d=2048 at 304µs vs 349µs. But MPerBlock must match block_m for correctness.
# Use 256x32x128x128_1x4 (MPerBlock=32, 4-WG) + matching 64x32x32x128_1x1 stage2.
...
_STAGE2_M32 = "moe_ck2stages_gemm2_64x32x32x128_1x1_MulABScaleExpertWeightShuffled_v1_Nswizzle0_Quant3_MulRoutedWeight1_FP4X2_FP4X2_B16"

_CUSTOM_CONFIGS[_make_key(512, 2048, 33)] = {
    "block_m": 32,
    "ksplit": 0,
    "kernelName1": _4WG_STAGE1_M32,
    "kernelName2": _STAGE2_M32,
    "run_1stage": False,
}

# NOTE: bs=512/E=33/d=512 NOT injected (block_m=64 heuristic is optimal for d=512)
