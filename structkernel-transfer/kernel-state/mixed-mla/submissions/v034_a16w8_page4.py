...
"""
v034: a16w8 persistent with page_size=4 for bs>=128,kv<=1024.

v030 baseline: 45.6µs geomean, bs=256,kv=1k at 88.3µs.
page_size=16 for kv=1024 fails tolerance (v029, v032). Try page_size=4:
kv=1024/4 = 256 pages per batch element, 8 pages per KV split (32 splits).
This should be reliable while still reducing metadata/indptr overhead.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 4 if kv_seq_len >= 1024 else 1
