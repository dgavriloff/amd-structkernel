...
"""
v094: Per-shape NUM_KV_SPLITS tuning for bf16_persist.
Based on v089.

For bf16_persist ps=64 with kv=8192:
- 128 pages per batch, kv_granularity=64, so 128 work blocks per batch
- bs=4: 512 total work blocks / 32 splits = 16 blocks/split
- bs=256: 32768 total work blocks / 32 splits = 1024 blocks/split

For bs=256, try 16 splits: 2048 blocks/split (halves reduce phase cost).
For bs=4, 32 splits is fine (enough work per split).
For kv=1024 a16w8 ps=8: 128 pages per batch, kv_granularity=16, 64 work blocks
- bs=256: 16384 / 32 = 512 blocks/split. Try 16 splits: 1024 blocks/split.
"""
...
def _get_splits(batch_size, kv_seq_len):
    """Get optimal NUM_KV_SPLITS for shape."""
    # For large batch + large kv: fewer splits reduces reduce overhead
    if batch_size >= 128 and kv_seq_len >= 4096:
        return 16
    return 32
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, num_kv_splits):
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    key = ('a16w8', batch_size, kv_seq_len, page_size, num_kv_splits)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, FP8_DTYPE, page_size, num_kv_splits)
    _cache[key] = cached
    return cached
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 64
    num_kv_splits = _get_splits(batch_size, kv_seq_len)
    key = ('bf16_persist', batch_size, kv_seq_len, page_size, num_kv_splits)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, torch.bfloat16, page_size, num_kv_splits)
    _cache[key] = cached
    return cached
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, num_kv_splits, o = _get_cached_bf16_persist(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, num_kv_splits, o = _get_cached_a16w8(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
