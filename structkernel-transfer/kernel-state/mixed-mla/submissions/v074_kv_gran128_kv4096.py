...
"""
v074: kv_granularity=128 for ALL kv>=4096 persistent shapes (a16w8 + bf16_persist).
Based on v057.

Previous kv_granularity tests: v048 (32, -0.7%), v062 (32, -0.4%), both within noise.
Neither tested large values like 128. Larger granularity = fewer but bigger work chunks,
potentially better memory coalescing and less scheduling overhead for large sequences.
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, kv_granularity=None):
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
        kv_gran = None
    key = ('a16w8', batch_size, kv_seq_len, page_size, kv_gran)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, FP8_DTYPE, page_size, kv_granularity=kv_gran)
    _cache[key] = cached
    return cached
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 16
    kv_gran = 128
    key = ('bf16_persist', batch_size, kv_seq_len, page_size, kv_gran)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, torch.bfloat16, page_size, kv_granularity=kv_gran)
    _cache[key] = cached
    return cached
