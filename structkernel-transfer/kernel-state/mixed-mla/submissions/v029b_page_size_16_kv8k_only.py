...
"""
v029b: Use page_size=16 for fp8 persistent path on kv=8192 shapes only.

v029 showed massive improvements with page_size=16:
  bs=32,kv=8k: 53.5µs (was 99.7µs) = -46.3%
  bs=64,kv=8k: 56.6µs (was 145µs) = -61.0%
  bs=256,kv=8k: 80.4µs (was 295µs) = -72.7%
But failed tolerance at bs=256,kv=1024.

Fix: only use page_size=16 for kv>=4096, keep page_size=1 for kv<=1024.
"""
...
def _get_cached_fp8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    key = (batch_size, kv_seq_len, FP8_DTYPE, FP8_DTYPE, page_size)
    if key in _cache:
        return _cache[key]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if use_bf16:
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o = _get_cached_fp8(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )

        # Reshape KV buffer for the chosen page_size
