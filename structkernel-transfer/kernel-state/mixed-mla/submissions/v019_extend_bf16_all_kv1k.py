...
"""
v019: Extend non-persistent bf16 to bs<=256 kv<=1024.

Hypothesis: Non-persistent bf16 for bs=256,kv=1024 may beat fp8 persistent (103µs).
v011 tried persistent bf16 at bs=256,kv=1k and got 127µs (+24.5% vs fp8).
But non-persistent bf16 has been 26% faster than persistent (v014 data).
127 * 0.74 = ~94µs, which would beat fp8's 103µs.

Target: bs=256,kv=1024 from 103µs -> ~94µs (-9%)
"""
...
def _should_use_bf16(batch_size, kv_seq_len):
    ...
    if kv_seq_len <= 1024:
        return True  # All kv<=1024 shapes use bf16 non-persistent
    if batch_size <= 4:
        return True
    return False
