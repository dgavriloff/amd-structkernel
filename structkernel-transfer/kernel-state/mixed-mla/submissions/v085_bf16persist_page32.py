...
"""
v085: bf16 persistent page_size=32 for bs=4,kv>=4096.
Based on v057.

Hypothesis: bf16 (a16w16) KV has higher precision than fp8, so page_size=32
should maintain tolerance even though fp8 page_size=32 failed (v040).
For bs=4,kv=8k with page_size=32: 256 pages (vs 512 with ps=16),
better memory coalescing and fewer page table lookups.
v040 showed page_size=32 gave massive speedups (-35% for bs=256,kv=8k).
bf16_persist at bs=4 should benefit similarly.
"""
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 32 if kv_seq_len >= 4096 else 16
