...
"""
v078: a8w8 persistent for bs>=128,kv>=4096 (fp8 Q + fp8 KV).
Based on v057.

The a8w8 persistent kernel uses qseqlen=1 (native decode) while a16w8 forces qseqlen=4.
For large batch kv=8k shapes (52.7us at bs=256), the a8w8 kernel may be more efficient
despite needing Q quantization (~1-2us). fp8 Q also halves Q memory bandwidth.
"""
...
def _get_path(batch_size, kv_seq_len):
    """Decide which path to use: 'bf16', 'bf16_persist', 'a16w8', or 'a8w8'."""
...
def _get_cached_a8w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build all cached buffers for a8w8 persistent path (fp8 Q + fp8 KV)."""
    page_size = 16  # same as a16w8 for kv>=4096
...
def custom_kernel(data: input_t) -> output_t:
    """MLA decode with hybrid bf16/bf16_persist/a16w8/a8w8 strategy."""
    ...
    if path == 'bf16':
        ...
        # a8w8: fp8 Q + fp8 KV persistent (qseqlen=1 native decode kernel)
        ...
        q_fp8, q_scale = _quantize_fp8(q_reshaped)
