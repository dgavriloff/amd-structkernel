...
"""
v071: bf16 non-persistent for all bs=4 shapes (remove bf16_persist path).
Based on v057.

bf16 non-persistent at bs=4,kv=1024: 20.0µs. bf16_persist at bs=4,kv=8192: 19.2µs.
Non-persistent path avoids metadata scheduling overhead and uses auto-tuned splits.
For bs=4, non-persistent picks 16 splits (from auto-tuner formula). Persistent uses 32 splits
with fast_mode metadata scheduling. Test if non-persistent can match persistent for kv=8k.
"""
