...
"""
v013: fast_mode=True + 16 splits for bf16, 32 splits for fp8.

Changes from v009:
- fast_mode=True for metadata generation (both paths)
- bf16 path uses 16 splits (v012 showed bs=4,kv=1k dropped 22.7%)
- fp8 path keeps 32 splits (proven optimal)
"""
...
def _get_cached(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, q_dtype, kv_dtype):
    ...
    # bf16 uses 16 splits (less overhead for small shapes), fp8 uses 32
    use_bf16 = (q_dtype == torch.bfloat16)
    num_kv_splits = 16 if use_bf16 else 32
