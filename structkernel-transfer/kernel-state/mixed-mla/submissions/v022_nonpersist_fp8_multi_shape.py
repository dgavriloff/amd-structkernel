...
"""
v022: Non-persistent fp8 for bs=256,kv<=1024 AND bs=32,kv=8192.

Hypothesis: Combine two marginal improvements:
1. bs=256,kv=1024: Non-persistent fp8 with 1 split (skip reduce). v021 showed -4.6%.
2. bs=32,kv=8192: Non-persistent fp8 with 9 auto-tuned splits vs 32 persistent.
   Fewer splits = less reduce overhead.

v015 had tolerance issues at bs=64,kv=8k but bs=32 has less data and
may be fine numerically. Combined improvements might push past 1% geomean.

Target shapes: bs=256,kv=1k (103->98µs) + bs=32,kv=8k (99.7->~95µs)
Expected geomean: ~64.5µs (-2%)
"""
...
def _should_use_nonpersist_fp8(batch_size, kv_seq_len):
    """Use non-persistent fp8 for specific shapes where auto-tuned splits are better."""
    # bs=256,kv<=1024: auto-tuner picks 1 split, skip reduce
    if batch_size >= 128 and kv_seq_len <= 1024:
        return True
    # bs=32,kv=8192: auto-tuner picks 9 splits vs persistent 32
...
def _get_cached_nonpersist_fp8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build cached buffers for non-persistent fp8 path."""
