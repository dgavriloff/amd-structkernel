...
"""
v031b: v030 + page_size=16 for bf16 non-persistent kv>=4096 shapes.

v031 showed bs=4,kv=8k: 17.0µs (was 35.2µs, -52%) but failed at bs=64,kv=1024
due to cached kv_indptr reference becoming stale. Fix: only cache page_size>1
tensors; use input kv_indptr directly for page_size=1.
"""
...
def _get_cached_bf16(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 16 if kv_seq_len >= 4096 else 1
    key = ('bf16', batch_size, kv_seq_len, page_size)
    if key in _cache:
        return _cache[key]
    ...
    if page_size > 1:
        num_pages = total_kv_len // page_size
        kv_indices = torch.arange(num_pages, dtype=torch.int32, device="cuda")
        kv_indptr_new = torch.arange(0, batch_size + 1, dtype=torch.int32, device="cuda") * (kv_seq_len // page_size)
        kv_last_page_len = torch.full((batch_size,), page_size, dtype=torch.int32, device="cuda")
    else:
        kv_indices = torch.arange(total_kv_len, dtype=torch.int32, device="cuda")
        kv_indptr_new = None  # Use input kv_indptr at call time
        kv_last_page_len = (kv_indptr[1:] - kv_indptr[:-1]).to(torch.int32)

    o = torch.empty((q_total, NUM_HEADS, V_HEAD_DIM), dtype=torch.bfloat16, device="cuda")

    cached = (kv_indices, kv_last_page_len, kv_indptr_new, page_size, o)
    _cache[key] = cached
    return cached
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        kv_indices, kv_last_page_len, kv_indptr_cached, page_size_bf16, o = _get_cached_bf16(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )

        kv_buffer_4d = kv_buffer.view(-1, page_size_bf16, NUM_KV_HEADS, kv_buffer.shape[-1])
        kv_indptr_use = kv_indptr_cached if kv_indptr_cached is not None else kv_indptr
