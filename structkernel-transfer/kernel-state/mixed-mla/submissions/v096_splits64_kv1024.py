...
"""
v096: 64 splits for a16w8 kv=1024 shapes.
Based on v089.

kv=1024 with ps=8 gives 128 pages/batch, kv_granularity=16 -> 64 work blocks/batch.
With 32 splits, only 2 blocks per split.
With 64 splits, 1 block per split = maximum parallelism.
For bs=256: 256*64=16384 work blocks spread across 64 splits -> 256 blocks/split.
MI355X has 304 CUs, so 64 splits may improve CU saturation for smaller batches.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    num_kv_splits = NUM_KV_SPLITS_KV1024 if kv_seq_len <= 1024 else NUM_KV_SPLITS
