...
"""
v069: Per-shape NUM_KV_SPLITS tuning with fast_mode=True.
Based on v057.

Auto-tuner model suggests fewer splits for large batches:
  bs=256,kv=8k: 7 splits (was 32)
  bs=64,kv=8k: 9 splits (was 32)
  bs=256,kv=1k: 1 split (was 32)
Previous v027 tried *more* splits (48) with no improvement.
Now trying *fewer* splits for large batch shapes.
"""
...
def _get_num_kv_splits(batch_size, kv_seq_len):
    """Get per-shape NUM_KV_SPLITS. Fewer splits for large batches where CU saturation is high."""
    if batch_size >= 256 and kv_seq_len >= 4096:
        return 8
    if batch_size >= 64 and kv_seq_len >= 4096:
        return 16
    return NUM_KV_SPLITS  # 32
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    num_splits = _get_num_kv_splits(batch_size, kv_seq_len)
