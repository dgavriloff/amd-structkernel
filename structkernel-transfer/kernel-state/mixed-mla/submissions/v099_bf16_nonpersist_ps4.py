...
"""
v099: bf16 non-persistent with page_size=4 for bs<=4, kv<=1024.
Based on v089.

Currently bf16 non-persistent uses page_size=1 for bs<=4,kv<=1024.
Hypothesis: page_size=4 groups consecutive KV tokens, potentially
improving memory access patterns in the ASM kernel. The kernel uses
log2(page_size) to compute page boundaries, so larger pages reduce
...
def _get_cached_bf16(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, page_size=1):
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        kv_indices, kv_last_page_len, kv_indptr_use, ps, o = _get_cached_bf16(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr, page_size=bf16_page_size
        )

        kv_buffer_4d = kv_buffer.view(-1, ps, NUM_KV_HEADS, kv_buffer.shape[-1])
