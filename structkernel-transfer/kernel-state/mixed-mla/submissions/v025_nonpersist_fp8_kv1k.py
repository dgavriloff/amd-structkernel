...
"""
v025: Non-persistent fp8 for bs>=128 kv<=1024.

Changes from v016:
- For bs>=128 with kv<=1024, use non-persistent fp8 instead of persistent.
  The auto-tuner picks 1 KV split for these shapes, avoiding the 32-way
  split overhead and reduce kernel. Direct stage1 + skip reduce.
- v021 showed -4.6% for bs=256,kv=1k (103->98µs) but only -0.3% geomean.
  With 0.1% keep threshold, this should count.
- Target: bs=256,kv=1k from ~103µs -> ~98µs
"""
...
# Cache per (batch_size, kv_seq_len, path)
_cache = {}
...
def _get_cached_fp8_persistent(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build all cached buffers for fp8 persistent path."""
    key = ('fp8_persist', batch_size, kv_seq_len)
    if key in _cache:
        return _cache[key]
...
def _get_cached_fp8_nonpersist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build cached buffers for fp8 non-persistent path."""
    key = ('fp8_nonpersist', batch_size, kv_seq_len)
    if key in _cache:
        return _cache[key]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
    ...
    elif path == 'fp8_nonpersist':
        kv_buffer_fp8, kv_scale = kv_data["fp8"]
        q_fp8, q_scale = _quantize_fp8(q)

        kv_indices, kv_last_page_len, o = _get_cached_fp8_nonpersist(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )

        kv_buffer_4d = kv_buffer_fp8.view(kv_buffer_fp8.shape[0], PAGE_SIZE, NUM_KV_HEADS, kv_buffer_fp8.shape[-1])

        # Non-persistent fp8: auto-tuned splits, no metadata
        mla_decode_fwd(
        ...
        meta, kv_indices, kv_last_page_len, o = _get_cached_fp8_persistent(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
