...
"""
v064: Combo sweep — kv_granularity=32 + NUM_KV_SPLITS=48 for a16w8 kv>=4096.
Based on v057.

Both kv_granularity=32 (v062, -0.4%) and 48 splits for a16w8 kv>=4096 (v061, +0.6%)
were individually neutral with fast_mode=True. Combining them may compound.
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, num_kv_splits=NUM_KV_SPLITS, kv_granularity=None):
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, FP8_DTYPE, page_size, num_kv_splits=num_splits, kv_granularity=kv_gran)
    _cache[key] = cached
    return cached
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, torch.bfloat16, page_size, num_kv_splits=NUM_KV_SPLITS)
    _cache[key] = cached
    return cached
