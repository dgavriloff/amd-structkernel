...
"""
v060: Per-shape NUM_KV_SPLITS tuning with fast_mode=True.
Based on v057.

v027 tried 48 splits without fast_mode (-0.2%). v052 tried 24 splits without fast_mode (-0.4%).
fast_mode=True changes scheduling algorithm — optimal splits may differ.
Try 48 splits for kv>=4096 (more KV data to split) and 32 for kv=1024.
"""
...
NUM_KV_SPLITS_DEFAULT = 32
NUM_KV_SPLITS_LONG_KV = 48  # for kv>=4096 with fast_mode=True
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        page_size = 16
        num_kv_splits = NUM_KV_SPLITS_LONG_KV
    elif kv_seq_len >= 1024 and batch_size >= 32:
        page_size = 8
        num_kv_splits = NUM_KV_SPLITS_DEFAULT
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 16
    num_kv_splits = NUM_KV_SPLITS_LONG_KV  # kv>=4096 uses more splits
