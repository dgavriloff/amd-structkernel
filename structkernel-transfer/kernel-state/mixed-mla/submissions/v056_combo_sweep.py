...
"""
v056: Combination sweep of three individually-neutral changes from exhausted branches.
Based on v047.

Combines:
1. kv_granularity=32 for kv>=4096 (v048: -0.7% within variance)
2. fast_mode=True for metadata on bs>=128,kv>=4096 (v051: -0.7% within variance)
3. NUM_KV_SPLITS=24 for a16w8 kv<=1024 page_size=8 (v052: -0.4% within variance)

Each was individually within noise. Combined, they target different phases
(granularity, scheduling, split count) and may compound.
"""
...
def _get_path(batch_size, kv_seq_len):
    ...
    if batch_size <= 4 and kv_seq_len <= 1024:
        return 'bf16'
    if batch_size <= 4 and kv_seq_len >= 4096:
        return 'bf16_persist'
    return 'a16w8'


def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, num_kv_splits=NUM_KV_SPLITS, kv_granularity=None, fast_mode=False):
    """Build persistent mode metadata for given Q/KV dtypes and page_size."""
    total_kv_len = batch_size * kv_seq_len

    if kv_granularity is None:
    ...
    return meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, num_kv_splits, o
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        page_size = 16
        # Change 1: kv_granularity=32 for kv>=4096 (was max(page_size,16)=16)
        kv_gran = 32
        # Change 2: fast_mode=True for large batch + long KV
        fm = batch_size >= 128
        nkv_splits = NUM_KV_SPLITS
    elif kv_seq_len >= 1024 and batch_size >= 32:
        page_size = 8
        kv_gran = None  # default
        fm = False
        # Change 3: NUM_KV_SPLITS=24 for a16w8 kv<=1024 page_size=8
        nkv_splits = 24
        ...
        fm = False
        nkv_splits = NUM_KV_SPLITS
    key = ('a16w8', batch_size, kv_seq_len, page_size, nkv_splits, kv_gran, fm)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, FP8_DTYPE, page_size, num_kv_splits=nkv_splits, kv_granularity=kv_gran, fast_mode=fm)
    _cache[key] = cached
    return cached
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build cached buffers for bf16 persistent path with page_size=16."""
    page_size = 16
    key = ('bf16_persist', batch_size, kv_seq_len, page_size)
    if key in _cache:
        return _cache[key]
    # Use kv_granularity=32 for kv>=4096 (Change 1 also applied to bf16_persist)
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, torch.bfloat16, page_size, kv_granularity=32)
    _cache[key] = cached
    return cached
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, nkv_splits, o = _get_cached_bf16_persist(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, nkv_splits, o = _get_cached_a16w8(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
