...
"""
v083: Combo sweep - bf16_persist for bs=32,kv>=4096 + 16 splits for bs>=256,kv>=4096.
Based on v057.

Combines two previously-neutral changes targeting different shapes:
1. bf16_persist for bs=32,kv>=4096: v068 showed -3.6% at bs=32,kv=8k
2. NUM_KV_SPLITS=16 for bs>=256,kv>=4096: v069 showed -0.2% overall
Small independent effects may compound.
"""
...
def _get_path(batch_size, kv_seq_len):
    ...
    if batch_size <= 4 and kv_seq_len >= 4096:
        return 'bf16_persist'
    if batch_size <= 32 and kv_seq_len >= 4096:
        return 'bf16_persist'
    return 'a16w8'


def _get_splits(batch_size, kv_seq_len):
    """Get NUM_KV_SPLITS for the shape."""
    if batch_size >= 256 and kv_seq_len >= 4096:
        return 16
    return NUM_KV_SPLITS


def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, num_splits=None):
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    num_splits = _get_splits(batch_size, kv_seq_len)
    key = ('a16w8', batch_size, kv_seq_len, page_size, num_splits)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, FP8_DTYPE, page_size, num_splits=num_splits)
    _cache[key] = cached
    return cached
