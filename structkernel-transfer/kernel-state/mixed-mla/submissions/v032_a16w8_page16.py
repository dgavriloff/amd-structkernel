...
"""
v032: page_size=16 for a16w8 persistent path (bs>=128,kv<=1024).

v030 baseline: 45.6µs. bs=256,kv=1k at 88.3µs using a16w8 persistent with page_size=1.
page_size=16 dramatically improved kv=8k shapes for fp8. Try for a16w8 kv=1024.
1024/16 = 64 pages per batch. a16w8 uses bf16 Q (higher precision than a8w8).

Target: bs=256,kv=1k from 88.3µs -> potentially lower.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 16  # Use page_size=16 for better memory coalescing
