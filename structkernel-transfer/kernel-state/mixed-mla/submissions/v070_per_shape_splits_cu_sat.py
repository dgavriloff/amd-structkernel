...
"""
v070: Per-shape NUM_KV_SPLITS to maximize CU utilization.
Based on v057.

MI355X has 304 CUs. With intra_batch_mode, total work items = min(304, splits*batch_size).
v057 uses splits=32 for all shapes:
  bs=4:   32*4=128 of 304 CUs active (42%)
  bs=32:  32*32=1024 -> capped at 304 (100%)
  bs=64:  32*64=2048 -> capped at 304 (100%)
  bs=256: 32*256=8192 -> capped at 304 (100%)

Increase splits for bs=4 to saturate all 304 CUs: 304/4=76.
For bs=32: 304/32=10, already >32 splits fills CUs, keep 32.
"""
...
def _get_num_kv_splits(batch_size):
    """Choose NUM_KV_SPLITS to maximize CU utilization (304 CUs on MI355X)."""
    # With intra_batch_mode, total work = min(304, splits * batch_size)
    # We want splits * batch_size >= 304 for full CU saturation
    CU_COUNT = 304
    if batch_size * NUM_KV_SPLITS >= CU_COUNT:
        return NUM_KV_SPLITS  # Already saturates CUs
    # Increase splits to fill all CUs
    return (CU_COUNT + batch_size - 1) // batch_size
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size):
    ...
    num_splits = _get_num_kv_splits(batch_size)
    ...
    return meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, num_splits
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    num_splits = _get_num_kv_splits(batch_size)
    key = ('a16w8', batch_size, kv_seq_len, page_size, num_splits)
    if key in _cache:
        return _cache[key]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, num_splits = _get_cached_bf16_persist(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, num_splits = _get_cached_a16w8(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
