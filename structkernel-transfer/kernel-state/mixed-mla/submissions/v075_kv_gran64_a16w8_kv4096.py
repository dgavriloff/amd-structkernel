...
"""
v075: kv_granularity=64 for a16w8 kv>=4096.
Based on v057.

kv_gran=128 showed BM improvement (-9.6% for bs=32,kv=8k) but failed secret LB tolerance.
kv_gran=64 is between default (16) and 128 — may provide scheduling benefit without
exceeding error bounds. kv_granularity controls the work chunk size in metadata scheduling.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    kv_gran = 64 if kv_seq_len >= 4096 else None
