...
"""
v031: page_size=16 for bf16 non-persistent kv=8192 shapes (bs<=4).

v030 baseline: 45.6µs geomean. bs=4,kv=8k at 35.2µs uses bf16 non-persistent
with page_size=1. page_size=16 dramatically improved fp8 kv=8k shapes.
Try the same for bf16.

Target: bs=4,kv=8k from 35.2µs -> potentially lower.
"""
...
def _get_cached_bf16(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    cached = (kv_indices, kv_last_page_len, kv_indptr_use, page_size, o)
    _cache[key] = cached
    return cached
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        kv_indices, kv_last_page_len, kv_indptr_use, page_size, o = _get_cached_bf16(
