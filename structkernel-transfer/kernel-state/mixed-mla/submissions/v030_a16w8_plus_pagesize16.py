...
"""
v030: Combine v029b (page_size=16 for kv>=4096) + a16w8 for bs>=128 kv<=1024.

v029b achieved 46.5µs geomean with page_size=16 for kv=8k shapes.
v028 showed a16w8 (bf16 Q + fp8 KV) reduces bs=256,kv=1k from 103->88µs (-14.6%)
but regressed kv=8k shapes. Since kv=8k shapes now use page_size=16, the regressions
no longer affect them.

Target: bs=256,kv=1k from 104µs -> ~88µs. Expected geomean: ~43µs.
"""
