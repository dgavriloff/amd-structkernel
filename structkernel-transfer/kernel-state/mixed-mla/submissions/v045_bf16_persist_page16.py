...
"""
v045: bf16 persistent with page_size=16 for bs=4,kv>=4096.

bs=4,kv=8k currently uses bf16 non-persistent at 35.2µs.
Persistent bf16 with page_size=16 avoids fp8 quantization error
while getting page grouping benefits. No tolerance risk since full bf16.
"""
...
def _get_path(batch_size, kv_seq_len):
    ...
    if batch_size <= 4 and kv_seq_len >= 4096:
        return 'bf16_persist'
    # a16w8 for all remaining shapes (avoids Q quantization overhead)
    return 'a16w8'
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build cached buffers for bf16 persistent path with page_size."""
    page_size = 16  # bf16 persistent always uses page_size=16
