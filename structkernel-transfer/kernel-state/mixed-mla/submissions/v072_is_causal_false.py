...
"""
v072: is_causal=False for persistent metadata.
Based on v057.

For decode (q_seq_len=1), causal masking has no effect -- all KV tokens are before the query.
Both is_causal=True and False produce the same KV ranges. But the metadata scheduler has
different code paths: is_causal=True computes min(kv_length - qo_length + local_qo_end, kv_length)
while False uses kv_length directly. Removing the causal logic may reduce metadata scheduling time.
"""
