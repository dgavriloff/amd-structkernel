...
"""
v079: Combination sweep: kv_granularity=32 + direct stage1+reduce bypass.
Based on v057.

Combines two individually-neutral changes:
1. kv_granularity=32 for all persistent paths (v062: -0.4%)
2. Direct stage1_asm+reduce_v1 with pre-cached logits/attn_lse (v063: 0%)
These target different phases (metadata scheduling + kernel dispatch overhead).
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size):
    ...
    max_q_len = 1
    # Use kv_granularity=32 for all persistent paths
    kv_gran = 32
    ...
    # Pre-allocate logits and attn_lse for direct stage1+reduce bypass
    logits = torch.empty(
        (reduce_partial_map.size(0) * max_q_len, 1, NUM_HEADS, V_HEAD_DIM),
        dtype=aiter_dtypes.fp32,
        device="cuda",
    )
    attn_lse = torch.empty(
        (reduce_partial_map.size(0) * max_q_len, 1, NUM_HEADS, 1),
        dtype=aiter_dtypes.fp32,
        device="cuda",
    )

    return meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, logits, attn_lse
...
def custom_kernel(data: input_t) -> output_t:
    """MLA decode with hybrid bf16/bf16_persist/a16w8 strategy + direct kernel bypass."""
    ...
    if path == 'bf16':
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, logits, attn_lse = _get_cached_bf16_persist(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )

        kv_buffer_4d = kv_buffer.view(-1, page_size, NUM_KV_HEADS, kv_buffer.shape[-1])

        # Direct stage1_asm + reduce_v1 bypass (avoid mla_decode_fwd wrapper allocations)
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, logits, attn_lse = _get_cached_a16w8(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
