...
"""
v004: Use aiter.scaled_fp8_quant for Q quantization + pre-allocated buffers.

Changes from v003:
- Use aiter.scaled_fp8_quant if available (C++ fused kernel, single pass)
- Pre-allocate output buffer in cache (avoid per-call allocation)
- Cleaner fallback quantization path
"""
...
def _get_cached(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build all cached buffers."""
...
def custom_kernel(data: input_t) -> output_t:
    """MLA decode with aiter fp8 quant and cached buffers."""
    ...
    # Quantize Q to fp8 (aiter native or fallback)
