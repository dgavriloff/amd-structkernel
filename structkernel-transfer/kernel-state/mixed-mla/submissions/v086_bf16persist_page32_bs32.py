...
"""
v086: Extend bf16_persist page_size=32 to bs<=32,kv>=4096.
Based on v057.

Hypothesis: bf16 (a16w16) has higher precision than fp8, allowing page_size=32
without the tolerance failures seen with a16w8 page_size=32 (v040).
For bs<=32,kv>=4096: bf16_persist avoids fp8 dequant, and page_size=32
gives 2x fewer pages than page_size=16, improving memory coalescing.
v068 showed bf16_persist for bs<=32,kv>=4096 gave -3.6% at bs=32,kv=8k.
v085 showed bf16_persist page_size=32 for bs=4,kv=8k passed tolerance.
Combining: bf16_persist with page_size=32 for bs<=32.
"""
