...
"""
v094: bf16_persist page_size=4 for kv=1024 bs=32-64 only.
Based on v089.

v093 showed bf16_persist ps=8 for kv=1024 gives 3-9% speedup for bs=32/64
but failed secret LB tolerance. v092 showed bf16_persist ps=16 for kv=1024
fails with 16 errors (0.0098 max error, just over atol=0.008).

ps=4 gives 256 pages per batch for kv=1024 (4x more than ps=16).
More pages = finer-grained split-K accumulation = better accuracy.
kv_granularity=max(4,16)=16, so 64 work blocks per batch.

Only apply to bs=32/64 kv=1024 where v093 showed improvement.
Keep a16w8 ps=8 for bs=256 kv=1024 (regression in v093).
Keep bf16 non-persist for bs<=4.
"""
...
def _get_path(batch_size, kv_seq_len):
    ...
    if kv_seq_len >= 4096:
        return 'bf16_persist'
    # kv=1024, bs=32-64: bf16_persist ps=4
    if kv_seq_len <= 1024 and batch_size <= 64:
        return 'bf16_persist'
    # kv=1024, bs=256: keep a16w8
    return 'a16w8'
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 4  # kv=1024: 256 pages per batch, more accurate
