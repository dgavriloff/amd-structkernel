...
"""
v084: Reduce page_size from 8 to 2 for bs=64,kv=1024 only.
Based on v057.

page_size=8 at bs=64,kv=1024 is flaky (~50% fail rate at seed 1357).
This trades speed for reliability at that shape.
Expected regression: bs=64,kv=1k from ~21.9µs to ~35µs, geomean +3-4%.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 2  # page_size=8 flaky at bs=64,kv=1024 seed 1357
