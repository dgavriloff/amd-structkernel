...
"""
v044: a16w8 page_size=8 for bs=4,kv=8192.

v041 tried a16w8 page_size=16 for bs=4,kv=8k and got 32.1µs (was 34.6µs)
but secret leaderboard failed. Try page_size=8 instead (1024 pages per batch,
128 pages/split) for better tolerance.
"""
...
def _get_path(batch_size, kv_seq_len):
    """Decide which path to use: 'bf16' or 'a16w8'."""
    # bf16 non-persistent only for bs<=4 with kv<=1024
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096 and batch_size >= 32:
        page_size = 16
    elif kv_seq_len >= 4096:
        page_size = 8  # bs=4: safer tolerance with more pages
