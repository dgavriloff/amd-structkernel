...
"""
v093: bf16_persist page_size=8 for kv=1024 shapes (bs>=32).
Based on v089.

v091 (ps=32) and v092 (ps=16) failed tolerance for kv=1024.
page_size=8 gives 128 pages per batch element for kv=1024 — more pages
should improve split-K accumulation accuracy.
Replace a16w8 path for kv=1024 with bf16_persist ps=8.
"""
...
def _get_path(batch_size, kv_seq_len):
    ...
    if kv_seq_len >= 4096:
        return 'bf16_persist'
    # kv=1024, bs>=32: use bf16_persist with page_size=8
    return 'bf16_persist'
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 8  # kv=1024: 128 pages per batch
...
def custom_kernel(data: input_t) -> output_t:
    """MLA decode with hybrid bf16/bf16_persist strategy."""
    ...
    else:  # bf16_persist for both kv>=4096 and kv=1024 (bs>=32)
        kv_buffer = kv_data["bf16"]
