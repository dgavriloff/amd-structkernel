...
"""
v092: bf16_persist for ALL shapes. page_size=64 for kv>=4096, page_size=16 for kv<=1024.
Based on v089.

v091 showed page_size=32 fails for kv=1024. Try page_size=16 (64 pages per batch at kv=1024).
This replaces a16w8 for kv=1024 shapes with bf16_persist.
bf16 avoids fp8 dequant overhead but reads 2x data.
For kv=1024, the data is small enough that dequant savings may not offset 2x bandwidth.
"""
