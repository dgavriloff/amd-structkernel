...
"""
v094: bf16_persist page_size=8 for kv=1024 bs<=64 only.
Based on v089.

v093 showed bf16_persist ps=8 for ALL kv=1024 gives:
- bs=32,kv=1k: 20.0µs (neutral vs a16w8)
- bs=64,kv=1k: 21.4µs (-9% vs a16w8 23.5µs)
- bs=256,kv=1k: 29.2µs (+16.3% vs a16w8 25.1µs -- regression)
Secret LB failed (likely bs=256 tolerance).
This version only applies bf16_persist ps=8 to bs<=64,kv=1024.
Keep a16w8 ps=8 for bs=256,kv=1024.
"""
...
def _get_path(batch_size, kv_seq_len):
    ...
    if kv_seq_len >= 4096:
        return 'bf16_persist'
    # kv=1024, bs<=64: bf16_persist ps=8
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 8  # kv=1024: 128 pages per batch element
