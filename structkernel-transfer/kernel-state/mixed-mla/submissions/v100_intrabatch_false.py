...
"""
v100: intra_batch_mode=False for bf16_persist at bs>=128,kv>=4096.
Based on v089.

intra_batch_mode=True forces same splits per batch. For bs=256,kv=8k with ps=64,
there are only 2 blocks per batch. With intra_batch_mode=False, the cross-batch
scheduler can distribute work more flexibly across 304 CUs, potentially improving
load balance for large batches.
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, use_intra_batch=True):
    ...
    info = get_mla_metadata_info_v1(
        ...
        num_kv_splits=NUM_KV_SPLITS, intra_batch_mode=use_intra_batch,
    )
    ...
    get_mla_metadata_v1(
        ...
        intra_batch_mode=use_intra_batch,
        dtype_q=dtype_q,
        dtype_kv=dtype_kv,
    )
    ...
    return meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, use_intra_batch
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 64
    # Use intra_batch_mode=False for large batches (cross-batch scheduling)
    use_intra_batch = batch_size < 128
    key = ('bf16_persist', batch_size, kv_seq_len, page_size, use_intra_batch)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, torch.bfloat16, page_size, use_intra_batch=use_intra_batch)
    _cache[key] = cached
    return cached
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, use_intra_batch = _get_cached_bf16_persist(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, _ = _get_cached_a16w8(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
