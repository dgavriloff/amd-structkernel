...
"""
v066: NUM_KV_SPLITS=64 for bs>=256,kv>=4096 only. All other shapes keep 32.
Based on v057.

bs=256,kv=8k is the largest shape (52.7µs). With 256*16*32=131k work items
on 304 CUs, each CU gets ~431 items. More splits (64) could better saturate
CUs for this large shape while keeping 32 for smaller shapes.
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, num_kv_splits=NUM_KV_SPLITS):
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    # Use more splits for largest shape to improve CU utilization
    num_splits = 64 if batch_size >= 256 and kv_seq_len >= 4096 else NUM_KV_SPLITS
    key = ('a16w8', batch_size, kv_seq_len, page_size, num_splits)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, FP8_DTYPE, page_size, num_splits)
    _cache[key] = cached
    return cached
