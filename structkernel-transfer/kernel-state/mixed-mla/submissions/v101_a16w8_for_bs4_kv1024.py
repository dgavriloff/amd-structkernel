...
"""
v101: a16w8 persistent for bs=4,kv=1024 (replacing bf16 non-persistent).
Based on v089.

bs=4,kv=1024 currently uses bf16 non-persistent at ~20µs.
a16w8 persistent with page_size=2 and fast_mode=True might be faster
due to the persistent scheduler + fp8 KV half bandwidth.
"""
...
def _get_path(batch_size, kv_seq_len):
    """Decide which path to use: 'bf16_persist' or 'a16w8'."""
    if kv_seq_len >= 4096:
        return 'bf16_persist'
    return 'a16w8'
...
def custom_kernel(data: input_t) -> output_t:
    """MLA decode with hybrid bf16_persist/a16w8 strategy."""
