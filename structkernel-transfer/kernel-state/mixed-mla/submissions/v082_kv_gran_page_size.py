...
"""
v082: Use kv_granularity=page_size (8) instead of max(page_size,16)=16 for page_size=8 shapes.
Based on v057.

Finer kv_granularity creates more work blocks for the persistent scheduler,
improving load balancing across CUs. For page_size=8 with kv=1024:
kv_granularity=16 -> 64 blocks, kv_granularity=8 -> 128 blocks.
More blocks allows more even distribution across 304 CUs.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    # Use kv_granularity=page_size for finer work distribution when page_size < 16
    kv_gran = page_size if page_size < 16 else None
