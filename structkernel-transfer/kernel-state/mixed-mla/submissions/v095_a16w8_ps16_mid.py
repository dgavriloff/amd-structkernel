...
"""
v095: a16w8 page_size=16 for bs=32/64 kv=1024.
Based on v089.

v029 showed page_size=16 for all fp8 shapes fails at bs=256,kv=1024.
But v029b showed page_size=16 for kv>=4096 works great.
Try ps=16 for mid-batch kv=1024 only (bs=32/64), keep ps=8 for bs=256.
ps=16 gives 64 pages per batch (kv=1024/16) vs 128 pages with ps=8.
kv_granularity=16, so each work block processes 1 page of 16 tokens.
Fewer pages should reduce metadata and dispatch overhead.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        page_size = 16
    elif kv_seq_len >= 1024 and batch_size >= 128:
        page_size = 8  # bs=256: keep ps=8 (ps=16 fails tolerance per v029)
    elif kv_seq_len >= 1024 and batch_size >= 32:
        page_size = 16  # bs=32/64: try ps=16
