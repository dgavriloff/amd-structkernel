...
"""
v080: Non-persistent bf16 with manual num_kv_splits=32 for bs=4,kv=1024.
Based on v057.

The non-persistent auto-tuner caps at 16 splits. For bs=4,kv=1024 with 304 CUs,
16 splits gives 4*16*16=1024 work items (~3.4 waves). Manual 32 splits gives
4*32*16=2048 items (~6.7 waves), potentially better CU utilization despite
higher reduce overhead. Pre-cache num_kv_splits_indptr to avoid recomputation.
"""
...
BF16_NONPERSIST_SPLITS = 32  # Manual splits for non-persistent bf16 (auto-tuner caps at 16)

def _get_cached_bf16(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build cached buffers for bf16 non-persistent path with manual splits."""
    ...
    kv_last_page_len = (kv_indptr[1:] - kv_indptr[:-1]).to(torch.int32)
    o = torch.empty((q_total, NUM_HEADS, V_HEAD_DIM), dtype=torch.bfloat16, device="cuda")
    # Pre-cache num_kv_splits_indptr for manual splits
    num_kv_splits_indptr = torch.arange(
        0, (batch_size + 1) * BF16_NONPERSIST_SPLITS, BF16_NONPERSIST_SPLITS,
        dtype=torch.int, device="cuda"
    )

    cached = (kv_indices, kv_last_page_len, o, num_kv_splits_indptr)
    _cache[key] = cached
    return cached
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        kv_indices, kv_last_page_len, o, num_kv_splits_indptr = _get_cached_bf16(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )

        # Non-persistent mode with manual splits (>16, bypassing auto-tuner cap)
        mla_decode_fwd(
            ...
            sm_scale=SM_SCALE,
            logit_cap=0.0,
            num_kv_splits=BF16_NONPERSIST_SPLITS,
