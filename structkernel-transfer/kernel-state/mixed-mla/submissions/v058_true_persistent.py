...
"""
v058: True persistent mode (intra_batch_mode=False) for all persistent paths.
Based on v057 (fast_mode=True).

v057 used intra_batch_mode=True (fake persistent = same splits per batch).
True persistent (intra_batch_mode=False) uses CU-based work scheduling where
the metadata scheduler assigns work dynamically to CUs. This is what aiter's
test code uses for persistent mode. May improve work distribution for large batch.
"""
