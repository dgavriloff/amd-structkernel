...
"""
v105: a8w8 non-persistent with GPU-only Q quantization for bs>=128,kv<=1024.
Based on v089.

v078 tried a8w8 but used Python Q quantization with .item() CPU sync (~28µs).
Fast GPU-only quantization: q_scale=amax(abs(q))/448, q_fp8=(q/q_scale).to(fp8).
No CPU sync. 3 GPU kernels: amax, div, cast.
a8w8 non-persistent with 1 split (auto-tuned) may beat a16w8 persistent.
"""
...
_FP8_MAX = torch.finfo(FP8_DTYPE).max  # 448.0 for float8_e4m3fn


def _get_cached_a8w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    """Get or build cached buffers for a8w8 non-persistent path (fp8 Q + fp8 KV)."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        # a8w8: fp8 Q + fp8 KV non-persistent (fast GPU-only Q quantization)
        kv_buffer_fp8, kv_scale = kv_data["fp8"]
        kv_buffer_4d = kv_buffer_fp8.view(kv_buffer_fp8.shape[0], 1, NUM_KV_HEADS, kv_buffer_fp8.shape[-1])

        kv_indices, kv_last_page_len, o = _get_cached_a8w8(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )

        # GPU-only Q quantization: no CPU sync
        q_view = q.view(-1, NUM_HEADS, QK_HEAD_DIM)
        q_scale = q_view.abs().amax() / _FP8_MAX
        q_fp8 = (q_view / q_scale).to(FP8_DTYPE)
