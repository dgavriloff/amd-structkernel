...
"""
v104: intra_batch_mode=False for bf16_persist bs<=4.
Based on v089.

For bs=4 with ps=64 and kv=8192: only 2 KV blocks per batch.
intra_batch_mode=True assigns splits within each batch -> most CUs idle.
intra_batch_mode=False pools all 8 blocks across batches for better CU utilization.
v100 tested this for bs>=128 and regressed, but bs=4 has opposite problem (too few blocks per batch).
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, intra_batch=True):
    ...
    info = get_mla_metadata_info_v1(
        ...
        num_kv_splits=NUM_KV_SPLITS, intra_batch_mode=intra_batch,
    )
    ...
    return meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, intra_batch
...
def _get_cached_bf16_persist(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    page_size = 64
    use_intra_batch = batch_size > 4
    key = ('bf16_persist', batch_size, kv_seq_len, page_size, use_intra_batch)
    if key in _cache:
        return _cache[key]
    cached = _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, torch.bfloat16, torch.bfloat16, page_size, use_intra_batch)
    _cache[key] = cached
    return cached
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if path == 'bf16':
        ...
        meta, kv_indices, kv_last_page_len, kv_indptr_use, page_size, o, use_intra = _get_cached_bf16_persist(
            batch_size, kv_seq_len, q.shape[0], qo_indptr, kv_indptr
        )
