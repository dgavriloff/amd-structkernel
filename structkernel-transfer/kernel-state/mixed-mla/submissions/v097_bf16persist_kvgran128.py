...
"""
v097: bf16_persist kv_granularity=128 for kv>=4096.
Based on v089.

v089 uses kv_granularity=64 (= page_size). Increasing kv_granularity to 128
means each work block covers 2 pages (128 KV tokens) instead of 1.
This halves the number of work blocks from 128 to 64 for kv=8192,
reducing scheduling overhead. Previously tried for a16w8 (v074-v076)
where it failed tolerance due to fp8 accumulation. bf16 has more
precision headroom so should handle larger granularity safely.
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size, kv_granularity=None):
    ...
    get_mla_metadata_v1(
        ...
        page_size=page_size,
        kv_granularity=kv_granularity if kv_granularity is not None else max(page_size, 16),
