...
"""
v063: Pre-cache logits/attn_lse and bypass mla_decode_fwd for persistent paths.
Based on v057.

v046 tried bypassing wrapper without fast_mode (+1.6%). This time:
pre-allocate logits+attn_lse to avoid torch.empty() per call,
and call stage1_asm + reduce_v1 directly. With fast_mode=True,
the cached metadata may make the direct path faster.
"""
...
def _build_persistent_meta(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr, dtype_q, dtype_kv, page_size):
    ...
    # Pre-allocate stage1 intermediates for direct bypass
    n_partials = reduce_partial_map.size(0)
    logits = torch.empty(
        (n_partials, 1, NUM_HEADS, V_HEAD_DIM),
        dtype=torch.float32, device="cuda",
    )
    attn_lse = torch.empty(
        (n_partials, 1, NUM_HEADS, 1),
        dtype=torch.float32, device="cuda",
    )
