...
"""
v076: kv_granularity=32 for a16w8 kv>=4096 only (with fast_mode=True).
Based on v057.

v048 tested kv_gran=32 before fast_mode was enabled (v057). v056 tested combo but
included bf16_persist kv_gran=32 which regressed. This isolates kv_gran=32 to a16w8
kv>=4096 only, which is the path most likely to benefit.
"""
...
def _get_cached_a16w8(batch_size, kv_seq_len, q_total, qo_indptr, kv_indptr):
    ...
    if kv_seq_len >= 4096:
        ...
        page_size = 1
    kv_gran = 32 if kv_seq_len >= 4096 else None
